...
**The agents' faces are DiceBear clay, decided 22 September 2026.** After
the cloud bodies (reverted), a glass pebble and a Siri orb (shown, declined),
the founder chose DiceBear's clay style (CC0 1.0, `@dicebear/styles`
`clay.json`, generated by `@dicebear/core`, MIT). A face is generated on the
Mac from the agent's id (`desktop/source/shared/agent/clay-face.ts`): no
picture file, no network, stable for the agent's life, never the same for two
agents. The preload paints it over `.sand-grok-bot-mark` in the pinned 0.18.0
renderer (`desktop/source/electron-preload/clay-face-overlay.ts`, the same
overlay path and hide rules as the clouds had) and moves it with Grok Bot's
own motion table, verbatim (`desktop/source/shared/agent/face-motion.ts`):
the body bobs, leans, turns once on sending and bounces on receiving; the eyes
squash for openness; the pupils follow the pointer inside the whites. The
style's own idle animation is generated off. Measured in the harness
(`desktop/harness`, `node harness/face-preview.mjs`, `node harness/shoot.mjs
faces`) and in `docs/product/faces-clay-measured.md`. **Still unmeasured:**
whether the pinned 0.18.0 mark is shaped as the reconstruction says; the
`[CaisraFaceOverlay]` console line on a Mac is the reading.
