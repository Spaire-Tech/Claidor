...
**The agents' faces are DiceBear slice, keyed on Grok's avatar, decided 22
September 2026.** After the cloud bodies (reverted), a glass pebble and a
Siri orb (shown, declined) and the clay faces (shipped for an afternoon), the
founder chose DiceBear's slice style (CC0 1.0, `@dicebear/styles`
`slice.json`, generated by `@dicebear/core`, MIT). **The correction that came
with it matters more than the style:** the clay face was keyed on the agent's
id alone, fell back to the colour when a mark carried none, and ignored the
persisted avatar, so every agent of a colour wore one face and every cell of
the "change avatar" picker was the same. Grok Bot persists an avatar as a
shape and a colour and writes both onto every mark (`data-avatar-shape`,
`data-avatar-color`). The face follows them: Grok's shape picks the slice
shape (blob→lump, pebble→egg, … teardrop→lens), Grok's colour is the body
colour, and only the cut pattern comes from the agent's id
(`desktop/source/shared/agent/agent-face.ts`). So the picker's cells are the
real alternatives, a choice persists through Grok's own save, and no picture
file or network is involved. The preload paints it over `.sand-grok-bot-mark`
in the pinned 0.18.0 renderer (`desktop/source/electron-preload/agent-face-overlay.ts`)
and moves the body with Grok Bot's own motion table, verbatim
