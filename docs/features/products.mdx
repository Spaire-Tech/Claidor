---
title: "Products"
description: "Create digital products on Claidor in minutes"
---

<Info>
  **Everything is a product**

  Subscriptions or pay once products are both considered a product in Claidor (API & data model). Just with different pricing & billing logic. So both are shown & managed under Products with the ability to filter based on pricing model.
</Info>
...
<Info>
  **What if I want both a monthly and yearly pricing?**
  ...
  Claidor has a unique approach to what the industry typically calls **variants**. Each product has a single pricing model, but you can create multiple products with different pricing models, and showcase them both at checkout.
</Info>
...
Claidor has a unique approach regarding what the industry typically calls **variants**.
...
For fixed price products, you can change the price. Existing subscribers remain on their current pricing.

When you add benefits, existing subscribers receive them automatically. When you remove benefits, existing subscribers lose access to them.

## Archive a product

Products on Claidor can't be deleted, but you can **archive** them by clicking the **Archive** button at the bottom right of the product page.

Existing customers retain access to the product, and subscriptions continue to renew. However, the product will no longer be available for new purchases.

You can unarchive a product using the [Products Update API](/api-reference/products/update#body-is-archived).