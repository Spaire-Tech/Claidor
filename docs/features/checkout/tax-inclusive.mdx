...
By default, Claidor displays prices exclusive of tax. Tax is calculated and shown separately at checkout before the customer confirms their purchase.
...
When tax-inclusive pricing is set on a product price, Claidor back-calculates the tax component from the displayed price rather than adding it on top.
...
const product = await claidor.products.create({
  name: 'Pro Plan',
  prices: [
    {
      type: 'recurring',
...
with Claidor(access_token="<YOUR_ACCESS_TOKEN>") as claidor:
    product = claidor.products.create(request={
        "name": "Pro Plan",
        "prices": [
            {
                "type": "recurring",
...
```bash cURL
curl -X POST https://api.claidorhq.com/v1/products \
...
<CardGroup cols={2}>
  <Card icon="earth-europe" href="/merchant-of-record/introduction" title="Merchant of Record">
    How Claidor handles tax collection and remittance on your behalf.
  </Card>
