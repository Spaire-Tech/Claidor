...
You can easily sell software license keys with Claidor without having to deal with sales tax or hosting an API to validate them in real-time. License keys with Claidor come with a lot of powerful features built-in.

* Brandable prefixes, e.g `CLAIDOR_*****`
...
**Enable user to deactivate instances via Claidor.** Instead of building your own custom admin for customers to manage their activation instances - leave it to Claidor instead.
...
It's super easy and straightforward to integrate Claidor license keys into your application, library or API.
...
```bash Terminal
curl -X POST https://api.claidorhq.com/v1/customer-portal/license-keys/activate
...
```bash Terminal
curl -X POST https://api.claidorhq.com/v1/customer-portal/license-keys/validate
-H "Content-Type: application/json"
-d '{
  "key": "1C285B2D-6CE6-4BC7-B8BE-ADB6A7E304DA",
  "organization_id": "fda84e25-7b55-4d67-916d-60ead04ff61f",
