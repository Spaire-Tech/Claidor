...
Metered usage is billed as part of the subscription invoice. At the end of each billing period, Claidor tallies each meter, calculates the charge, and adds it to the invoice alongside the base subscription fee.
...
```ts
const preview = await claidor.subscriptions.chargePreview('<subscription_id>')
