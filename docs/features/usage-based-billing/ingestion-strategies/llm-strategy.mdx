...
```
pnpm add @claidor/ingestion ai @ai-sdk/openai
```
...
const llmIngestion = Ingestion({ accessToken: process.env.CLAIDOR_ACCESS_TOKEN })
...
Our Python SDK includes an ingestion helper and strategies for common use cases. It's installed as part of the Claidor SDK.

<CodeGroup>

```bash pip
pip install claidor-sdk
```
...
The ingestion helper is a simple wrapper around the Claidor events ingestion API. It takes care of batching and sending events to Claidor in the background, without blocking your main thread.
...
With our PydanticAI strategy, you can easily track the usage of LLMs and send the data to Claidor for billing.
