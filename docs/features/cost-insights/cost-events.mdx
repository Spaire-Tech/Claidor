...
Cost Insights works by allowing you to add a special `_cost` property to any event you ingest through Claidor's Event Ingestion API. These costs are then aggregated and made available through the Metrics API alongside revenue metrics.
...
```json cURL
POST https://api.claidorhq.com/v1/events/ingest
