...
<img
  ...
  src="/assets/features/customer-portal/overview.light.png"
/>

## Redirect to your Customer Portal

The customer portal is directly available from the URL `https://www.claidorhq.com/your-org-slug/portal`. Your customers will be able to authenticate there by entering the email they used to purchase or subscribe to your products.

<img
  className="block dark:hidden"
  src="/assets/features/customer-portal/signin.light.png"
/>
...
Using the Claidor API, all you need is to call the `customerSessions` endpoint. Here’s an example using our TypeScript SDK.
...
async function run() {
  const result = await claidor.customerSessions.create({
    customerId: "<value>",
  });
...
export const GET = CustomerPortal({
  ...
  server: 'sandbox' // Use sandbox if you're testing Claidor - pass 'production' otherwise
});
```