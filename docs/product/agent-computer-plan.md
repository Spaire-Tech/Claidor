...
This half of the document is the spec, measured against the tree as it stood on
18 September, when nothing was built. **Part Two, from "The measurement" on, is
what was then measured and built**, on 18–19 September. Where the two disagree,
Part Two is later and wins; the specific lines this build made false are
corrected in place below and say so.
...
|---|---|---|
| A box (Linux, persistent) | `openclaw-extensions/box/` | **Built**, never run against E2B (Part Two §8) |
| Registered user machines, `ListMachines` | `main/box/`, `ipcHandlers/box/` | **Built**, needs the server's route |
| `CopyToBox` / `CopyFromBox` | `main/box/custody.ts` | **Built**, guards tested |
| ~~`computerUse` subagent~~ | ~~`main/computerUse/`~~ | **Deleted** in `e84eb38e` — see below |
...
| `box-doctor` | `main/box/doctor.ts` | **Built** — ten checks in one round trip |
| Update / Reset Computer | `main/box/operations.ts` | **Built**, person-confirmed only |
| Per-agent live desktop preview | — | **Blocked**, not merely absent — Part Two §3c |
| Sandbox as a substrate | engine `sandbox.mode` | **On** since `ab08138e` — see below |
...
**Deleted on 18 September in `e84eb38e`**, in the change that built the box, as
recommended below. Each of the three claims above was re-checked against the
source first rather than taken from this paragraph.
...
**That gate is gone, since `ab08138e`.** Execution mode now decides whether to
sandbox and what is configured decides where; see Part Two §2 and
`libs/boxSandboxSettings.ts`. The rest of this section stands as it was written.
...
- ~~What does OpenClaw's `sandbox.mode: 'all'` actually provision?~~ **Answered**
  in Part Two §2–3: a pluggable backend registry, two shipped backends, and a
  browser that is Docker-only.
- ~~Does the engine's browser `target: "sandbox"` reach that same sandbox?~~
  **Answered, and it is the finding of Part Two**: §3c. It reaches a second
  Docker container over host loopback, and no plugin can change that.
- What does the maty runner actually do today, end to end? **Still unread.**
- Does E2B's own documentation say paused sandboxes survive past 30 days?
  **Still unverified, and it cannot be verified from the build container** —
  `api.e2b.dev` is refused by its egress proxy. It needs someone with a
  browser.
...
**18–19 September 2026.** Part One above is the spec and the substrate lock.
This half answers the question Part One ends on — *whether our engine drives the
box acceptably from the other side of the internet* — and then builds against
the answer.
...
## 0. A correction, and where the sources actually were
