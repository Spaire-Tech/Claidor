...
| model-call budget per turn, counted in the session the turn shell holds | `provider-session.ts` `createModelCallBudget`, `shared/inference/turn-step-budget.ts` | 5,000 for a turn the person asked for (Grok Bot's), 40 for a hidden one (`SAND_HIDDEN_TURN_MAX_STEPS`). **Per turn**: an intro whose kickstart sends nothing gets one reply nudge, a second hidden turn with its own 40 (F-298). **Until 25 September 2026 the flag never reached the executor on the production path** (`createAgentOwnerInput` dropped `runOptions.hidden`), so every hidden turn ran with 5,000; the `[claidor] model=` line now ends with `budget=40 hidden=true` on such a turn (F-001, F-015, F-117) |
| the intro greets and stops | `shared/agents/onboarding.ts` | told not to start the assignment or use tools; the toolset is not withheld (`hidden` changes only the budget, `skipLabeling` and the reply reminder). Corrected 25 September 2026, design-audit-ledger F-438 |
...
| the box stops on quit unless an enabled routine exists (25 September 2026: it is kept for routines, and renews its own model credential; `SAND_STOP_BOX_ON_QUIT=1` forces the stop) | `local-docker-host-connector.ts` `stopLocalDockerBoxOnQuit`, called from the quit flush in `main-production-services.ts` | on for local Docker; `SAND_KEEP_BOX_RUNNING_ON_QUIT=1` keeps it |
