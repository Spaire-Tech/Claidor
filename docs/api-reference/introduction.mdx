...
<CardGroup cols={2}>
  <Card title="Production Base URL" icon="globe">
    `https://api.claidorhq.com/v1`
  </Card>
  <Card title="Sandbox Base URL" icon="flask">
    `https://sandbox-api.claidorhq.com/v1`
  </Card>
...
| ----------- | --------------------------------- | ------------------------------- |
| Production  | `https://api.claidorhq.com/v1`         | Real customers & live payments  |
| Sandbox     | `https://sandbox-api.claidorhq.com/v1` | Safe testing & integration work |
...
```http
Authorization: Bearer claidor_oat_xxxxxxxxxxxxxxxxx
```
...
curl https://api.claidorhq.com/v1/products/ \
  -H "Authorization: Bearer $CLAIDOR_OAT" \
  -H "Accept: application/json"
```

```bash curl (Sandbox - Core API)
curl https://sandbox-api.claidorhq.com/v1/products/ \
  -H "Authorization: Bearer $CLAIDOR_OAT_SANDBOX" \
  -H "Accept: application/json"
```

```bash curl (Customer Portal API)
curl https://api.claidorhq.com/v1/customer-portal/orders/ \
  -H "Authorization: Bearer $CLAIDOR_CUSTOMER_TOKEN" \
  -H "Accept: application/json"
```
...
```py Python
from claidor import Claidor

client = Claidor(
    access_token=os.environ["CLAIDOR_ACCESS_TOKEN"],
    server="sandbox",
)
````
...
```go Go
s := claidorgo.New(
  claidorgo.WithServer("sandbox"),
  claidorgo.WithSecurity(os.Getenv("CLAIDOR_ACCESS_TOKEN")),
)
```

```php PHP
$sdk = Claidor\Claidor::builder()
    ->setServer('sandbox')
...
List endpoints in the Claidor API support pagination to help you efficiently retrieve large datasets. Use the `page` and `limit` query parameters to control pagination.
...
<CodeGroup>
```bash Request
curl https://api.claidorhq.com/v1/products/?page=1&limit=100 \
  -H "Authorization: Bearer $CLAIDOR_OAT" \
  -H "Accept: application/json"
```
...
Claidor API has rate limits to ensure fair usage and maintain performance. The limits are as follows:
