...
description: "Learn how to automatically schedule subscription downgrades at the end of the current billing period using the Claidor API and Upstash QStash."
---
...
- A [Claidor API access token](/integrate/authentication)
...
```bash Terminal
# Claidor API credentials
CLAIDOR_MODE="sandbox" # can be "production"
...
const claidor = new Claidor({
  server: process.env.CLAIDOR_MODE,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const subscription = await claidor.subscriptions.get({
      id: subscriptionId,
    });
...
async function handler(req: NextRequest) {
  try {
    ...
    const updatedSubscription = await claidor.subscriptions.update({
      id: subscriptionId,
      subscriptionUpdate: {
...
<Steps>
  <Step title="Test in Sandbox Environment">
    Use Claidor's [sandbox environment](/integrate/sandbox) to test without affecting production data.
