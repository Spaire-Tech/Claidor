...
description: "Learn how to grant meter credits to customers before they make any purchase using the Claidor API."
---
...
1. Create a customer in Claidor (if they don't exist)
...
<Steps>
  <Step title="Navigate to Meters">
    In the Claidor dashboard sidebar, click on **Products** > **Meters**.
  </Step>
...
If your customer doesn't already exist in Claidor, you need to create them first.

### Option A: Create via Dashboard

<Steps>
  <Step title="Navigate to Customers">
    In the Claidor dashboard sidebar, click on **Customers**.
  </Step>
...
Use the Claidor SDK or API to create a customer programmatically:
...
await claidor.customers.create({
...
<Tip>
  Use the `externalId` field to link Claidor customers with your internal user system. This allows you to reference customers without storing Claidor's internal ID.
</Tip>

### Using External ID

If you set an `externalId` when creating the customer, you can use it in event ingestion instead of the Claidor customer ID:
...
const meters = await claidor.customerMeters.list({
