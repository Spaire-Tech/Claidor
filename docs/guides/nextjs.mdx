---
title: "Integrate Claidor with Next.js"
sidebarTitle: "Next.js"
description: "In this guide, we'll show you how to integrate Claidor with Next.js."
---
...
## Install the Claidor JavaScript SDK

To get started, you need to install the Claidor JavaScript SDK and the Claidor Nextjs helper package. You can do this by running the following command:

```bash Terminal
pnpm install @spaire/sdk @claidor/nextjs
```
...
To authenticate with Claidor, you need to create an access token, and supply it to Next.js using a `CLAIDOR_ACCESS_TOKEN` environment variable.

You can create an organization access token from your organization settings.

## Configuring a Claidor API Client

To interact with the Claidor API, you need to create a new instance of the `Claidor` class. This class uses the provided access token to authenticate with the Claidor API.

```typescript
// src/claidor.ts
import { Claidor } from "@spaire/sdk";

export const api = new Claidor({
...
```bash Terminal
# .env
CLAIDOR_ACCESS_TOKEN="claidor_pat..."
...
```typescript
// src/app/api/webhook/claidor/route.ts
