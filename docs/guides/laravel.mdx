---
title: "Integrate Claidor with Laravel"
sidebarTitle: "Laravel"
description: "In this guide, we'll show you how to integrate Claidor with Laravel."
hidden: true
---

<img src="/assets/guides/laravel/hero.jpeg" />

Consider following this guide while using the Claidor Sandbox Environment. This will allow you to test your integration without affecting your production data.

Claidor Laravel Example App
--------------------------------

We've created a simple example Laravel application that you can use as a reference.

[View Code on GitHub](https://github.com/claidorhq/claidor-laravel)
...
To authenticate with Claidor, you need to create an access token, and supply it to Laravel using a `CLAIDOR_API_KEY` environment variable.

You can create an organization access token from your organization settings.

Fetching Claidor Products for display
------------------------------------------
...
class ProductsController extends Controller
{
    ...
    {
        // Change from sandbox-api.claidorhq.com -> api.claidorhq.com when ready to go live
        // And don't forget to update the .env file with the correct CLAIDOR_ORGANIZATION_ID and CLAIDOR_WEBHOOK_SECRET
        $data = Http::get('https://sandbox-api.claidorhq.com/v1/products', [
            'is_archived' => false,
        ]);
...
Generating Claidor Checkout Sessions
-----------------------------------------

This endpoint will be responsible for creating a new checkout session, redirecting the user to the Claidor Checkout page & redirect back to a configured confirmation page.
...
class CheckoutController extends Controller
{
    public function handle(Request $request)
    {
        ...
        // Claidor will replace {CHECKOUT_ID} with the actual checkout ID upon a confirmed checkout
        $confirmationUrl = $request->getSchemeAndHttpHost() . '/confirmation?checkout_id={CHECKOUT_ID}';

        // Change from sandbox-api.claidorhq.com -> api.claidorhq.com when ready to go live
        // And don't forget to update the .env file with the correct CLAIDOR_ORGANIZATION_ID and CLAIDOR_WEBHOOK_SECRET
        $result = Http::withHeaders([
            'Authorization' => 'Bearer ' . env('CLAIDOR_API_KEY'),
            'Content-Type' => 'application/json',
        ])->post('https://sandbox-api.claidorhq.com/v1/checkouts/custom/', [
...
class ConfirmationController extends Controller
{
    public function handle(Request $request)
    {
        ...
        $data = Http::withHeaders([
            'Authorization' => 'Bearer ' . env('CLAIDOR_API_KEY'),
            'Content-Type' => 'application/json',
        ])->get('https://sandbox-api.claidorhq.com/v1/checkouts/custom/' . $request->query('checkout_id'));
...
Handling Claidor Webhooks
------------------------------

Claidor can send you events about various things happening in your organization. This is very useful for keeping your database in sync with Claidor checkouts, orders, subscriptions, etc.
...
1. Point the Webhook to `your-app.com/api/webhook/claidor`. This must be an absolute URL which Claidor can reach. If you use ngrok, the URL will look something like this: `https://<your-ngrok-id>.ngrok-free.app/api/webhook/claidor`.
...
3. Generate a secret key to sign the requests. This will allow you to verify that the requests are truly coming from Claidor.
4. Add the secret key to your environment variables.

```bash Terminal
# .env
CLAIDOR_API_KEY="claidor_oat..."
...
```php
// routes/api.php
Route::webhooks('/webhook/claidor');
```
...
return [
    'configs' => [
        [
             ...
             */
            'signing_secret' => env('CLAIDOR_WEBHOOK_SECRET'),
            ...
            'signature_validator' => App\Handler\ClaidorSignature::class,
...
* ClaidorSignature.php
* ProcessWebhook.php

Inside app/Handler/ClaidorSignature.php, what we want to do is to validate that the request came from Claidor. Add the code to that file.

```php
// app/Handler/ClaidorSignature.php
<?php
...
class ClaidorSignature implements SignatureValidator
{
