...
- Claidor organization with `seat_based_pricing_enabled` feature flag
- Claidor SDK installed (`npm install @spaire/sdk` or `pip install claidor-sdk`)
...
- Basic understanding of Claidor products and subscriptions
...
<Steps>
  <Step title="Navigate to Products">
    In the Claidor dashboard, go to **Products** and click **Create Product**.
  </Step>
...
```typescript
const subscriptionProduct = await claidor.products.create({
  name: "Team Pro Plan",
...
```typescript
const oneTimeProduct = await claidor.products.create({
...
```typescript
// Webhook handler
app.post('/webhooks/claidor', async (req, res) => {
...
async function getSeatInfo(params: { subscription_id?: string; order_id?: string }) {
  const { seats, available_seats, total_seats } =
    await claidor.customerSeats.list(params);
...
async function assignSeat(
  ...
  try {
    const seat = await claidor.customerSeats.assign({
      ...params,
      email: email,
...
async function handleClaimPage(token: string) {
  ...
  const claimInfo = await claidor.customerSeats.getClaimInfo({
...
async function claimSeat(token: string) {
  const { seat, customer_session_token } =
    await claidor.customerSeats.claim({
      invitation_token: token
    });

  // Store the customer session token
  // This allows immediate portal access
  localStorage.setItem('claidor_session', customer_session_token);
...
async function revokeSeat(seatId: string) {
  const revokedSeat = await claidor.customerSeats.revoke({
    seat_id: seatId
  });
...
async function addSeats(subscriptionId: string, newTotal: number) {
  ...
  const subscription = await claidor.subscriptions.update({
    id: subscriptionId,
    seats: newTotal
  });
...
async function reduceSeats(subscriptionId: string, newTotal: number) {
  const { seats } = await claidor.customerSeats.list({
...
```typescript
await claidor.customerSeats.assign({
...
async function getTeamSubscriptions(customerId: string) {
  const subs = await claidor.customerPortal.seats.listSubscriptions({
...
```typescript
await claidor.customerSeats.resend({ seat_id: seatId });
```
