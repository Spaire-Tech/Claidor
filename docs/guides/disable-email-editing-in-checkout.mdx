...
By passing either `customer_id` or `external_customer_id` when creating a checkout session, Claidor will:
...
If you've already created a customer in Claidor and have their Claidor customer ID, you can use it directly.

<Steps>
    <Step title = "Get the Customer ID">
        Retrieve the customer ID from your Claidor dashboard or through the [Customers API](/api-reference/customers/list).
...
const claidor = new Claidor({ accessToken: process.env["CLAIDOR_ACCESS_TOKEN"] });
...
If you have your own user management system, you can use your internal customer ID. This is the recommended approach as it makes reconciliation between your system and Claidor easier.
