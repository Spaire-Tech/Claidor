...
There are two ways to grant initial meter credits in Claidor:
...
- A Claidor account with an organization
- A meter created with **Sum** aggregation
- A product with the meter attached
- Webhooks enabled
- A Claidor access token for API calls
...
```typescript icon="square-js" title="app/api/webhook/claidor/route.ts"
...
<Steps>
  <Step title="Use the sandbox environment">
    Test your integration in Claidor's [sandbox environment](/integrate/sandbox) to avoid affecting production data.
  </Step>
