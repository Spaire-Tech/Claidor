...
Seamlessly integrate Claidor.sh subscriptions and payments into your Laravel application. This package provides an elegant way to handle subscriptions, manage recurring payments, and interact with Claidor's API. With built-in support for webhooks, subscription management, and a fluent API, you can focus on building your application while we handle the complexities of subscription billing.

<Note>
  This provider is not maintained or officially supported by Claidor. Use at your own discretion. If you have questions about the provider, please contact the project maintainer.
</Note>
...
```bash Terminal
composer require danestves/laravel-claidor
```

**Step 2:** Run `:install`:

```bash Terminal
php artisan claidor:install
```
...
```bash Terminal
php artisan vendor:publish --tag="claidor-migrations"
```

```bash Terminal
php artisan vendor:publish --tag="claidor-config"
```

```bash Terminal
php artisan vendor:publish --tag="claidor-views"
```
...
return [
    /*
    |--------------------------------------------------------------------------
    | Claidor Access Token
    |--------------------------------------------------------------------------
    |
    | The Claidor access token is used to authenticate with the Claidor API.
    | You can find your access token in the Claidor dashboard > Settings
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Claidor Webhook Secret
    |--------------------------------------------------------------------------
    |
    | The Claidor webhook secret is used to verify that the webhook requests
    | are coming from Claidor. You can find your webhook secret in the Claidor
    ...
    |
    */
    'webhook_secret' => env('CLAIDOR_WEBHOOK_SECRET'),
    ...
    |--------------------------------------------------------------------------
    |
    | This is the base URI where routes from Claidor will be served
    | from. The URL built into Claidor is used by default; however,
    | you can modify this path as you see fit for your application.
    |
    */
    'path' => env('CLAIDOR_PATH', 'claidor'),
    ...
    | from a checkout session in your Claidor account.
    |
    */
    'redirect_url' => null,
    ...
    |
    */
    'currency_locale' => env('CLAIDOR_CURRENCY_LOCALE', 'en'),
];
```
...
Configure your access token. Create a new token in the Claidor Dashboard \> Settings \> Developers and paste it in the `.env` file.

- [https://sandbox.claidorhq.com/dashboard/ORG\_SLUG/settings](https://sandbox.claidorhq.com/dashboard/ORG_SLUG/settings) (Sandbox)
- [https://www.claidorhq.com/dashboard/ORG\_SLUG/settings](https://www.claidorhq.com/dashboard/ORG_SLUG/settings) (Production)
...
Configure your webhook secret. Create a new webhook in the Claidor Dashboard \> Settings \> Webhooks.

- [https://sandbox.claidorhq.com/dashboard/ORG\_SLUG/settings/webhooks](https://sandbox.claidorhq.com/dashboard/ORG_SLUG/settings/webhooks) (Sandbox)
- [https://www.claidorhq.com/dashboard/ORG\_SLUG/settings/webhooks](https://www.claidorhq.com/dashboard/ORG_SLUG/settings/webhooks) (Production)
...
Claidor includes a JavaScript script that you can use to initialize the [Claidor Embedded Checkout](https://www.claidorhq.com/docs/features/checkout/embed). If you going to use this functionality, you can use the `@claidorEmbedScript` directive to include the script in your views inside the `<head>` tag.
...
This package includes a webhook handler that will handle the webhooks from Claidor.
...
Or if you're using Laravel v11 and up, you should exclude `claidor/*` in your application's `bootstrap/app.php` file:
...
This package includes a list of commands that you can use to retrieve information about your Claidor account.

| Command | Description |
| --- | --- |
| `php artisan claidor:products` | List all available products with their ids |
...
```blade
<x-claidor-button :checkout="$checkout" />
```
...
```blade
<x-claidor-button :checkout="$checkout" data-claidor-checkout-theme="dark" />
```
...
```php
public function claidorName(): ?string; // default: $model->name
public function claidorEmail(): ?string; // default: $model->email
```
...
Customers can update their personal information (e.g., name, email address) by accessing their [self-service customer portal](https://www.claidorhq.com/docs/features/customer-portal). To redirect customers to this portal, call the `redirectToCustomerPortal()` method on your billable model (e.g., the User model).
...
```blade
@if ($order->refunded())
    Order {{ $order->claidor_id }} was refunded on {{ $order->refunded_at->toFormattedDateString() }}
@endif
```
...
Claidor does not offer immediate cancellation. To resume a subscription while it is still in its grace period, use the resume method.
...
Claidor can send webhooks to your app, allowing you to react. By default, this package handles the majority of the work for you. If you have properly configured webhooks, it will listen for incoming events and update your database accordingly. We recommend activating all event kinds so you may easily upgrade in the future.

#### Webhook Events

- `Danestves\LaravelClaidor\Events\BenefitGrantCreated`
- `Danestves\LaravelClaidor\Events\BenefitGrantUpdated`
- `Danestves\LaravelClaidor\Events\BenefitGrantRevoked`
- `Danestves\LaravelClaidor\Events\OrderCreated`
- `Danestves\LaravelClaidor\Events\OrderRefunded`
- `Danestves\LaravelClaidor\Events\SubscriptionActive`
- `Danestves\LaravelClaidor\Events\SubscriptionCanceled`
- `Danestves\LaravelClaidor\Events\SubscriptionCreated`
- `Danestves\LaravelClaidor\Events\SubscriptionRevoked`
- `Danestves\LaravelClaidor\Events\SubscriptionUpdated`
...
use Danestves\LaravelClaidor\Events\WebhookHandled;

class ClaidorEventListener
{
    /**
     * Handle received Claidor webhooks.
     */
...
The [Claidor documentation](https://www.claidorhq.com/docs/integrate/webhooks/events) includes an example payload.
...
use App\Listeners\ClaidorEventListener;
...
Please see [CHANGELOG](https://github.com/danestves/laravel-claidor/blob/main/CHANGELOG.md) for more information on what has changed recently.

## License

The MIT License (MIT). Please see [License File](https://github.com/danestves/laravel-claidor/blob/main/LICENSE.md) for more information.