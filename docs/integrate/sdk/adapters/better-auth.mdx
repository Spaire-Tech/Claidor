...
A [Better Auth](https://www.better-auth.com) plugin for integrating [Claidor](https://www.claidorhq.com) payments and subscriptions into your authentication flow.
...
- [Handle Claidor Webhooks securely with signature verification](#webhooks-plugin)
- [Customer Portal](#portal-plugin)

## Installation

Install the required Better Auth and Claidor packages using the following command:

<Tabs>
  <Tab title="npm">
    ```bash Terminal
    npm install better-auth @claidor/better-auth @spaire/sdk
    ```
  </Tab>
  <Tab title="yarn">
    ```bash Terminal
    yarn add better-auth @claidor/better-auth @spaire/sdk
    ```
  </Tab>
  <Tab title="pnpm">
    ```bash Terminal
    pnpm add better-auth @claidor/better-auth @spaire/sdk
    ```
  </Tab>
  <Tab title="bun">
    ```bash Terminal
    bun add better-auth @claidor/better-auth @spaire/sdk
    ```
  </Tab>
</Tabs>

## Integrate Claidor with BetterAuth

<Steps>
  <Step title="Configure Claidor Access Token">
    Go to your Claidor Organization Settings, create an Organization Access Token, and add it to the environment variables of your application.
  ...
  <Step title="Configure BetterAuth Server">
    The Claidor plugin comes with it's own set of plugins to add functionality to your stack:
    ...
    - **webhooks** - Listen for relevant Claidor webhooks

    ```typescript BetterAuth Server with Claidor Example icon=square-js
    import { betterAuth } from "better-auth";
    import { claidor, checkout, portal, usage, webhooks } from "@claidor/better-auth"; // [!code ++]
    import { Claidor } from "@spaire/sdk"; // [!code ++]
    
    const claidorClient = new Claidor({ // [!code ++]
        accessToken: process.env.CLAIDOR_ACCESS_TOKEN, // [!code ++]
        // Use 'sandbox' if you're using the Claidor Sandbox environment
    ...
    const auth = betterAuth({
        ...
        plugins: [
            claidor({ // [!code ++]
                client: claidorClient, // [!code ++]
                createCustomerOnSignUp: true, // [!code ++]
                use: [ // [!code ++]
                    checkout({ // [!code ++]
                        products: [ // [!code ++]
                            { // [!code ++]
                                productId: "123-456-789", // ID of Product from Claidor Dashboard // [!code ++]
                    ...
                    usage(), // [!code ++]
                    webhooks({ // [!code ++]
                        secret: process.env.CLAIDOR_WEBHOOK_SECRET, // [!code ++]
    ...
    #### Claidor Plugin Configuration Options
    ...
    const auth = betterAuth({
      ...
      plugins: [
        claidor({
          ...
          use: [ // [!code ++]
            // This is where you add Claidor plugins // [!code ++]
          ], // [!code ++]
        }),
      ],
    });
    ...
    - `client` (required): Claidor SDK client instance
    - `createCustomerOnSignUp` (optional): Automatically create a Claidor customer when a user signs up
    - `getCustomerCreateParams` (optional): Custom function to provide additional customer creation metadata
    - `use` (optional): Array of Claidor plugins to enable specific functionality (checkout, portal, usage, and webhooks)
  </Step>
  <Step title="Configure BetterAuth Client">
    You will be using the BetterAuth Client to interact with the Claidor functionalities.

    ```typescript BetterAuth Client with Claidor Example icon=square-js
    import { createAuthClient } from "better-auth/react";
    import { claidorClient } from "@claidor/better-auth/client"; // [!code ++]
    import { organizationClient } from "better-auth/client/plugins"; // [!code ++]
    
    // All Claidor plugins, etc. should be attached to BetterAuth server
    export const authClient = createAuthClient({ // [!code ++]
      plugins: [claidorClient()], // [!code ++]
    }); // [!code ++]
    ```
  </Step>
</Steps>
...
Enable the `createCustomerOnSignUp` [Claidor plugin configuration option](#claidor-plugin-configuration-options) to automatically create a new Claidor Customer when a new User is added in the BetterAuth database.

All new customers are created with an associated `externalId`, i.e. the ID of your User in the Database. This skips any Claidor to User mapping in your database.

## Sync Customer deletion

To add user deletion logic with external Claidor customer deletion in BetterAuth, extend the user config of your betterAuth setup with the deleteUser option and an afterDelete hook. Here’s how to integrate this alongside your Claidor plugin and automatic customer creation:

```typescript Customer Deletion Example icon=square-js
const auth = betterAuth({
  user: {
 // [!code ++]
    deleteUser: {
 ...
 // [!code ++]
        await claidor.customers.deleteExternal({
 // [!code ++]
...
<Steps>
  <Step title="Use Checkout Plugin">
    Update the `use` property of the Claidor plugin for BetterAuth client to have the `checkout` plugin.

    ```typescript Checkout Plugin Example icon=square-js
    import {
      claidor,
      checkout // [!code ++]
    } from "@claidor/better-auth";
  ...
  <Step title="Create checkouts using BetterAuth client">
    ...
    - **`products`** (optional): An array of Claidor Product IDs.
    ...
    ```typescript BetterAuth Checkout with Claidor Example icon=square-js
    ...
    ```typescript BetterAuth Checkout with Claidor Organization Example icon=square-js
    const organizationId = (await authClient.organization.list())?.data?.[0]?.id,
    
    await authClient.checkout({
        // Any Claidor Product ID can be passed here
...
```typescript Usage Plugin Example icon=square-js
import {
  claidor, checkout, portal,
  usage // [!code ++]
} from "@claidor/better-auth";
...
Claidor's Usage Based Billing builds entirely on event ingestion. Ingest events from your application, create Meters to represent that usage, and add metered prices to Products to charge for it.
...
```typescript Event Ingestion with Usage Plugin Example icon=square-js
...
```typescript Customer Meters with Usage Plugin Example icon=square-js
...
The `webhooks` plugin can be used to capture incoming events from your Claidor organization.

To set up the Claidor `webhooks` plugin with the BetterAuth client, follow the steps below:

<Steps>
  <Step title="Configure Webhook Endpoints in Claidor">
    Configure a Webhook endpoint in your Claidor Organization Settings page by following [this guide](/integrate/webhooks/endpoints). Webhook endpoint is configured at `/api/auth/claidor/webhooks`.
  </Step>
  <Step title="Add the Webhook Secret">
    Add the obtained webhook secret to your application environment as an environment variable (to be used as `process.env.CLAIDOR_WEBHOOK_SECRET`):

    ```bash .env
    CLAIDOR_WEBHOOK_SECRET="..."
    ```
  </Step>
  <Step title="Use Webhooks Plugin in BetterAuth client">
    Pass the `webhooks` plugin in the `use` property.

    ```typescript Webhooks Plugin Example icon=square-js
    import {
      claidor,
      webhooks // [!code ++]
    } from "@claidor/better-auth";
...
The `webhooks` plugin allows you to invoke handlers for all Claidor webhook events:
...
```typescript Portal Plugin Example icon=square-js
import {
  claidor, checkout,
  portal // [!code ++]
} from "@claidor/better-auth";
...
The following method will redirect the user to the Claidor Customer Portal, where they can see their orders, purchases, subscriptions, benefits, etc.

```typescript Open Customer Portal Example icon=square-js
...
```typescript Retrieve Customer State Example icon=square-js
...
Learn more about the Claidor Customer State [in the Claidor Docs](https://www.claidorhq.com/docs/integrate/customer-state).
...
```typescript List User Benefits Example icon=square-js
...
```typescript List User Orders Example icon=square-js
...
```typescript List Organization Subscriptions Example icon=square-js
const organizationId = (await authClient.organization.list())?.data?.[0]?.id,
...
```typescript List User Subscriptions Example icon=square-js
