...
description: "Add Claidor checkout and usage billing to your project with guided AI agent commands in Claude Code"
sidebarTitle: "Agent Commands"
---

Agent commands are [Claude Code custom commands](https://docs.anthropic.com/en/docs/claude-code/tutorials#create-custom-slash-commands) that teach the AI agent how to integrate Claidor into your project. The agent reads your codebase, detects your framework, and writes production-ready integration code — interactively, with your approval at every step.

## How It Works

Claude Code doesn't know about Claidor out of the box. The agent commands are Markdown files you add to your project's `.claude/commands/` directory. Once added, Claude Code loads them as custom slash commands and knows exactly how to set up Claidor checkout, usage billing, webhooks, and more — tailored to your stack.
...
<Steps>
  ...
  <Step title="Add the command files to your project">
    Create a `.claude/commands/` directory in your project root and download the Claidor command files:
    ...
    <Tip>
      Commit these files to your repo. Anyone on your team who uses Claude Code will automatically get the Claidor commands.
    </Tip>
  </Step>
...
<Info>
  **Why command files?** Claude Code custom commands are project-scoped. By adding the `.claude/commands/` files to your project, the agent gets detailed instructions on how to integrate Claidor — including which SDKs to use, what dashboard steps to guide you through, safety guardrails for production codebases, and framework-specific code templates. Without these files, Claude Code has no knowledge of Claidor.
</Info>
...
- **No silent account mutations** — Never creates products, meters, or prices in your Claidor account; walks you through dashboard steps manually
...
The `/setup-checkout` command adds Claidor checkout to your project. The agent asks which approach you want:

| Approach | Description | Best for |
|---|---|---|
| **Overlay** | Embed script + `data-claidor-checkout` links. No backend needed. | Landing pages, simple apps |
| **Programmatic** | `ClaidorEmbedCheckout.create()` for dynamic control. | Apps needing custom trigger logic |
| **Server-side** | Checkout sessions via `@spaire/sdk` or `@claidor/nextjs`. | Apps with backends, dynamic pricing |

### What the agent does

1. Scans your project files to detect your framework
2. Asks if you have products in Claidor (walks you through creation if not)
...
| [MCP](/integrate/mcp) | AI agents querying your Claidor data at runtime |
