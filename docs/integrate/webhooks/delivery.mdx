...
description: "How to parse, validate and handle webhooks and monitor their deliveries on Claidor"
---
...
You now need to setup a route handler for the endpoint registered on Claidor to receive, validate and parse webhooks before handling them according to your needs.
...
```typescript JS (Express) icon=square-js
  ...
  (req: Request, res: Response) => {
    try {
      const event = validateEvent(
        req.body,
        req.headers,
        process.env['CLAIDOR_WEBHOOK_SECRET'] ?? '',
      )
...
from claidor_sdk.webhooks import validate_event, WebhookVerificationError
...
def webhook():
    try:
        event = validate_event(
            ...
            secret=os.getenv('CLAIDOR_WEBHOOK_SECRET', ''),
        )
...
Both examples above expect an environment variable named `CLAIDOR_WEBHOOK_SECRET` to be set to the secret you configured during the endpoint setup.

### Custom validation

We follow the [Standard Webhooks](https://www.standardwebhooks.com/) standard which offers many libraries across languages to easily validate signatures. You can follow their specification in case you want to roll your own.
...
Seeing deliveries on Claidor, but not receiving them on your end? Below are some common techniques to resolve the issue depending on the reported error status.
...
- Try adding trailing `/` to the URL on Claidor. Often `/foo` is resolved to `/foo/` by frameworks.

`HTTP 3xx`

Redirect responses (301, 302, 307, etc.) are treated as failures. Claidor does not follow redirects for webhook deliveries. Update your webhook URL to the final destination URL to avoid redirects.
