...
<Steps>
  <Step title="Add new endpoint">
    ...
    <img
      ...
      src="/assets/integrate/webhooks/create.light.png"
    />
  </Step>
  <Step title="Specify your endpoint URL">
    ...
    <img
      className="block dark:hidden"
      src="/assets/integrate/webhooks/url.light.png"
    />
    ...
    <Tip>
      **Developing locally?**
      ...
      Just be sure to provide the URL ngrok gives you as the webhook endpoint on Claidor.
    </Tip>
  </Step>
  <Step title="Choose a delivery format">
    ...
    If you wish to send notifications to a Discord or Slack channel, you can select the corresponding format here. Claidor will then adapt the payload so properly formatted messages are sent to your channel.

    <img
      className="block dark:hidden"
      src="/assets/integrate/webhooks/format.light.png"
    />

    If you paste a Discord or Slack Webhook URL, the format will be automatically selected.
  </Step>
  <Step title="Set a secret">
    We cryptographically sign the requests using this secret. So you can easily verify them using our SDKs to ensure they are legitimate webhook payloads from Claidor.

    You can set your own or generate a random one.

    <img
      className="block dark:hidden"
      src="/assets/integrate/webhooks/secret.light.png"
    />
  </Step>
