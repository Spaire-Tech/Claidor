...
```bash
export SKILLS_ROOT="${CAISRA_SKILLS_ROOT:-${LOBSTERAI_SKILLS_ROOT:-${SKILLS_ROOT:-$HOME/Library/Application Support/Caisra/SKILLs}}}"
export PWCLI="$SKILLS_ROOT/playwright/scripts/playwright_cli.sh"
"$PWCLI" --help
```

Installed skills resolve from `$CAISRA_SKILLS_ROOT` / `$LOBSTERAI_SKILLS_ROOT` / `$SKILLS_ROOT` (production default: app `userData/SKILLs`, macOS usually `~/Library/Application Support/Caisra/SKILLs`).
