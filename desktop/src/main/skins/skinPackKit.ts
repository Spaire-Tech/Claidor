...
const SKIN_CREATOR_SKILL_METADATA: KitSkillMetadata = {
  ...
  description: {
    en: 'Designs and applies coordinated Maties backgrounds, emblems, and interface colors.',
  },
};

const SKIN_PACK_STARTER_PROMPT = {
  en: 'Create a custom AI skin for Maties using {primary and accent colors} as the palette, centered on {theme or character}, set in {scene and mood}, and designed for {use case or purpose}. Generate a coordinated backdrop, emblem, and interface colors from this idea, then apply them to Maties.',
};
...
export function buildSkinPackMarketplaceKit(): Record<string, unknown> {
  return {
    ...
    description: {
      en: 'Describe the look you want. AI creates a custom backdrop and emblem, coordinates the interface colors, and applies it to Maties.',
    },
    ...
    tryAsking: [
      ...
      {
        en: 'Turn Maties into a bright post-rain morning study with an orange cat: creamy white, pale wood, soft daylight, and green plants by the window, fresh, quiet, comforting, and suited to reading, learning, and everyday work',
      },
      {
        en: 'Turn Maties into a deep-blue data and automation command center with abstract trend light trails, precise grids, and amber status lights, built for coding, analysis, market monitoring, and long-running tasks',
      },
      {
