...
describe('resolveComputerUseRuntimePaths', () => {
  function writeRuntimeFixture(): {
    ...
    const runtimePackageRoot = path.join(rootDir, 'node_modules', '@swen', 'computer-use');
    ...
    fs.writeFileSync(path.join(rootDir, 'runtime.json'), `\uFEFF${JSON.stringify({
      ...
      clientModule: 'node_modules/@swen/computer-use/dist/windows/computer_use_client.js',
      helper: 'node_modules/@swen/computer-use/bin/windows/lobster-computer-use.exe',
      runtimePackageRoot: 'node_modules/@swen/computer-use',
    })}`);
  ...
  test('configures the helper with Swen branding', () => {
    writeRuntimeFixture();
    ...
    expect(config.strings?.usingComputer).toBe('Swen is using your computer');
  ...
  test('reports Escape cancellation before renewing the helper turn', () => {
    ...
    expect(script).toContain("const APPROVED_APP_META_KEY = 'x-swen-computer-use-approved-app'");
