...
const RULES: readonly string[] = [
  ...
  'Pictures make a card, so go and get them. When a block has picture places in it — places, hotels, restaurants, products, the days of a trip, the dishes of a plan — search for the images before you write the block, in one pass for the whole set rather than one search per item. Take the address from the result and write it into `src` exactly as you saw it: the place\'s own site first, then a reputable publication.',
