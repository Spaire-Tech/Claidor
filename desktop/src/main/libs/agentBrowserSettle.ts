...
export type SettleSenses = {
  /** Whether the page is mid-load right now. */
  isLoading: () => boolean;
  /**
   * Resolve `true` once the DOM has been quiet for `quietMs`, `false` if
   * `maxMs` passes first. Reject if the page navigated away underneath.
   */
  domQuiet: (quietMs: number, maxMs: number) => Promise<boolean>;
  sleep: (ms: number) => Promise<void>;
  now: () => number;
};

export type SettleOptions = {
  /** How long to watch for a navigation to begin. */
  startWindowMs?: number;
  /** How long to allow a navigation that began to finish. */
  loadTimeoutMs?: number;
  /** How long the DOM must be still. */
  quietMs?: number;
  /** The most we wait for stillness. */
  quietTimeoutMs?: number;
  pollMs?: number;
};

export type SettleReport = {
  /** A page load began after the action. */
  navigated: boolean;
  /** That load finished within the allowance (true when none began). */
  loaded: boolean;
  /** The DOM went quiet within the allowance. */
  quiet: boolean;
  /** Milliseconds spent waiting, for the log. */
  waitedMs: number;
};

export const DEFAULT_SETTLE: Required<SettleOptions> = {
  startWindowMs: 400,
  loadTimeoutMs: 8_000,
  quietMs: 500,
...
export async function settlePage(senses: SettleSenses, options: SettleOptions = {}): Promise<SettleReport> {
  const opts = { ...DEFAULT_SETTLE, ...options };
  ...
  if (navigated) {
    ...
    while (senses.isLoading()) {
      if (senses.now() - loadStarted >= opts.loadTimeoutMs) { loaded = false; break; }
      await senses.sleep(opts.pollMs);
    }
  }
 ...
 */
export function domQuietScript(quietMs: number, maxMs: number): string {
