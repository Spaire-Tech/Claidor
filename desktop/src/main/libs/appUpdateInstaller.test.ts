...
const INSTALLER_PATH = 'C:\\Users\\test\\AppData\\Roaming\\Swen\\updates\\swen-update-manual-1.exe';
...
describe('Windows update install', () => {
  ...
  test('launches the update-mode installer through PowerShell and quits on success', async () => {
    ...
    expect(script).toContain('$env:SWEN_UPDATE_INSTALLER_PATH');
    ...
    expect(options.env?.SWEN_UPDATE_INSTALLER_PATH).toBe(INSTALLER_PATH);
  ...
  test('passes a metacharacter-containing installer path only through the child environment', async () => {
    ...
    expect(options.env?.SWEN_UPDATE_INSTALLER_PATH).toBe(installerPath);
  });
});
...
describe('Windows update download URL enforcement', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'swen-download-policy-'));
  ...
  test('rejects an insecure input before fetching or creating a partial file', async () => {
    await expect(downloadUpdate(
      'http://downloads.example.com/Swen.exe',
      'manual',
      () => {},
  ...
  test('downloads directly without relying on Electron response.url', async () => {
    ...
    const inputUrl =
      'https://downloads.example.com/Swen.exe?inputToken=do-not-log';
  ...
  test('leaves no cached file when the Windows fetch rejects a redirect', async () => {
    ...
    await expect(downloadUpdate(
      'https://downloads.example.com/Swen.exe',
      'auto',
      () => {},
...
describe('hdiutil plist parsing', () => {
  test('extracts the mount point and dev entries from an APFS attach result', () => {
    ...
    const json = JSON.stringify({
      'system-entities': [
        ...
        { 'dev-entry': '/dev/disk5s1', 'mount-point': '/Volumes/Swen', 'volume-kind': 'apfs' },
    ...
    expect(result.mountPoint).toBe('/Volumes/Swen');
  ...
  test('extracts the mount point from an HFS attach result', () => {
    const json = JSON.stringify({
      'system-entities': [
        { 'content-hint': 'GUID_partition_scheme', 'dev-entry': '/dev/disk4' },
        { 'content-hint': 'Apple_HFS', 'dev-entry': '/dev/disk4s1', 'mount-point': '/Volumes/Swen 1' },
      ],
    });

    const result = parseHdiutilAttachOutput(json);

    expect(result.mountPoint).toBe('/Volumes/Swen 1');
  });
...
describe('mac swap builders', () => {
  ...
  test('builds a staged-copy, guarded-backup, rollback and cleanup sequence', () => {
    const target = '/Applications/Swen.app';
    const swapPaths = buildMacSwapPaths(target, 7);

    const cmd = buildMacSwapInstallCommand('/Volumes/Swen/Swen.app', target, swapPaths);

    const cpIndex = cmd.indexOf(`cp -R '/Volumes/Swen/Swen.app' '${swapPaths.staging}'`);
...
describe('macOS DMG install', () => {
  ...
  const USER_DATA = '/Users/test/Library/Application Support/Swen';
  const DMG_PATH = `${USER_DATA}/updates/swen-update-auto-1.dmg`;
  const TARGET_APP = '/Applications/Swen.app';
  ...
  const respondMountedAtVolumes = () => attachMountedJson('/Volumes/Swen');
  ...
  test('mounts, swap-installs and relaunches on the happy path', async () => {
    ...
    expect(detachCommands[0]).toContain('/Volumes/Swen');
  ...
  test('cleans up leftover staging and backup directories before installing', async () => {
    attachResponders = [respondMountedAtVolumes];
    applicationsEntries = [
      `.Swen.app${MAC_SWAP_STAGING_INFIX}1`,
      `.Swen.app${MAC_SWAP_BACKUP_INFIX}2`,
      'Swen.app',
      'Other.app',
    ];
    ...
    expect(fs.promises.rm).toHaveBeenCalledWith(
      `/Applications/.Swen.app${MAC_SWAP_STAGING_INFIX}1`,
      { recursive: true, force: true },
    );
    expect(fs.promises.rm).toHaveBeenCalledWith(
      `/Applications/.Swen.app${MAC_SWAP_BACKUP_INFIX}2`,
      { recursive: true, force: true },
    );
