 ...
 * to local Cowork sessions so that conversations are visible in the Swen UI.
 */
...
const SWEN_SESSION_PREFIX = 'swen:';
...
export function buildManagedSessionKey(
  ...
  return `agent:${normalizedAgentId}:swen:${normalizedSessionId}`;
}
...
export function parseManagedSessionKey(
  ...
  if (raw.startsWith(SWEN_SESSION_PREFIX)) {
    const sessionId = raw.slice(SWEN_SESSION_PREFIX.length).trim();
  ...
  if (parts.length < 4 || parts[0] !== 'agent' || parts[2] !== 'swen') {
    return null;
  }
...
export class OpenClawChannelSessionSync {
  ...
  resolveOrCreateSession(sessionKey: string): string | null {
    // 1. Skip Swen-originated sessions
    if (isManagedSessionKey(sessionKey)) {
      console.log('[ChannelSessionSync] skipped: Swen-originated session');
      return null;
    }
