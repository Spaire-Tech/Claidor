...
function trySwenServerFallback(modelId?: string): MatchedProvider | null {
  ...
  const effectiveApiFormat = cachedMeta?.apiFormat
    ...
    : 'openai';
  console.debug('[ClaudeSettings] swen-server provider resolved:', {
    baseURL,
...
function resolveMatchedProvider(appConfig: AppConfig): { matched: MatchedProvider | null; error?: string } {
  ...
  if (!modelId) {
    ...
    if (!fallback) {
      const serverFallback = trySwenServerFallback(configuredModelId);
  ...
  // Handle swen-server provider: dynamically construct from auth tokens
  if (preferredProviderName === ProviderName.SwenServer) {
    const serverMatch = trySwenServerFallback(modelId);
    if (serverMatch) {
  ...
  if (!providerEntry) {
    ...
    if (fallback) {
      ...
      const serverFallback = trySwenServerFallback(modelId);
      if (serverFallback) return { matched: serverFallback };
...
export function resolveCurrentApiConfig(target: OpenAICompatProxyTarget = 'local'): ApiConfigResolution {
  ...
  const effectiveApiKey = resolvedApiKey
    || (!providerRequiresApiKey(matched.providerName) ? 'sk-swen-local' : '');
  ...
  return {
    config: {
      apiKey: resolvedApiKey || 'swen-openai-compat',
...
export function resolveAllProviderApiKeys(): Record<string, string> {
  ...
  // swen-server token is now managed by the token proxy
  ...
  for (const [providerName, providerConfig] of Object.entries(appConfig.providers)) {
    ...
    result[envName] = apiKey || 'sk-swen-local';
  }
...
export function resolveAllEnabledProviderConfigs(): ProviderRawConfig[] {
  ...
  for (const [providerName, providerConfig] of Object.entries(appConfig.providers)) {
    if (!providerConfig?.enabled) continue;
    if (providerName === ProviderName.SwenServer) continue;
