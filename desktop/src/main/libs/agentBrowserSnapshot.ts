...
export type SnapshotBuild = {
  ...
  /** `ax-<page>-<node>` → backend DOM node id, for click/fill by uid. */
  refs: Map<string, number>;
  /** How many nodes the agent was given. */
  shown: number;
  /** How many actionable nodes were cut by the cap. Zero means the whole page. */
  dropped: number;
};

/** The default cap on emitted nodes. Roughly 100k characters of JSON at worst. */
export const DEFAULT_SNAPSHOT_NODE_CAP = 4_000;
...
const toScalar = (value: unknown): string | number | boolean | undefined => {
...
export function buildAccessibilitySnapshot(
  rawNodes: readonly AxNode[],
  options: { pageId: number; title: string; cap?: number },
): SnapshotBuild {
  const cap = options.cap ?? DEFAULT_SNAPSHOT_NODE_CAP;
  ...
  const isWrapper = (node: AxNode): boolean => {
  ...
  type Pruned = { node: AxNode; children: Pruned[] };

  const prune = (node: AxNode, ancestors: Set<string>): Pruned[] => {
  ...
  const count = (list: Pruned[]): number => list.reduce((sum, one) => sum + 1 + count(one.children), 0);
  const total = count(roots);

  // Emit in document order until the cap, then stop.
  let shown = 0;
  const emit = (pruned: Pruned): SnapshotNode | null => {
    if (shown >= cap) return null;
    shown += 1;
    const { node } = pruned;
    const ref = `ax-${options.pageId}-${node.nodeId}`;
    if (typeof node.backendDOMNodeId === 'number') refs.set(ref, node.backendDOMNodeId);
    const role = toScalar(node.role?.value);
  ...
  const emitted = roots.map(emit).filter((node): node is SnapshotNode => Boolean(node));
  const snapshot: SnapshotNode = emitted.length === 1
    ? emitted[0]
...
export function describeSnapshot(build: SnapshotBuild, prefix = 'Accessibility snapshot captured.'): string {
  if (build.dropped === 0) return prefix;
