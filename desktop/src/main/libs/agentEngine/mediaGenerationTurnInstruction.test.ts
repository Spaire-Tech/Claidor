...
describe('buildMediaGenerationTurnInstruction', () => {
  test('keeps an ordinary image request to exactly one generation call', () => {
    ...
    expect(instruction).not.toContain('caisra_skin_manage');
  });
  ...
  test('guides a two-slot serial skin flow without a hard generation quota', () => {
    ...
    expect(instruction).toContain('soft budget of about two serial caisra_image_generate calls');
    ...
    expect(instruction).not.toContain('you must call the caisra_image_generate tool exactly once');
  });

  test('uses the native image route for a skin workflow without Caisra image selection', () => {
    const instruction = buildMediaGenerationTurnInstruction(
      undefined,
      false,
      SkinWorkflowKind.SkinPack,
    ...
    expect(instruction).toContain('locked to the engine-native image_generate tool');
    expect(instruction).toContain('soft budget of about two serial image_generate calls');
    expect(instruction).toContain('(count=1)');
    expect(instruction).toContain('There is no caisra_image_generate status step');
    expect(instruction).toContain('caisra_skin_manage');
    expect(instruction).not.toContain('Caisra media generation tools - NOT AVAILABLE');
  });

  test('uses the Caisra image route for auto media selection', () => {
    const instruction = buildMediaGenerationTurnInstruction(
    ...
    expect(instruction).toContain('locked to caisra_image_generate');
    expect(instruction).toContain('model "image-model" for both image generation jobs');
    expect(instruction).not.toContain('engine-native image_generate tool');
  });

  test('preserves the media-skill fallback when Caisra tools are unavailable', () => {
    const instruction = buildMediaGenerationTurnInstruction(undefined, true);

    expect(instruction).toContain('Caisra media generation tools - NOT AVAILABLE');
