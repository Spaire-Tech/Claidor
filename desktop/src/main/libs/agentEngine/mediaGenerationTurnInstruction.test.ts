...
describe('buildMediaGenerationTurnInstruction', () => {
  test('keeps an ordinary image request to exactly one generation call', () => {
    ...
    expect(instruction).not.toContain('swen_skin_manage');
  });
  ...
  test('guides a two-slot serial skin flow without a hard generation quota', () => {
    ...
    expect(instruction).toContain('soft budget of about two serial swen_image_generate calls');
    ...
    expect(instruction).not.toContain('you must call the swen_image_generate tool exactly once');
  });

  test('uses the native image route for a skin workflow without Swen image selection', () => {
    const instruction = buildMediaGenerationTurnInstruction(
      undefined,
      false,
      SkinWorkflowKind.SkinPack,
    ...
    expect(instruction).toContain('There is no swen_image_generate status step');
    expect(instruction).toContain('swen_skin_manage');
    expect(instruction).not.toContain('Swen media generation tools - NOT AVAILABLE');
  });

  test('uses the Swen image route for auto media selection', () => {
    const instruction = buildMediaGenerationTurnInstruction(
    ...
    expect(instruction).toContain('locked to swen_image_generate');
  ...
  test('preserves the media-skill fallback when Swen tools are unavailable', () => {
    const instruction = buildMediaGenerationTurnInstruction(undefined, true);

    expect(instruction).toContain('Swen media generation tools - NOT AVAILABLE');
