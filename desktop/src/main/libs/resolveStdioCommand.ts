 ...
 * the MATIES_NPM_BIN_DIR path without resolving API config or proxy settings.
 */
...
export async function resolveStdioCommand(server: McpServerRecord): Promise<ResolvedStdioCommand> {
  ...
  // (PATH, APPDATA, TEMP, etc.) — our node shims in PATH need MATIES_ELECTRON_PATH
  // and MATIES_NPM_BIN_DIR which won't be inherited. Pre-resolving to absolute
