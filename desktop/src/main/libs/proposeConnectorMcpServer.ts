...
export interface ProposeConnectorMcpLaunchOptions {
  electronNodeRuntimePath: string;
  /** The propose-connector route. */
  bridgeUrl: string;
  bridgeSecret: string;
  platform?: NodeJS.Platform;
}

export interface ProposeConnectorMcpStdioLaunch {
  command: string;
  args: string[];
...
const TOOL_DESCRIPTION = [
  ...
  '',
  'Give the service\'s id from the list: ' + connectorIds().join(', ') + '.',
].join('\n');
  ...
  inputSchema: {
    ...
    properties: {
      connectionId: {
        ...
        enum: ${JSON.stringify(connectorIds())},
      },
      reason: {
        type: 'string',
        description: 'One line, in your words, on why now. Shown under the service\'s own line. Up to ${PROPOSE_CONNECTOR_LIMITS.reason} characters. Optional.',
      },
    },
...
async function proposeConnector(args) {
  ...
  if (payload.behavior === '${ProposeConnectorBehavior.Failed}') {
  ...
  if (payload.behavior === '${ProposeConnectorBehavior.Declined}') {
  ...
  if (payload.behavior !== '${ProposeConnectorBehavior.Connected}') {
...
async function handleRequest(message) {
  ...
  if (message.method === 'initialize') {
    ...
    result = message.params?.name === '${PROPOSE_CONNECTOR_TOOL}'
 ...
 */
const prepare = (baseDir: string, options: ProposeConnectorMcpLaunchOptions): { serverDir: string } => {
...
export const resolveProposeConnectorMcpStdioLaunch = (
  baseDir: string,
  options: ProposeConnectorMcpLaunchOptions,
): ProposeConnectorMcpStdioLaunch => {
