...
export function findAuthorizationUrl(output: string): string | null {
  for (const raw of output.split(/\s+/)) {
    ...
    if (!url.searchParams.get('redirect_uri')) continue;
    if (!url.searchParams.get('client_id') && !url.searchParams.get('response_type')) continue;
    return candidate;
  }
  return null;
}
...
/**
 * The authorization code out of a redirect the browser made.
 *
 * Returns the provider's error instead when it sent one: a person who
 * pressed Cancel should be told that, not left watching a spinner.
 */
...
export function readCallback(requestUrl: string): CallbackResult {
  let url: URL;
  try {
  ...
  if (!code) return { error: 'The service did not send a code back.' };
 ...
 */
export function saysAuthorized(output: string): boolean {
