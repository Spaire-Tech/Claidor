...
test('parseManagedSessionKey handles raw local session keys', () => {
  expect(parseManagedSessionKey('maties:abc-123')).toEqual({
    agentId: null,
    sessionId: 'abc-123',
  });
});
...
test('parseManagedSessionKey handles canonical local session keys', () => {
  expect(parseManagedSessionKey('agent:main:maties:abc-123')).toEqual({
    agentId: 'main',
    sessionId: 'abc-123',
  });
});
...
test('buildManagedSessionKey emits canonical local session keys', () => {
  ...
  ).toBe(`agent:${DEFAULT_MANAGED_AGENT_ID}:maties:abc-123`);
  expect(
    buildManagedSessionKey('abc-123', 'secondary'),
  ).toBe('agent:secondary:maties:abc-123');
});

test('parseChannelSessionKey ignores managed local session keys', () => {
  expect(parseChannelSessionKey('maties:abc-123')).toBe(null);
  expect(parseChannelSessionKey('agent:main:maties:abc-123')).toBe(null);
});

test('channel sync does not treat managed local session keys as channel sessions', () => {
  const sync = createSync();

  expect(isManagedSessionKey('agent:main:maties:abc-123')).toBe(true);
  expect(sync.isChannelSessionKey('agent:main:maties:abc-123')).toBe(false);
  expect(sync.resolveOrCreateSession('agent:main:maties:abc-123')).toBe(null);
  expect(sync.resolveOrCreateMainAgentSession('agent:main:maties:abc-123')).toBe(null);
});
