...
const makeTempDir = (): string => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), `${APP_ID}-data-migration-test-`));
  tempRoots.push(dir);
  return dir;
};
...
const writeManifestFixture = (userDataPath: string): void => {
  writeFile(
    path.join(userDataPath, `.${APP_ID}-migration.json`),
    JSON.stringify({
      format: `${APP_ID}-data-migration`,
      version: 1,
      archiveRoot: APP_NAME,
      sqlite: { exists: true },
...
test('createMigrationArchive excludes cache and log data and writes a manifest', () => {
  ...
  const userData = path.join(root, APP_NAME);
  ...
  writeFile(path.join(userData, 'backups', 'sqlite', 'snapshots', `${APP_ID}-latest.sqlite`), 'old-snapshot');
  writeFile(path.join(userData, 'sqlite-backups', `${APP_ID}-latest.sqlite`), 'legacy-snapshot');
  ...
  expect(entries).toContain(`${APP_NAME}/.${APP_ID}-migration.json`);
  expect(entries).toContain(`${APP_NAME}/${DB_FILENAME}`);
  expect(entries).toContain(`${APP_NAME}/openclaw/state/openclaw.json`);
  expect(entries).toContain(`${APP_NAME}/SKILLs/demo/SKILL.md`);
  ...
  const manifest = JSON.parse(
    fs.readFileSync(path.join(extractRoot, APP_NAME, `.${APP_ID}-migration.json`), 'utf8'),
  ) as {
    format?: string;
    archiveRoot?: string;
  ...
  expect(manifest.format).toBe(`${APP_ID}-data-migration`);
  expect(manifest.archiveRoot).toBe(APP_NAME);
...
test('createMigrationArchive replaces the live sqlite database with the snapshot', () => {
  ...
  const extractRoot = extractArchive(archivePath);
  expect(readSqliteString(path.join(extractRoot, APP_NAME, DB_FILENAME), 'SELECT value FROM kv WHERE key = ?', ['auth_tokens']))
    .toContain('snapshot-refresh');
  expect(fs.existsSync(path.join(extractRoot, APP_NAME, `${DB_FILENAME}-wal`))).toBe(false);
});
...
test('inspectMigrationArchive rejects legacy Windows PowerShell archive root', () => {
  const root = makeTempDir();
  const legacyRoot = path.join(root, 'AppData', 'Roaming', APP_NAME);
  ...
  expect(() => inspectMigrationArchiveSync(archivePath)).toThrow(new RegExp(`does not contain ${APP_NAME} user data`));
});

test('inspectMigrationArchive rejects archives without a migration manifest', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, APP_NAME);
  ...
  expect(() => inspectMigrationArchiveSync(archivePath)).toThrow(new RegExp(`missing \\.${APP_ID}-migration\\.json`));
});
...
test('inspectMigrationArchive rejects archives whose manifest does not match sqlite content', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'source', APP_NAME);
  ...
  const extractRoot = extractArchive(archivePath);
  const manifestPath = path.join(extractRoot, APP_NAME, `.${APP_ID}-migration.json`);
...
test('performPendingDataMigrationRestoreSync creates rollback and restores backup data', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'source', APP_NAME);
  const targetUserData = path.join(root, 'target', APP_NAME);
...
test('performPendingDataMigrationRestoreSync replaces data in place and preserves runtime locks', () => {
  ...
  writeFile(path.join(sourceUserData, 'backups', 'sqlite', 'snapshots', `${APP_ID}-latest.sqlite`), 'source-backup');
  writeFile(path.join(sourceUserData, 'sqlite-backups', `${APP_ID}-latest.sqlite`), 'source-legacy-backup');
  ...
  writeFile(path.join(targetUserData, 'old-only.txt'), 'old');
  writeFile(path.join(targetUserData, 'backups', 'sqlite', 'snapshots', `${APP_ID}-latest.sqlite`), 'target-backup');
  writeFile(path.join(targetUserData, 'sqlite-backups', `${APP_ID}-latest.sqlite`), 'target-legacy-backup');
  ...
  createMigrationArchiveSync({ userDataPath: sourceUserData, outputPath: archivePath });
  const extractRoot = extractArchive(archivePath);
  writeFile(path.join(extractRoot, APP_NAME, 'backups', 'sqlite', 'snapshots', `${APP_ID}-latest.sqlite`), 'legacy-source-backup');
  writeFile(path.join(extractRoot, APP_NAME, 'sqlite-backups', `${APP_ID}-latest.sqlite`), 'legacy-source-legacy-backup');
  writeFile(path.join(extractRoot, APP_NAME, 'cowork', 'bin', 'node.cmd'), 'legacy-source-shim');
  writeFile(path.join(extractRoot, APP_NAME, 'install-timing.log'), 'legacy-source-install-log');
  writeFile(path.join(extractRoot, APP_NAME, 'skill-migrate.log'), 'legacy-source-skill-migrate-log');
  writeFile(path.join(extractRoot, APP_NAME, 'Dictionaries', 'source.bdic'), 'legacy-source-dictionary');
  writeFile(path.join(extractRoot, APP_NAME, 'Local Storage', 'leveldb', 'source.log'), 'legacy-source-local-storage');
  writeFile(path.join(extractRoot, APP_NAME, 'Preferences'), 'legacy-source-preferences');
  writeFile(path.join(extractRoot, APP_NAME, 'Session Storage', 'leveldb', 'source.log'), 'legacy-source-session-storage');
  writeFile(path.join(extractRoot, APP_NAME, 'openclaw', 'logs', 'gateway-2026-06-10.log'), 'legacy-source-gateway-log');
  writeFile(path.join(extractRoot, APP_NAME, 'openclaw', 'mcp-packages', 'demo', 'node_modules', 'native.node'), 'legacy-source-native');
  writeFile(path.join(extractRoot, APP_NAME, 'openclaw', 'state', 'logs', 'commands.log'), 'legacy-source-commands-log');
  writeFile(path.join(extractRoot, APP_NAME, 'runtimes', 'python', 'python.exe'), 'legacy-source-runtime');
  tar.create({
    sync: true,
    gzip: true,
    file: archivePath,
  ...
  expect(fs.existsSync(path.join(targetUserData, `${DB_FILENAME}-shm`))).toBe(false);
  expect(fs.existsSync(path.join(targetUserData, 'old-only.txt'))).toBe(false);
  expect(fs.readFileSync(path.join(targetUserData, 'backups', 'sqlite', 'snapshots', `${APP_ID}-latest.sqlite`), 'utf8'))
    .toBe('target-backup');
  expect(fs.readFileSync(path.join(targetUserData, 'sqlite-backups', `${APP_ID}-latest.sqlite`), 'utf8'))
