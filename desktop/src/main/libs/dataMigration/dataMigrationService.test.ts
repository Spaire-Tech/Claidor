...
const makeTempDir = (): string => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'swen-data-migration-test-'));
  tempRoots.push(dir);
  return dir;
};
...
const writeManifestFixture = (userDataPath: string): void => {
  writeFile(
    path.join(userDataPath, '.swen-migration.json'),
    JSON.stringify({
      format: 'swen-data-migration',
      version: 1,
      archiveRoot: 'Swen',
      sqlite: { exists: true },
...
test('createMigrationArchive excludes cache and log data and writes a manifest', () => {
  ...
  const userData = path.join(root, 'Swen');
  ...
  writeFile(path.join(userData, 'backups', 'sqlite', 'snapshots', 'swen-latest.sqlite'), 'old-snapshot');
  writeFile(path.join(userData, 'sqlite-backups', 'swen-latest.sqlite'), 'legacy-snapshot');
  ...
  expect(entries).toContain('Swen/.swen-migration.json');
  expect(entries).toContain(`Swen/${DB_FILENAME}`);
  expect(entries).toContain('Swen/openclaw/state/openclaw.json');
  expect(entries).toContain('Swen/SKILLs/demo/SKILL.md');
  ...
  const manifest = JSON.parse(
    fs.readFileSync(path.join(extractRoot, 'Swen', '.swen-migration.json'), 'utf8'),
  ) as {
    format?: string;
    archiveRoot?: string;
  ...
  expect(manifest.format).toBe('swen-data-migration');
  expect(manifest.archiveRoot).toBe('Swen');
...
test('createMigrationArchive replaces the live sqlite database with the snapshot', () => {
  ...
  const extractRoot = extractArchive(archivePath);
  expect(readSqliteString(path.join(extractRoot, 'Swen', DB_FILENAME), 'SELECT value FROM kv WHERE key = ?', ['auth_tokens']))
    .toContain('snapshot-refresh');
  expect(fs.existsSync(path.join(extractRoot, 'Swen', `${DB_FILENAME}-wal`))).toBe(false);
});
...
test('inspectMigrationArchive rejects legacy Windows PowerShell archive root', () => {
  const root = makeTempDir();
  const legacyRoot = path.join(root, 'AppData', 'Roaming', 'Swen');
  ...
  expect(() => inspectMigrationArchiveSync(archivePath)).toThrow(/does not contain Swen user data/);
});

test('inspectMigrationArchive rejects archives without a migration manifest', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'Swen');
  ...
  expect(() => inspectMigrationArchiveSync(archivePath)).toThrow(/missing \.swen-migration\.json/);
});
...
test('inspectMigrationArchive rejects archives whose manifest does not match sqlite content', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'source', 'Swen');
  ...
  const extractRoot = extractArchive(archivePath);
  const manifestPath = path.join(extractRoot, 'Swen', '.swen-migration.json');
...
test('performPendingDataMigrationRestoreSync creates rollback and restores backup data', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'source', 'Swen');
  const targetUserData = path.join(root, 'target', 'Swen');
...
test('performPendingDataMigrationRestoreSync replaces data in place and preserves runtime locks', () => {
  ...
  writeFile(path.join(sourceUserData, 'backups', 'sqlite', 'snapshots', 'swen-latest.sqlite'), 'source-backup');
  writeFile(path.join(sourceUserData, 'sqlite-backups', 'swen-latest.sqlite'), 'source-legacy-backup');
  ...
  writeFile(path.join(targetUserData, 'old-only.txt'), 'old');
  writeFile(path.join(targetUserData, 'backups', 'sqlite', 'snapshots', 'swen-latest.sqlite'), 'target-backup');
  writeFile(path.join(targetUserData, 'sqlite-backups', 'swen-latest.sqlite'), 'target-legacy-backup');
  ...
  createMigrationArchiveSync({ userDataPath: sourceUserData, outputPath: archivePath });
  const extractRoot = extractArchive(archivePath);
  writeFile(path.join(extractRoot, 'Swen', 'backups', 'sqlite', 'snapshots', 'swen-latest.sqlite'), 'legacy-source-backup');
  writeFile(path.join(extractRoot, 'Swen', 'sqlite-backups', 'swen-latest.sqlite'), 'legacy-source-legacy-backup');
  writeFile(path.join(extractRoot, 'Swen', 'cowork', 'bin', 'node.cmd'), 'legacy-source-shim');
  writeFile(path.join(extractRoot, 'Swen', 'install-timing.log'), 'legacy-source-install-log');
  writeFile(path.join(extractRoot, 'Swen', 'skill-migrate.log'), 'legacy-source-skill-migrate-log');
  writeFile(path.join(extractRoot, 'Swen', 'Dictionaries', 'source.bdic'), 'legacy-source-dictionary');
  writeFile(path.join(extractRoot, 'Swen', 'Local Storage', 'leveldb', 'source.log'), 'legacy-source-local-storage');
  writeFile(path.join(extractRoot, 'Swen', 'Preferences'), 'legacy-source-preferences');
  writeFile(path.join(extractRoot, 'Swen', 'Session Storage', 'leveldb', 'source.log'), 'legacy-source-session-storage');
  writeFile(path.join(extractRoot, 'Swen', 'openclaw', 'logs', 'gateway-2026-06-10.log'), 'legacy-source-gateway-log');
  writeFile(path.join(extractRoot, 'Swen', 'openclaw', 'mcp-packages', 'demo', 'node_modules', 'native.node'), 'legacy-source-native');
  writeFile(path.join(extractRoot, 'Swen', 'openclaw', 'state', 'logs', 'commands.log'), 'legacy-source-commands-log');
  writeFile(path.join(extractRoot, 'Swen', 'runtimes', 'python', 'python.exe'), 'legacy-source-runtime');
  tar.create({
    sync: true,
    gzip: true,
    file: archivePath,
  ...
  expect(fs.existsSync(path.join(targetUserData, `${DB_FILENAME}-shm`))).toBe(false);
  expect(fs.existsSync(path.join(targetUserData, 'old-only.txt'))).toBe(false);
  expect(fs.readFileSync(path.join(targetUserData, 'backups', 'sqlite', 'snapshots', 'swen-latest.sqlite'), 'utf8'))
    .toBe('target-backup');
  expect(fs.readFileSync(path.join(targetUserData, 'sqlite-backups', 'swen-latest.sqlite'), 'utf8'))
