export function buildScheduledTaskEnginePrompt(): string {
  return [
    ...
    '- If the user\'s prompt asks to "send" or "notify", and you are in a cron session, produce the content as session output rather than calling `message`. Append a note: "(This scheduled task has no IM notification channel configured, so the result was saved to the run history. To have it delivered automatically, configure a notification channel in the scheduled task settings.)"',
  ].join('\n');
}
