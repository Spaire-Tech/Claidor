/**
 * Where the failing model comes from in Maties terms, so users can tell at
 * a glance whether an error concerns the Maties plan, a vendor coding plan
 * they configured, or their own custom provider.
 */
export const CoworkErrorModelSource = {
  MatiesPlan: 'maties-plan',
...
export interface CoworkErrorDetail {
  /** Runtime error message before Maties i18n normalization. */
  rawErrorMessage?: string;
  provider?: string;
  model?: string;
  /** Maties-side classification of where the failing model comes from. */
