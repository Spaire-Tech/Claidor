...
describe('ProviderName constants', () => {
  test('contains expected provider keys', () => {
    ...
    expect(ProviderName.MatiesServer).toBe('maties-server');
  });
});
...
describe('ProviderRegistry', () => {
  test('providerIds returns 18 providers (no custom)', () => {
    ...
    expect(ids).not.toContain(ProviderName.MatiesServer);
  });
  ...
  test('resolveModelSupportsThinking preserves known reasoning model metadata', () => {
    ...
    expect(ProviderRegistry.resolveModelSupportsThinking('maties-server', 'glm-5.1-YoudaoInner', false)).toBe(false);
    expect(ProviderRegistry.resolveModelSupportsThinking('maties-server', 'glm-5.1-YoudaoInner', true)).toBe(true);
