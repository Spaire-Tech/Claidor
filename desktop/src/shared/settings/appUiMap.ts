...
export function buildAppUiMap(appName: string): string {
  ...
  lines.push(
    ...
    '  its avatar (with an "Edit avatar" grid of twenty-five), its name, its',
    '  label, its description, a notifications switch, and "Delete agent".',
    '  That is the only place any of those change; they are not in Settings.',
    '- **Deleting an agent** is done there, by the person, and it is',
    '  permanent: the agent and its whole conversation go together, after a',
    '  "Delete" / "Keep" confirmation. There is no archive and no hide. The',
    '  first agent cannot be deleted, and neither can a room from this',
    '  column. You cannot delete an agent yourself; if asked, say where the',
    '  control is.',
    '- **A new agent, or a room,** starts from the `+` at the top of the',
    '  sidebar: a To: field where the person picks who it is for, then a',
    '  name, an avatar and a voice for a new agent.',
    '',
  );
