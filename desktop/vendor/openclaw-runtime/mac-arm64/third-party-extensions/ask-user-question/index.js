...
// vendor/openclaw-runtime/mac-arm64/third-party-extensions/ask-user-question/sessionKey.ts
...
function isAskUserQuestionCandidateSessionKey(sessionKey) {
...
// vendor/openclaw-runtime/mac-arm64/third-party-extensions/ask-user-question/index.ts
...
var ReactToMessageSchema = Type.Object({
  emoji: Type.String({ description: "One emoji, such as \u{1F44D} or \u2764\uFE0F or \u{1F64F}." })
});
async function react(config, input) {
  ...
  try {
    ...
    const reason = isRecord(parsed) && typeof parsed.reason === "string" ? parsed.reason : `The app answered ${response.status}.`;
    return { behavior: "nothing", reason };
  } catch (error) {
    const reason = error instanceof Error && error.name === "AbortError" ? "The app did not answer in time." : error instanceof Error ? error.message : String(error);
    return { behavior: "nothing", reason };
  } finally {
    clearTimeout(timer);
  }
...
var AskUserQuestionSchema = Type.Object({
...
async function askUser(config, input) {
  const controller = new AbortController();
  ...
  try {
    ...
    return {
      ...
      answers: isRecord(parsed?.answers) ? parsed.answers : void 0
    };
  } catch (error) {
      ...
      return {
        ...
        async execute(_id, params) {
          ...
          try {
            ...
            const answerLines = response.answers ? Object.entries(response.answers).map(([q, a]) => `${q}: ${a}`).join("\n") : "User approved.";
            return {
...
var ask_user_question_default = plugin;
export {
  ask_user_question_default as default
};
