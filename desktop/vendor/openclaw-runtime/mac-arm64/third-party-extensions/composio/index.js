...
// vendor/openclaw-runtime/mac-arm64/third-party-extensions/composio/client.ts
var COMPOSIO_BASE_URL = "https://backend.composio.dev";
var isRecord = (value) => !!value && typeof value === "object" && !Array.isArray(value);
var describeFailure = (status, payload) => {
  if (isRecord(payload)) {
...
var ComposioClient = class {
  ...
  async searchTools(useCase, limit) {
    const id = await this.session();
    const found = await this.request("POST", `/api/v3.1/tool_router/session/${encodeURIComponent(id)}/search`, {
  ...
  async toolkits() {
    const id = await this.session();
    const listed = await this.request("GET", `/api/v3.1/tool_router/session/${encodeURIComponent(id)}/toolkits`);
    return (listed.items ?? []).filter((item) => typeof item.slug === "string" && this.isToolkitAllowed(item.slug)).map((item) => {
...
// vendor/openclaw-runtime/mac-arm64/third-party-extensions/composio/index.ts
var isRecord2 = (value) => !!value && typeof value === "object" && !Array.isArray(value);
var stringList = (value) => Array.isArray(value) ? value.filter((one) => typeof one === "string" && one.trim() !== "").map((one) => one.trim()) : void 0;
...
var plugin = {
  ...
  register(api) {
    ...
    api.registerTool({
      ...
      description: "Find tools across the person's apps (Gmail, Slack, Notion, GitHub, and more) by describing what you want to do. Returns tool slugs with their input schemas.",
    ...
    api.registerTool({
      ...
      description: "Run one tool from composio_search_tools by its exact slug. The app must be connected (composio_manage_connections says which are). Ask the person before anything that sends, writes or deletes.",
      parameters: Type.Object({
      ...
      async execute(_id, params) {
        ...
        try {
          ...
          return result.ok ? asText({ tool_slug: slug, success: true, data: result.data }) : asFailure(result.error, { tool_slug: slug, success: false });
        } catch (error) {
    ...
    api.registerTool({
      ...
      label: "App sign-ins",
      description: "Which apps the person has signed into through Composio, and a sign-in link for one that is not. Hand the link to the person as a URL; they open it, not you.",
      parameters: Type.Object({
...
var composio_default = plugin;
export {
  composio_default as default
};
