...
export async function processOneMessage(
  ...
  const { senderAllowedForCommands, commandAuthorized } =
    await resolveSenderCommandAuthorizationWithRuntime({
      ...
      rawBody,
      isGroup: false,
      dmPolicy: (() => { /* chanCfg_dmPolicy_patch */ const _cc = (deps.config.channels)?.['openclaw-weixin'] ?? {}; return _cc.dmPolicy || 'pairing'; })(),
      ...
      senderId,
      isSenderAllowed: (id: string, list: string[]) => list.length === 0 || list.includes('*') || list.includes(id),
