...
// vendor/openclaw-runtime/mac-arm64/third-party-extensions/lobster-media-generation/sessionKey.ts
var LEGACY_LOBSTERAI_SESSION_PREFIX = "lobsterai:";
var AGENT_SESSION_PREFIX = "agent:";
var LOBSTERAI_SESSION_MARKER = "lobsterai";
var SUBAGENT_SESSION_MARKER = "subagent";
function isLobsterAiDesktopSessionKey(sessionKey) {
  ...
  return agentId.length > 0 && sessionId.length > 0 && (source === LOBSTERAI_SESSION_MARKER || source === SUBAGENT_SESSION_MARKER);
}

// vendor/openclaw-runtime/mac-arm64/third-party-extensions/lobster-media-generation/statusPolling.ts
var TERMINAL_STATUSES = /* @__PURE__ */ new Set(["succeeded", "failed", "cancelled"]);
var readMediaStatusPollCount = (value) => typeof value === "number" && Number.isFinite(value) && value > 0 ? Math.floor(value) : void 0;
var extractMediaStatusResponseText = (response) => response.content.map((item) => item.text).filter((text) => typeof text === "string" && text.trim()).join("\n").trim();
var isTerminalMediaStatus = (status) => TERMINAL_STATUSES.has(status);
var getMediaStatusPollInterval = (policy, completedPollCount) => {
  ...
  if (completedPollCount < policy.fastPollCount + policy.slowPollCount + policy.mediumPollCount) {
    return policy.mediumIntervalMs;
  }
  return policy.idleIntervalMs;
};
var waitForDelay = (ms, signal) => {
...
var buildPollingStoppedResponse = (mediaType, taskId, upstreamTaskId, pollCount) => ({
  content: [{
    type: "text",
...
var buildPollingTimeoutResponse = (mediaType, taskId, upstreamTaskId, pollCount, elapsedMs) => ({
  content: [{
    type: "text",
    text: `${mediaType === "image" ? "Image" : "Video"} generation timed out after ${Math.max(1, Math.round(elapsedMs / 6e4))} minutes.
Task ID: ${upstreamTaskId || taskId}
...
async function pollMediaStatus(options) {
  const now = options.now ?? Date.now;
  const wait = options.wait ?? waitForDelay;
  const startedAt = now();
  let pollCount = 0;
  let upstreamTaskId;
  let firstStatusOutput;
  ...
  while (true) {
    ...
    try {
      ...
      const currentStatus = typeof statusDetails.status === "string" ? statusDetails.status : void 0;
...
// vendor/openclaw-runtime/mac-arm64/third-party-extensions/lobster-media-generation/index.ts
var DEFAULT_TIMEOUT_MS = 15e4;
var STATUS_REQUEST_TIMEOUT_MS = 15e4;
var IMAGE_STATUS_POLL_POLICY = {
  timeoutMs: 30 * 6e4,
  fastIntervalMs: 5e3,
  slowIntervalMs: 15e3,
  mediumIntervalMs: 3e4,
  idleIntervalMs: 6e4,
  fastPollCount: 12,
  slowPollCount: 12,
  mediumPollCount: 20
};
var VIDEO_STATUS_POLL_POLICY = {
  timeoutMs: 36e6,
  fastIntervalMs: 1e4,
  slowIntervalMs: 3e4,
  mediumIntervalMs: 12e4,
  idleIntervalMs: 6e5,
  fastPollCount: 6,
  slowPollCount: 18,
  mediumPollCount: 10
...
var MediaToolName = {
  ImageGenerate: "caisra_image_generate",
  VideoGenerate: "caisra_video_generate",
  SkinManage: "caisra_skin_manage"
};
var MediaToolAction = {
  Generate: "generate",
  List: "list",
...
var sanitizeArgsForLog = (args) => {
  ...
  return {
    ...
    imageCount: Array.isArray(args.images) ? args.images.length : void 0,
    hasVideo: typeof args.video === "string",
    videoCount: Array.isArray(args.videos) ? args.videos.length : void 0,
...
var sanitizeSkinArgsForLog = (args) => ({
...
async function callMediaBridge(config, request) {
  ...
  try {
    ...
    return {
      ...
      details: isRecord(parsed) ? parsed : void 0
    };
  } catch (error) {
    if (error instanceof Error && error.name === "AbortError") {
...
var ImageGenerateSchema = Type.Object({
...
var VideoGenerateSchema = Type.Object({
  action: Type.Optional(Type.Union([
    Type.Literal(MediaToolAction.Generate),
    Type.Literal(MediaToolAction.List),
    Type.Literal(MediaToolAction.Status),
...
var SkinPresentationPaletteSchema = Type.Object({
...
var SkinPresentationSchema = Type.Object({
...
var SkinManageSchema = Type.Union([
  Type.Object({
...
var plugin = {
  ...
  name: "Caisra Media Generation",
  description: "Image/video generation and AI skin management tools powered by Caisra.",
  configSchema: {
    parse(value) {
  ...
  register(api) {
    ...
    api.registerTool((ctx) => {
      ...
      return {
        ...
        description: [
          "Generate images using the Caisra server.",
          "Supports text-to-image and image-to-image generation.",
          "If the system prompt includes a Caisra media reference mapping, use mapped file paths or URLs in image/images arguments and never pass @ media tokens as tool argument values.",
    ...
    api.registerTool((ctx) => {
      ...
      return {
        ...
        description: [
          "Generate videos using the Caisra server.",
          ...
          "If the system prompt includes a Caisra media reference mapping, use mapped file paths or URLs in image/images/firstFrame/referenceImages/video/videos/media arguments and never pass @ media tokens as tool argument values.",
    ...
    api.registerTool((ctx) => {
      ...
      return {
        ...
        description: [
          "Create and manage a Caisra AI skin pack through the trusted desktop callback.",
          ...
          "Caisra deterministically infers a preferred light or dark appearance from presentation colors and applies it through the existing theme system; do not choose a color theme ID.",
    ...
    api.logger.info("[lobster-media-generation] registered caisra_image_generate, caisra_video_generate, and caisra_skin_manage tools.");
  }
};
var lobster_media_generation_default = plugin;
export {
  lobster_media_generation_default as default
};
