...
const BrowserAnnotationProtocolVersion = 1;
const BrowserAnnotationAnchorKind = {
  Element: "element",
  Region: "region",
  Text: "text"
};
const BrowserAnnotationScreenshotStatus = {
  Capturing: "capturing"
};
const BrowserAnnotationGuestCommandType = {
  Start: "start",
  Sync: "sync",
  Focus: "focus",
...
const BrowserAnnotationGuestEventType = {
  Ready: "ready",
  Changed: "changed",
  CaptureReady: "capture-ready",
  CloseRequested: "close-requested"
};
const BrowserAnnotationGuestChannel = {
  Command: "lobster:browser-annotation:command",
  Event: "lobster:browser-annotation:event"
};
const BrowserAnnotationLimit = {
  MaxAnnotations: 20,
  MaxCommentLength: 2e3,
  MaxExcerptLength: 500,
  MaxSelectorLength: 1024,
  MaxUrlLength: 4096,
  MaxTitleLength: 512
};
const BrowserAnnotationElementStyleProperty = {
  Color: "color",
...
const BrowserAnnotationElementChangeProperty = {
  Text: "text",
...
function hasBrowserAnnotationContent(comment, elementEdit) {
  return Boolean(comment == null ? void 0 : comment.trim()) || getBrowserAnnotationElementChanges(elementEdit).length > 0;
}
function resolveBrowserAnnotationViewportRect(anchor, capture, currentScroll) {
  ...
  return {
    ...anchor.rect,
    x: anchor.rect.x - (currentScroll.x - ((capture == null ? void 0 : capture.scrollX) ?? 0)),
    y: anchor.rect.y - (currentScroll.y - ((capture == null ? void 0 : capture.scrollY) ?? 0))
  };
}
function resolveBrowserAnnotationMarkerViewportPoint(anchor, capture, currentScroll) {
  ...
  const offsetX = (capture == null ? void 0 : capture.markerViewportPoint) ? capture.markerViewportPoint.x - capture.targetRect.x : Math.min(16, rect.width / 2);
  const offsetY = (capture == null ? void 0 : capture.markerViewportPoint) ? capture.markerViewportPoint.y - capture.targetRect.y : Math.min(16, rect.height / 2);
...
const cleanText = (value, max = BrowserAnnotationLimit.MaxExcerptLength) => (value || "").replace(/\s+/g, " ").trim().slice(0, max);
...
const cloneElementEdit = (edit) => ({
  ...
  originalInlineStyle: {
    ...
    styles: edit.originalInlineStyle.styles ? Object.fromEntries(Object.entries(edit.originalInlineStyle.styles).map(([key, value]) => [
      key,
      value ? { ...value } : value
    ])) : void 0
  }
...
function selectorFor(element) {
  ...
  while (current && parts.length < 6 && current !== document.documentElement) {
    ...
    if (parent) {
      const siblings = Array.from(parent.children).filter((item) => item.tagName === (current == null ? void 0 : current.tagName));
...
function baseAnchor(rect, element) {
  return {
    ...
    selector: element ? selectorFor(element) : void 0,
    elementPath: element ? selectorFor(element) : void 0,
    role: (element == null ? void 0 : element.getAttribute("role")) || void 0,
    name: cleanText((element == null ? void 0 : element.getAttribute("aria-label")) || (element == null ? void 0 : element.getAttribute("alt"))) || void 0,
    immediateText: element ? cleanText(element.textContent) : void 0,
    nearbyText: (element == null ? void 0 : element.parentElement) ? cleanText(element.parentElement.textContent) : void 0,
    isFixed: element ? getComputedStyle(element).position === "fixed" : void 0
  };
}
...
function readElementEdit(element) {
  ...
  for (const definition of ELEMENT_STYLE_DEFINITIONS) {
    originalValues[definition.key] = definition.key === BrowserAnnotationElementStyleProperty.Opacity ? Number.isFinite(opacity) ? opacity : 1 : computed.getPropertyValue(definition.cssProperty);
...
function applyElementEdit(element, edit) {
  var _a;
  if (edit.canEditText) {
    element.textContent = edit.current.text === edit.original.text ? edit.original.text || "" : edit.current.text || "";
  }
  for (const definition of ELEMENT_STYLE_DEFINITIONS) {
    const inlineStyle = (_a = edit.originalInlineStyle.styles) == null ? void 0 : _a[definition.key];
    const legacyInlineStyle = definition.key === BrowserAnnotationElementStyleProperty.Color ? { value: edit.originalInlineStyle.color, priority: edit.originalInlineStyle.colorPriority } : definition.key === BrowserAnnotationElementStyleProperty.BackgroundColor ? {
      value: edit.originalInlineStyle.backgroundColor,
      priority: edit.originalInlineStyle.backgroundColorPriority
    } : definition.key === BrowserAnnotationElementStyleProperty.Opacity ? { value: edit.originalInlineStyle.opacity, priority: edit.originalInlineStyle.opacityPriority } : definition.key === BrowserAnnotationElementStyleProperty.FontFamily ? {
    ...
    setOrRestoreStyle(
      ...
      (inlineStyle == null ? void 0 : inlineStyle.value) ?? (legacyInlineStyle == null ? void 0 : legacyInlineStyle.value),
      (inlineStyle == null ? void 0 : inlineStyle.priority) ?? (legacyInlineStyle == null ? void 0 : legacyInlineStyle.priority)
    );
  }
}
...
function send(type, payload = {}) {
  ...
  electron.ipcRenderer.sendToHost(BrowserAnnotationGuestChannel.Event, {
    ...activeEnvelope,
    type,
    revision,
    ...payload
...
function start(envelope) {
  if (activeCommandListener) {
    electron.ipcRenderer.removeListener(BrowserAnnotationGuestChannel.Command, activeCommandListener);
  ...
  const borderRadiusInput = createNumberInput(0, 9999, 0.5);
  const borderWidthInput = createNumberInput(0, 128, 0.5);
  const widthInput = createNumberInput(0, 1e5, 0.5);
  const heightInput = createNumberInput(0, 1e5, 0.5);
  const paddingInputs = [
    createNumberInput(0, 1e4, 0.5),
    createNumberInput(0, 1e4, 0.5),
    createNumberInput(0, 1e4, 0.5),
    createNumberInput(0, 1e4, 0.5)
  ];
  const marginInputs = [
    createNumberInput(-1e4, 1e4, 0.5),
    createNumberInput(-1e4, 1e4, 0.5),
    createNumberInput(-1e4, 1e4, 0.5),
    createNumberInput(-1e4, 1e4, 0.5)
  ];
  ...
  const resolveAnchorRect = (anchor, capture) => resolveBrowserAnnotationViewportRect(anchor, capture, {
    x: window.scrollX,
    y: window.scrollY
  });
  const isMarkerVisible = (left, top) => left + 26 > 0 && top + 26 > 0 && left < window.innerWidth && top < window.innerHeight;
  const positionEditor = (rect) => {
    ...
    const top = preferredTop + height <= window.innerHeight - 8 ? preferredTop : Math.max(8, rect.y - height - 8);
  ...
  const refreshEditingGeometry = () => {
    ...
    if (pendingElement == null ? void 0 : pendingElement.isConnected) pendingAnchor = elementAnchor(pendingElement);
    const annotation = editingId ? annotations.find((item) => item.id === editingId) : void 0;
    const rect = resolveAnchorRect(pendingAnchor, annotation == null ? void 0 : annotation.capture);
    showHighlight(rect);
  ...
  const updateSaveDisabled = () => {
    const disabled = !hasBrowserAnnotationContent(textarea.value, draftElementEdit || void 0);
    topConfirm.disabled = disabled;
    saveButton.disabled = disabled;
  };
  const colorToHex = (value) => {
    const match = value == null ? void 0 : value.match(/rgba?\(\s*(\d+)[,\s]+(\d+)[,\s]+(\d+)/i);
  ...
  const fillSelect = (select, currentValue, options) => {
    ...
    select.value = currentValue || ((_a = values[0]) == null ? void 0 : _a.value) || "";
  };
  const fillPropertyControls = (anchor) => {
    ...
    [current.paddingTop, current.paddingRight, current.paddingBottom, current.paddingLeft].forEach((value, index) => fillNumberInput(paddingInputs[index], value));
    [current.marginTop, current.marginRight, current.marginBottom, current.marginLeft].forEach((value, index) => fillNumberInput(marginInputs[index], value));
  ...
  const openEditor = (anchor, id, targetElement, suppressImmediateOutsideClick = false) => {
    var _a, _b, _c, _d;
    const annotation = id ? annotations.find((item) => item.id === id) : void 0;
    pendingAnchor = anchor;
    editingId = id || null;
    pendingElement = targetElement || (anchor.kind === BrowserAnnotationAnchorKind.Element ? resolveElement(anchor.selector) : null);
    if (pendingElement && anchor.kind === BrowserAnnotationAnchorKind.Element) {
      const liveElementEdit = readElementEdit(pendingElement);
      if (annotation == null ? void 0 : annotation.elementEdit) {
        ...
        if (!((_a = savedElementEdit.originalInlineStyle.styles) == null ? void 0 : _a.color)) {
        ...
        if (!((_b = savedElementEdit.originalInlineStyle.styles) == null ? void 0 : _b.backgroundColor)) {
        ...
        if (!((_c = savedElementEdit.originalInlineStyle.styles) == null ? void 0 : _c.opacity)) {
        ...
        if (!((_d = savedElementEdit.originalInlineStyle.styles) == null ? void 0 : _d.fontFamily)) {
    ...
    textarea.value = (annotation == null ? void 0 : annotation.comment) || "";
    ...
    annotations.slice().sort((a, b) => a.order - b.order).forEach((annotation, index) => {
      ...
      marker.addEventListener("click", (event) => {
        ...
        openEditor(
          ...
          annotation.id,
          annotation.anchor.kind === BrowserAnnotationAnchorKind.Element ? resolveElement(annotation.anchor.selector) : null
        );
      });
  ...
  const selectedTextAnchor = () => {
    ...
    const rects = Array.from(range.getClientRects()).map(rectOf).filter((rect2) => rect2.width > 0 && rect2.height > 0);
    if (rects.length === 0) return null;
    const left = Math.min(...rects.map((rect2) => rect2.x));
    const top = Math.min(...rects.map((rect2) => rect2.y));
    const right = Math.max(...rects.map((rect2) => rect2.x + rect2.width));
    const bottom = Math.max(...rects.map((rect2) => rect2.y + rect2.height));
    ...
    return {
      ...baseAnchor(rect, element || void 0),
  ...
  const onMouseUp = (event) => {
    ...
    if (anchor) openEditor(anchor, void 0, null, true);
  };
  const onClick = (event) => {
    ...
    const existing = annotations.find((item) => item.anchor.kind === BrowserAnnotationAnchorKind.Element && item.anchor.selector === anchor.selector);
    if (existing) {
      openEditor({ ...existing.anchor, rect: anchor.rect }, existing.id, element);
    } else {
      openEditor(anchor, void 0, element);
    }
  };
  const saveAnnotation = () => {
    const comment = textarea.value.trim().slice(0, BrowserAnnotationLimit.MaxCommentLength);
    const elementEdit = draftElementEdit && getBrowserAnnotationElementChanges(draftElementEdit).length > 0 ? cloneElementEdit(draftElementEdit) : void 0;
    if (!pendingAnchor || !hasBrowserAnnotationContent(comment, elementEdit)) return;
    const now = Date.now();
    const anchor = (pendingElement == null ? void 0 : pendingElement.isConnected) ? elementAnchor(pendingElement) : pendingAnchor;
    if (editingId) {
  ...
  const bindPixelInput = (input, property, min, max) => input.addEventListener("input", () => {
    ...
    updateDraft({
      [property]: Number.isFinite(value) ? `${Math.max(min, Math.min(max, value))}px` : void 0
    });
  });
  ...
  bindPixelInput(borderRadiusInput, BrowserAnnotationElementStyleProperty.BorderRadius, 0, 9999);
  bindPixelInput(borderWidthInput, BrowserAnnotationElementStyleProperty.BorderWidth, 0, 128);
  bindPixelInput(widthInput, BrowserAnnotationElementStyleProperty.Width, 0, 1e5);
  bindPixelInput(heightInput, BrowserAnnotationElementStyleProperty.Height, 0, 1e5);
  ...
  paddingInputs.forEach((input, index) => {
    bindPixelInput(input, paddingProperties[index], 0, 1e4);
  });
  ...
  marginInputs.forEach((input, index) => {
    bindPixelInput(input, marginProperties[index], -1e4, 1e4);
  });
  ...
  bindPixelInput(gapInputs[0], BrowserAnnotationElementStyleProperty.RowGap, 0, 1e4);
  bindPixelInput(gapInputs[1], BrowserAnnotationElementStyleProperty.ColumnGap, 0, 1e4);
  ...
  remove.addEventListener("click", () => {
    if (editingId) {
      ...
      } else if ((annotation == null ? void 0 : annotation.elementEdit) && element) {
        restoreElementEdit(element, annotation.elementEdit);
      }
      annotations = annotations.filter((item) => item.id !== editingId).map((item, index) => ({ ...item, order: index }));
      closeEditor(false);
      emitChanged();
    }
  });
  ...
  const commandListener = (_event, command) => {
    ...
    if (command.browserTabId !== activeEnvelope.browserTabId || command.documentId !== activeEnvelope.documentId || command.navigationVersion !== activeEnvelope.navigationVersion || command.batchId !== activeEnvelope.batchId) return;
    if (command.type === BrowserAnnotationGuestCommandType.Sync) {
      ...
      const annotation = annotations.find((item) => item.id === command.annotationId);
      const resolvedRect = annotation ? resolveAnchorRect(annotation.anchor, annotation.capture) : null;
      captureMode = true;
      editor.style.display = "none";
      pendingMarker.style.display = "none";
      ...
      electron.ipcRenderer.removeListener(BrowserAnnotationGuestChannel.Command, commandListener);
      activeCommandListener = null;
      cleanup == null ? void 0 : cleanup();
      cleanup = null;
      activeEnvelope = null;
  ...
  electron.ipcRenderer.on(BrowserAnnotationGuestChannel.Command, commandListener);
  send(BrowserAnnotationGuestEventType.Ready);
}
electron.ipcRenderer.on(
  BrowserAnnotationGuestChannel.Command,
  (_event, envelope) => {
    if ((envelope == null ? void 0 : envelope.protocolVersion) === BrowserAnnotationProtocolVersion && envelope.type === BrowserAnnotationGuestCommandType.Start) start(envelope);
  }
);
