...
const parseClientVersion = (value) => {
    ...
    if (parts.some(part => (!Number.isSafeInteger(part) || part > MAX_CLIENT_VERSION_PART)))
        return null;
    return { parts, prerelease };
};
const isClientVersionAtLeast = (currentVersion, minimumVersion) => {
    ...
    if (current.prerelease === null && minimum.prerelease !== null)
        return true;
    if (current.prerelease !== null && minimum.prerelease === null)
        return false;
    if (current.prerelease === null || minimum.prerelease === null)
...
exports.isClientVersionAtLeast = isClientVersionAtLeast;
