...
exports.PHASES = exports.WordState = exports.OTHER_PLACEHOLDER = exports.OTHER_REPLY = exports.ROLES = exports.INTRO_MS = exports.LINE_GAP_BEATS = exports.WORDS_PER_SECOND = exports.PermissionAnswer = exports.OnboardingPhase = void 0;
exports.taskCopy = taskCopy;
exports.linesOf = linesOf;
exports.wordsOf = wordsOf;
exports.segmentsOf = segmentsOf;
exports.beatsAt = beatsAt;
exports.shownWords = shownWords;
exports.phaseDone = phaseDone;
...
exports.OTHER_REPLY = "Noted. I'll work out the shape of it as we go.";
exports.OTHER_PLACEHOLDER = 'So what is it you do?';
/**
 ...
 */
function taskCopy(appearance) {
    const toDark = appearance !== constants_1.Appearance.Dark;
    const other = toDark ? 'dark' : 'light';
    return [
        {
            task: constants_1.OnboardingTask.Notes,
        ...
        {
            task: constants_1.OnboardingTask.Appearance,
        ...
        {
            task: constants_1.OnboardingTask.Messages,
...
function linesOf(phase, context) {
    switch (phase) {
        case exports.OnboardingPhase.Open: {
        ...
        case exports.OnboardingPhase.Role: {
            const reply = context.role === undefined
                ? ''
                : typeof context.role === 'number'
                    ? (exports.ROLES[context.role]?.reply ?? '')
                    : exports.OTHER_REPLY;
            return [
                reply,
        ...
        case exports.OnboardingPhase.Perm:
            return [context.task?.permLine ?? ''];
        case exports.OnboardingPhase.Task:
            if (context.answer !== exports.PermissionAnswer.Allowed) {
                return [
...
function segmentsOf(lines) {
    ...
    const segments = lines.map(text => {
        ...
        offset += words.length + exports.LINE_GAP_BEATS;
        return segment;
    });
    return { segments, total: offset };
}
/** How many beats have landed after `elapsedMs`, capped at the phase's end. */
function beatsAt(elapsedMs, total) {
    return Math.min(total, Math.floor((elapsedMs / 1000) * exports.WORDS_PER_SECOND));
}
exports.WordState = {
 ...
 */
function shownWords(lines, k, current, phase, order = exports.PHASES) {
    ...
    if (current.phase !== phase) {
        ...
        return played ? segment.words.map(text => ({ text, state: exports.WordState.Still })) : [];
    }
    const n = current.beats - segment.offset;
    return segment.words.map((text, i) => ({ text, state: i < n ? exports.WordState.In : exports.WordState.Hidden }));
}
/** Whether a phase has finished streaming. */
function phaseDone(lines, current, phase, order = exports.PHASES) {
    if (current.phase !== phase)
        return order.indexOf(phase) < order.indexOf(current.phase);
    return current.beats >= segmentsOf(lines).total;
}
exports.PHASES = [
    exports.OnboardingPhase.Open, exports.OnboardingPhase.Role, exports.OnboardingPhase.Perm, exports.OnboardingPhase.Task,
];
