...
exports.EVERY_ROW = exports.MAP_PLACEHOLDERS = exports.APP_UI_MAP_PATH = void 0;
exports.buildAppUiMap = buildAppUiMap;
...
exports.APP_UI_MAP_PATH = 'reference/app-ui.md';
/**
 ...
 */
exports.MAP_PLACEHOLDERS = ['PLACEHOLDER-NAME', 'PLACEHOLDER-DIR', 'PLACEHOLDER-VERSION'];
/**
 ...
 */
exports.EVERY_ROW = {
    accountName: exports.MAP_PLACEHOLDERS[0],
    computerName: 'this computer',
    workingDirectory: exports.MAP_PLACEHOLDERS[1],
    execPolicy: constants_1.ExecPolicy.Ask,
    memoryEnabled: true,
    usage: { fraction: 0, value: '', desc: '' },
    version: exports.MAP_PLACEHOLDERS[2],
    onSignOut: noop,
    onAddAccount: noop,
    onExecPolicy: noop,
    onMemory: noop,
...
function rowLine(row) {
    ...
    const control = row.kind === rows_1.SettingsRowKind.Select ? 'a menu'
        : row.kind === rows_1.SettingsRowKind.Toggle ? 'a switch'
            : row.kind === rows_1.SettingsRowKind.Button ? `a button marked "${row.action}"`
                : row.kind === rows_1.SettingsRowKind.Field ? (row.readOnly ? 'shown, not editable' : 'a box to type in')
 ...
 */
function buildAppUiMap(appName) {
    const lines = [
        ...
        '## The tabs',
        '',
        `Four, in this order: ${rows_1.SETTINGS_TABS.map(one => `**${one}**`).join(', ')}.`,
        '',
    ];
    for (const tab of rows_1.SETTINGS_TABS) {
        lines.push(`### ${tab}`, '');
        for (const group of (0, rows_1.settingsFor)(tab, exports.EVERY_ROW)) {
    ...
    lines.push('## What is not in this app', '', 'Say so plainly if asked for any of these, rather than hunting for', 'them:', '', '- A second computer, or a choice of which computer. There is one,', '  and it is the one the person is sitting at. Their files are worked', '  on where they live; nothing is copied to a machine of yours.', '- Any cloud computer to update or reset.', '- A theme or appearance setting, an accent colour, a language', '  setting, a microphone setting, hardware acceleration, a network', '  debugger, notification sounds, security keys, or auto-review rules.', '- Anything about a plan, a trial, or payment beyond the usage figure', '  on Usage & Billing.', '', '## Elsewhere in the app', '', '- **The computer icon** in the conversation header opens the panel:', '  the browser you drive, the files you have made, what you have', '  delegated, and what the person gave you.', '- **Models** are not a setting anywhere. Which model runs is decided', '  in code: the account\'s models through Caisra\'s own service. There', '  is no API key field and no allowance choice for the person to find.', '- **An agent\'s own settings** open from its name in the conversation', '  header, or from the small trash icon that appears on its row in the', '  sidebar while that conversation is open. The column that opens holds', '  its avatar (with an "Edit avatar" grid of twenty-five), its name, its', '  label, its description, a notifications switch, and "Delete agent".', '  That is the only place any of those change; they are not in Settings.', '- **Deleting an agent** is done there, by the person, and it is', '  permanent: the agent and its whole conversation go together, after a', '  "Delete" / "Keep" confirmation. There is no archive and no hide. The', '  first agent cannot be deleted, and neither can a room from this', '  column. You cannot delete an agent yourself; if asked, say where the', '  control is.', '- **A new agent, or a room,** starts from the `+` at the top of the', '  sidebar: a To: field where the person picks who it is for, then a', '  name, an avatar and a voice for a new agent.', '');
    return lines.join('\n');
}
//# sourceMappingURL=appUiMap.js.map