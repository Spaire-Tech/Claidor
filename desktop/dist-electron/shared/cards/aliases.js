...
exports.CARD_COMPONENT_ALIASES = void 0;
exports.normaliseCardProgram = normaliseCardProgram;
/** Their name, and ours. Argument order is identical for every pair. */
exports.CARD_COMPONENT_ALIASES = {
    CardHeader: 'Header',
    Callout: 'CalloutV2',
 ...
 */
const CALL = new RegExp(`(^|[^A-Za-z0-9_])(${Object.keys(exports.CARD_COMPONENT_ALIASES).join('|')})\\s*\\(`, 'g');
/** The program with the aliases corrected, and which ones were found. */
function normaliseCardProgram(program) {
    const corrected = new Set();
    const out = program.replace(CALL, (_all, before, name) => {
        corrected.add(name);
        return `${before}${exports.CARD_COMPONENT_ALIASES[name]}(`;
    });
