"use strict";
/**
 * One-time migration: move scheduled tasks from legacy SQLite tables
 * (used in the non-openclaw release) into the OpenClaw gateway via CronJobService.
 *
 * Safe to call multiple times — a kv flag prevents re-running.
 */
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.migrateScheduledTasksToOpenclaw = migrateScheduledTasksToOpenclaw;
exports.migrateScheduledTaskRunsToOpenclaw = migrateScheduledTaskRunsToOpenclaw;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const constants_1 = require("./constants");
// ---------------------------------------------------------------------------
// Converters
// ---------------------------------------------------------------------------
function formatLocalTimezoneOffset() {
    const offsetMinutes = -new Date().getTimezoneOffset();
    const sign = offsetMinutes >= 0 ? '+' : '-';
    const absMinutes = Math.abs(offsetMinutes);
    const hours = Math.floor(absMinutes / 60).toString().padStart(2, '0');
    const minutes = (absMinutes % 60).toString().padStart(2, '0');
    return `${sign}${hours}:${minutes}`;
}
/**
 * Ensure a datetime string has an explicit timezone offset.
 * Legacy `at` datetimes were stored as local time without offset (e.g. "2026-03-17T21:30:00").
 * The OpenClaw gateway interprets offset-less strings as UTC, causing an 8-hour drift
 * for users in UTC+8.  Append the local timezone offset when missing.
 */
function ensureTimezoneOffset(datetime) {
    // Already has an offset (+HH:MM, -HH:MM) or trailing 'Z'
    if (/(?:Z|[+-]\d{2}:\d{2})\s*$/.test(datetime))
        return datetime;
    return `${datetime}${formatLocalTimezoneOffset()}`;
}
function convertSchedule(legacy) {
    if (legacy.type === 'at') {
        if (!legacy.datetime)
            return null;
        const withTz = ensureTimezoneOffset(legacy.datetime);
        // Skip one-time tasks whose scheduled time is already in the past —
        // the gateway rejects them and they would never fire anyway.
        if (new Date(withTz).getTime() <= Date.now())
            return null;
        return { kind: constants_1.ScheduleKind.At, at: withTz };
    }
    if (legacy.type === 'interval') {
        const ms = legacy.intervalMs;
        if (!ms || ms <= 0)
            return null;
        return { kind: constants_1.ScheduleKind.Every, everyMs: ms };
    }
    if (legacy.type === 'cron') {
        if (!legacy.expression)
            return null;
        return { kind: constants_1.ScheduleKind.Cron, expr: legacy.expression };
    }
    return null;
}
function convertDelivery(platformsJson) {
    let platforms = [];
    try {
        platforms = JSON.parse(platformsJson);
    }
    catch {
        // ignore
    }
    if (!Array.isArray(platforms) || platforms.length === 0) {
        return { mode: constants_1.DeliveryMode.None };
    }
    // New format supports one delivery target — use the first platform as channel.
    return { mode: constants_1.DeliveryMode.Announce, channel: platforms[0] };
}
function rowToInput(row) {
    let legacySchedule;
    try {
        legacySchedule = JSON.parse(row.schedule_json);
    }
    catch {
        console.warn(`[MigrateScheduledTasks] Skipping task "${row.name}" — invalid schedule_json`);
        return null;
    }
    const schedule = convertSchedule(legacySchedule);
    if (!schedule) {
        console.warn(`[MigrateScheduledTasks] Skipping task "${row.name}" — cannot convert schedule`, legacySchedule);
        return null;
    }
    return {
        name: row.name,
        description: row.description ?? '',
        enabled: row.enabled === 1,
        schedule,
        // 旧任务都带有 prompt，使用 isolated session + agentTurn。
        // main session 仅支持 systemEvent payload，不适用于迁移场景。
        sessionTarget: constants_1.SessionTarget.Isolated,
        wakeMode: constants_1.WakeMode.NextHeartbeat,
        payload: { kind: constants_1.PayloadKind.AgentTurn, message: row.prompt },
        delivery: convertDelivery(row.notify_platforms_json ?? '[]'),
        agentId: constants_1.DefaultAgentId,
    };
}
async function migrateScheduledTasksToOpenclaw(deps) {
    const { db, getKv, setKv, cronJobService } = deps;
    // 1. Idempotency guard
    if (getKv(constants_1.MigrationKey.TasksToOpenclaw) === 'true')
        return;
    // 2. Check if the legacy table exists (new installs won't have it)
    try {
        const tableExists = db
            .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='scheduled_tasks'")
            .get();
        if (!tableExists) {
            setKv(constants_1.MigrationKey.TasksToOpenclaw, 'true');
            return;
        }
    }
    catch (err) {
        console.warn('[MigrateScheduledTasks] Could not check legacy table existence, skipping:', err);
        return;
    }
    let rows = [];
    try {
        rows = db
            .prepare('SELECT id, name, description, enabled, schedule_json, prompt, notify_platforms_json FROM scheduled_tasks')
            .all();
        if (!rows.length) {
            setKv(constants_1.MigrationKey.TasksToOpenclaw, 'true');
            return;
        }
    }
    catch (err) {
        console.warn('[MigrateScheduledTasks] Failed to read legacy tasks, skipping migration:', err);
        return;
    }
    console.log(`[MigrateScheduledTasks] Migrating ${rows.length} task(s) to OpenClaw gateway...`);
    // 4. Push each task to the OpenClaw gateway
    let succeeded = 0;
    let skipped = 0;
    let gatewayErrors = 0;
    for (const row of rows) {
        const input = rowToInput(row);
        if (!input) {
            skipped++;
            continue;
        }
        try {
            await cronJobService.addJob(input);
            console.log(`[MigrateScheduledTasks] Migrated task: "${row.name}"`);
            succeeded++;
        }
        catch (err) {
            console.error(`[MigrateScheduledTasks] Failed to migrate task "${row.name}":`, err);
            gatewayErrors++;
        }
    }
    console.log(`[MigrateScheduledTasks] Done. succeeded=${succeeded}, skipped=${skipped}, gatewayErrors=${gatewayErrors}`);
    // 5. Mark as done only when there are no gateway errors.
    // Skipped tasks (invalid schedule etc.) are unrecoverable and don't block completion.
    // Gateway errors may be transient, so we leave the flag unset to allow a retry on next launch.
    if (gatewayErrors === 0) {
        setKv(constants_1.MigrationKey.TasksToOpenclaw, 'true');
    }
}
// ---------------------------------------------------------------------------
// Run history migration: SQLite scheduled_task_runs → OpenClaw JSONL files
// ---------------------------------------------------------------------------
const RUN_HISTORY_MIGRATION_KEY = constants_1.MigrationKey.RunsToOpenclaw;
function toGatewayStatus(status) {
    if (status === 'success')
        return constants_1.GatewayStatus.Ok;
    if (status === 'error')
        return constants_1.GatewayStatus.Error;
    return constants_1.GatewayStatus.Skipped;
}
async function migrateScheduledTaskRunsToOpenclaw(deps) {
    const { db, getKv, setKv, openclawStateDir } = deps;
    // 1. Idempotency guard
    if (getKv(RUN_HISTORY_MIGRATION_KEY) === 'true')
        return;
    try {
        const tableExists = db
            .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='scheduled_task_runs'")
            .get();
        if (!tableExists) {
            setKv(RUN_HISTORY_MIGRATION_KEY, 'true');
            return;
        }
    }
    catch (err) {
        console.warn('[MigrateRunHistory] Could not check legacy tables, skipping:', err);
        return;
    }
    let runs = [];
    try {
        runs = db
            .prepare('SELECT id, task_id, session_id, status, started_at, finished_at, duration_ms, error FROM scheduled_task_runs ORDER BY started_at ASC')
            .all();
        if (!runs.length) {
            setKv(RUN_HISTORY_MIGRATION_KEY, 'true');
            return;
        }
    }
    catch (err) {
        console.warn('[MigrateRunHistory] Failed to read legacy runs:', err);
        return;
    }
    const taskIdToName = new Map();
    try {
        const taskRows = db.prepare('SELECT id, name FROM scheduled_tasks').all();
        for (const row of taskRows) {
            if (row.id && row.name)
                taskIdToName.set(row.id, row.name);
        }
    }
    catch {
        // Non-fatal: names will be omitted if the table is unavailable
    }
    console.log(`[MigrateRunHistory] Migrating ${runs.length} run(s) to OpenClaw cron/runs/...`);
    // 4. Ensure runs directory exists
    const runsDir = path_1.default.join(openclawStateDir, 'cron', 'runs');
    try {
        fs_1.default.mkdirSync(runsDir, { recursive: true });
    }
    catch (err) {
        console.warn('[MigrateRunHistory] Failed to create runs directory:', err);
        return;
    }
    let succeeded = 0;
    let skipped = 0;
    // 5. Group runs by task_id (used as-is for the JSONL filename)
    const runsByTaskId = new Map();
    for (const run of runs) {
        let arr = runsByTaskId.get(run.task_id);
        if (!arr) {
            arr = [];
            runsByTaskId.set(run.task_id, arr);
        }
        arr.push(run);
    }
    for (const [taskId, taskRuns] of runsByTaskId.entries()) {
        const jsonlPath = path_1.default.join(runsDir, `${taskId}.jsonl`);
        // Collect existing timestamps to avoid duplicates on re-run
        const existingTs = new Set();
        try {
            const existing = fs_1.default.readFileSync(jsonlPath, 'utf-8');
            for (const line of existing.split('\n')) {
                const trimmed = line.trim();
                if (!trimmed)
                    continue;
                try {
                    const entry = JSON.parse(trimmed);
                    if (typeof entry.ts === 'number')
                        existingTs.add(entry.ts);
                }
                catch { /* ignore malformed lines */ }
            }
        }
        catch { /* file doesn't exist yet — that's fine */ }
        const lines = [];
        for (const run of taskRuns) {
            const startedMs = new Date(run.started_at).getTime();
            const finishedMs = run.finished_at ? new Date(run.finished_at).getTime() : startedMs;
            if (existingTs.has(finishedMs)) {
                skipped++;
                continue;
            }
            const entry = {
                ts: finishedMs,
                jobId: taskId,
                action: 'finished',
                status: toGatewayStatus(run.status),
                runAtMs: startedMs,
            };
            if (typeof run.duration_ms === 'number')
                entry['durationMs'] = run.duration_ms;
            if (run.error)
                entry['error'] = run.error;
            if (run.session_id)
                entry['sessionId'] = run.session_id;
            const jobName = taskIdToName.get(taskId);
            if (jobName)
                entry['summary'] = jobName;
            lines.push(JSON.stringify(entry));
            succeeded++;
        }
        if (lines.length > 0) {
            try {
                fs_1.default.appendFileSync(jsonlPath, lines.join('\n') + '\n', 'utf-8');
            }
            catch (err) {
                console.error(`[MigrateRunHistory] Failed to write runs for task ${taskId}:`, err);
            }
        }
    }
    console.log(`[MigrateRunHistory] Done. succeeded=${succeeded}, skipped=${skipped}`);
    setKv(RUN_HISTORY_MIGRATION_KEY, 'true');
}
//# sourceMappingURL=migrate.js.map