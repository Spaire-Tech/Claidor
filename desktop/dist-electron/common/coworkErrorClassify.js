...
exports.CoworkErrorI18nKey = void 0;
exports.classifyErrorKey = classifyErrorKey;
exports.isLobsterAIQuotaExhaustedError = isLobsterAIQuotaExhaustedError;
const constants_1 = require("../shared/openclawEngine/constants");
const constants_2 = require("../shared/openclawTranscript/constants");
exports.CoworkErrorI18nKey = {
...
const LOBSTERAI_QUOTA_EXHAUSTED_PATTERN = /\b(?:4020[0-2]|4160[678])\b|(?:今日)?免费额度.*(用完|耗尽)|本月积分.*(用完|耗尽)|积分额度.*(用完|耗尽)|free.*quota.*(exhausted|used up|limit)|monthly.*credits?.*(exhausted|used up|limit)/i;
const MODEL_CAPACITY_OVERLOAD_PATTERN = /overloaded_error|\boverloaded\b|(?:selected\s+)?model\s+(?:is\s+)?at capacity|system capacity(?: limits?)?|(?:service|model).*(?:high demand|high load)|服务过载|当前负载过高|系统容量(?:不足|限制)?/i;
...
const ERROR_RULES = [
    ...
    [/oauth.*(invalid|expired|failed|error|scope|token|callback|authorization|not completed)|auth[_ ]refresh|refresh[_ ]timeout|callback[_ ](timeout|validation)|token.*(expired|invalid)|invalid.*token|authorization method/i, exports.CoworkErrorI18nKey.OAuthInvalid],
    // Provider/model permission errors. Must precede generic auth handling.
    [/无权访问|没有权限|access denied|access.*forbidden|forbidden|permission denied|\b403\b|auth[_ ]scope/i, exports.CoworkErrorI18nKey.ModelAccessDenied],
    // Auth: Anthropic, DeepSeek, OpenAI, Gemini, HTTP 401
    [new RegExp(`authentication[_ ](error|fails?)|${API_KEY_PATTERN}.*(invalid|expired|deleted|inactive|not[_ ]valid|not\\s+valid)|invalid.*${API_KEY_PATTERN}|incorrect.*${API_KEY_PATTERN}|unauthorized|PERMISSION_DENIED|\\b401\\b`, 'i'), exports.CoworkErrorI18nKey.AuthInvalid],
    // LobsterAI plan/free quota. Must precede generic 402/billing handling.
    [LOBSTERAI_QUOTA_EXHAUSTED_PATTERN, exports.CoworkErrorI18nKey.QuotaExhausted],
    ...
    [MODEL_CAPACITY_OVERLOAD_PATTERN, exports.CoworkErrorI18nKey.ModelOverloaded],
    ...
    [/\b429\b|rate[_ ]limit|too many requests|RESOURCE_EXHAUSTED/i, exports.CoworkErrorI18nKey.RateLimit],
    // Billing: DeepSeek 402, OpenAI, OpenRouter, Qwen, StepFun
    [/insufficient.*(balance|quota|credits)|billing|quota[_ ]exceeded|Arrearage|account.*not.*in.*good.*standing|余额不足|\b402\b/i, exports.CoworkErrorI18nKey.InsufficientBalance],
    ...
    [new RegExp(constants_2.OpenClawTranscriptSafetyErrorCode.ActiveTranscriptOversized, 'i'), exports.CoworkErrorI18nKey.TranscriptOversized],
    [new RegExp(`gatewayFailureKind=${constants_1.OpenClawGatewayFailureKind.HeapOutOfMemory}|JavaScript heap out of memory`, 'i'), exports.CoworkErrorI18nKey.GatewayHeapOutOfMemory],
    ...
    [/LLM (?:idle timeout|request timed out)|no response from model|model response (?:timeout|timed out)/i, exports.CoworkErrorI18nKey.ModelResponseTimeout],
    // Network errors
    [new RegExp(`${UNAVAILABLE_NETWORK_CODE_PATTERN}|fetch failed|ConnectTimeoutError|network request failed|socket (?:hang up|closed|reset)|connection.*(?:refused|reset|aborted|closed|timeout|timed out)|could not connect|network.*error|request.*timed out`, 'i'), exports.CoworkErrorI18nKey.NetworkError],
    // Server errors: HTTP 500/502/503
    [/internal.server.error|bad.gateway|service.unavailable|\b50[023]\b/i, exports.CoworkErrorI18nKey.ServerError],
 ...
 */
function classifyErrorKey(error) {
...
function isLobsterAIQuotaExhaustedError(error) {
    return LOBSTERAI_QUOTA_EXHAUSTED_PATTERN.test(error);
}
//# sourceMappingURL=coworkErrorClassify.js.map