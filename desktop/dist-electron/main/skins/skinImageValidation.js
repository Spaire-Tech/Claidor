...
exports.inspectSkinImage = inspectSkinImage;
...
function hasValidPngHeaderFields(data, dataOffset) {
    ...
    return Boolean(allowedBitDepths[colorType]?.includes(bitDepth) &&
...
function inspectPng(data) {
    ...
    while (offset + 12 <= data.length) {
        ...
        if (!/^[A-Za-z]{4}$/.test(chunkType))
            return null;
        if (data.readUInt32BE(crcOffset) !== crc32(data.subarray(typeOffset, crcOffset)))
            return null;
        if (chunkType === 'IHDR') {
            if (sawHeader || offset !== PNG_SIGNATURE.length || chunkLength !== 13)
                return null;
            ...
            if (chunkLength !== 0 || !sawImageData || chunkEnd !== data.length)
                return null;
            return {
                format: constants_1.SkinAssetFormat.Png,
                extension: constants_1.SkinAssetExtension.Png,
                mimeType: constants_1.SkinAssetMimeType.Png,
                width,
                height,
            };
        }
...
function inspectJpeg(data) {
    ...
    while (offset < data.length - 2) {
        ...
        if (marker === 0xda) {
            if (segmentLength < 6 || width === 0 || height === 0)
                return null;
            sawScan = true;
            break;
        }
    ...
    if (!sawScan || width === 0 || height === 0)
        return null;
    return {
        format: constants_1.SkinAssetFormat.Jpeg,
        extension: constants_1.SkinAssetExtension.Jpeg,
        mimeType: constants_1.SkinAssetMimeType.Jpeg,
        width,
        height,
    };
}
...
function inspectWebp(data) {
    ...
    if (offset !== data.length || !hasImageData || width === 0 || height === 0)
        return null;
    return {
        format: constants_1.SkinAssetFormat.Webp,
        extension: constants_1.SkinAssetExtension.Webp,
        mimeType: constants_1.SkinAssetMimeType.Webp,
        width,
        height,
    };
}
function inspectSkinImage(data) {
    return inspectPng(data) ?? inspectJpeg(data) ?? inspectWebp(data);
}
//# sourceMappingURL=skinImageValidation.js.map