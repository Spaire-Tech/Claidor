...
const libraryMigrations_1 = require("./library/libraryMigrations");
...
const extractScheduledTaskIdFromMessageMetadata = (metadata) => {
    ...
    try {
        ...
        const sessionKey = parsedMetadata[openclawCronSessionKey_1.OpenClawCronRunMetadataKey.SessionKey];
        if (typeof sessionKey !== 'string')
            return null;
        return (0, openclawCronSessionKey_1.parseOpenClawCronSessionKey)(sessionKey.trim())?.scheduledTaskId ?? null;
    }
    catch {
        return null;
    }
...
class SqliteStore {
    ...
    didRunMigration = false;
    constructor(db, dbPath) {
        this.db = db;
        this.dbPath = dbPath;
    }
    static async create(userDataPath) {
        const basePath = userDataPath ?? electron_1.app.getPath('userData');
        const dbPath = path_1.default.join(basePath, appConstants_1.DB_FILENAME);
        let db = (0, sqliteBackupManager_1.openSqliteDatabaseWithRecovery)(basePath, dbPath);
        const autoBackupEnabled = SqliteStore.readSqliteAutoBackupEnabled(db);
        if (autoBackupEnabled) {
            const backupManager = new sqliteBackupManager_1.SqliteBackupManager(basePath);
            ...
            if (!health.ok) {
                ...
                db = (0, sqliteBackupManager_1.openSqliteDatabaseWithRecovery)(basePath, dbPath);
            }
        }
    ...
    `);
        (0, libraryMigrations_1.initializeLibraryTables)(this.db);
        this.db.exec(`
    ...
    `);
        ...
        try {
            const subagentCols = this.db.pragma('table_info(subagent_runs)');
      ...
      `);
        ...
        try {
            const sessionCols = this.db.pragma('table_info(cowork_sessions)');
        ...
        try {
            ...
            const msgColumns = this.db.pragma('table_info(cowork_messages)');
      ...
      `);
        ...
        try {
            if (this.get(SCHEDULED_TASK_SESSION_BACKFILL_KEY) !== '1') {
                this.db.function(EXTRACT_SCHEDULED_TASK_ID_SQL_FUNCTION, { deterministic: true }, extractScheduledTaskIdFromMessageMetadata);
                const backfillScheduledTaskIds = this.db.transaction(() => {
                    const result = this.db.prepare(`WITH candidate_sessions AS MATERIALIZED (
             ...
             WHERE sessions.id = candidates.session_id
               AND candidates.scheduled_task_id IS NOT NULL`).run(`%"${openclawCronSessionKey_1.OpenClawCronRunMetadataKey.SessionKey}"%`);
                    return result.changes;
                });
                const backfilledSessionCount = backfillScheduledTaskIds();
                if (backfilledSessionCount > 0) {
                    this.didRunMigration = true;
                    console.log(`[SqliteStore] backfilled scheduled task ids for ${backfilledSessionCount} legacy sessions.`);
                }
        ...
        try {
            const agentCols = this.db.pragma('table_info(agents)');
        ...
        try {
          ...
          VALUES (?, ?, '', ?, '', '', ?, '[]', 1, 1, 'custom', '', ?, ?)
        `)
                    .run(agent_1.AgentId.Main, agent_1.DefaultAgentProfile.Name, existingSystemPrompt, agent_1.DefaultAgentAvatarIcon, now, now);
            }
            else {
                ...
                const shouldUpgradeName = !normalizedName
                    || normalizedName === agent_1.LegacyAgentName.Main
                    || normalizedName === agent_1.LegacyAgentName.Upstream
                    || normalizedName === agent_1.LegacyAgentName.Faiser
                    || normalizedName === agent_1.LegacyAgentName.Caisra;
                if (shouldUpgradeName) {
                    this.db
                        .prepare('UPDATE agents SET name = ?, updated_at = ? WHERE id = ?')
                        .run(agent_1.DefaultAgentProfile.Name, Date.now(), agent_1.AgentId.Main);
                    this.didRunMigration = true;
                }
            }
        }
        ...
        `)
            ...
            if (rows.length > 0) {
                ...
                const backfillSortOrder = this.db.transaction((agents) => {
        ...
        try {
            if (this.get(AGENT_WORKING_DIRECTORY_BACKFILL_KEY) !== '1') {
    ...
    delete(key) {
        ...
        this.emitter.emit('change', { key, newValue: undefined, oldValue });
    }
    getDatabase() {
        return this.db;
    }
    ...
    static readSqliteAutoBackupEnabled(db) {
        try {
    ...
    tryReadLegacyMemoryText() {
        const candidates = [
            path_1.default.join(process.cwd(), 'MEMORY.md'),
            path_1.default.join(electron_1.app.getAppPath(), 'MEMORY.md'),
            path_1.default.join(process.cwd(), 'memory.md'),
            path_1.default.join(electron_1.app.getAppPath(), 'memory.md'),
        ];
    ...
    memoryFingerprint(text) {
        const normalized = text
            ...
            .replace(/\s+/g, ' ')
            .trim();
        return crypto_1.default.createHash('sha1').update(normalized).digest('hex');
    }
    ...
    `);
        const checkExisting = this.db.prepare(`SELECT id FROM user_memories WHERE fingerprint = ? AND status != 'deleted' LIMIT 1`);
        const migrate = this.db.transaction(() => {
            for (const text of entries) {
                const fingerprint = this.memoryFingerprint(text);
                if (checkExisting.get(fingerprint))
                    continue;
                const memoryId = crypto_1.default.randomUUID();
                insertMemory.run(memoryId, text, fingerprint, 0.9, now, now);
                insertSource.run(crypto_1.default.randomUUID(), memoryId, now);
            }
        });
        try {
            migrate();
    ...
    migrateFromElectronStore(userDataPath) {
        const row = this.db.prepare('SELECT COUNT(*) as count FROM kv').get();
        if (row.count > 0)
            return;
        const legacyPath = path_1.default.join(userDataPath, 'config.json');
        if (!fs_1.default.existsSync(legacyPath))
            return;
        try {
            const raw = fs_1.default.readFileSync(legacyPath, 'utf8');
