...
exports.ComputerUseMcpEnv = exports.ComputerUseMcpServerName = void 0;
exports.resolvePackageRoot = resolvePackageRoot;
exports.resolveComputerUseRuntimePaths = resolveComputerUseRuntimePaths;
exports.ensureComputerUseMcpServerScript = ensureComputerUseMcpServerScript;
exports.resolveComputerUseMcpServer = resolveComputerUseMcpServer;
const electron_1 = require("electron");
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const resolveStdioCommand_1 = require("../libs/resolveStdioCommand");
const computerUseLogs_1 = require("./computerUseLogs");
const computerUseRuntime_1 = require("./computerUseRuntime");
exports.ComputerUseMcpServerName = {
    BuiltIn: 'computer-use',
};
exports.ComputerUseMcpEnv = {
...
function resolvePackageRoot(packageName) {
    try {
        let currentDir = path_1.default.dirname(require.resolve(`${packageName}/package.json`));
        while (currentDir && currentDir !== path_1.default.dirname(currentDir)) {
...
function resolveComputerUseRuntimePaths() {
    if (process.platform !== 'win32') {
        return null;
    }
    const inspection = (0, computerUseRuntime_1.inspectComputerUseRuntime)();
    return inspection.paths;
}
function ensureComputerUseMcpServerScript() {
    const scriptDir = path_1.default.join(electron_1.app.getPath('userData'), 'mcp-bridge', 'bin');
    fs_1.default.mkdirSync(scriptDir, { recursive: true });
    const scriptPath = path_1.default.join(scriptDir, SERVER_SCRIPT_NAME);
    const existing = isFile(scriptPath) ? fs_1.default.readFileSync(scriptPath, 'utf8') : '';
    if (existing !== COMPUTER_USE_MCP_SERVER_SCRIPT) {
        fs_1.default.writeFileSync(scriptPath, COMPUTER_USE_MCP_SERVER_SCRIPT, 'utf8');
    }
    return scriptPath;
}
function resolveComputerUseMcpServer(options) {
    if (process.platform !== 'win32') {
        return null;
    }
    ...
    if (!runtimePaths) {
        ...
        const missing = inspection.missing.length > 0
            ...
            : '';
        console.warn(`[ComputerUseMCP] skipped built-in server because Computer Use runtime is not installed (status=${inspection.status}, userData=${electron_1.app.getPath('userData')}${missing})`);
        return null;
    }
    ...
    const systemNodePath = electron_1.app.isPackaged ? null : (0, resolveStdioCommand_1.findSystemNodePath)();
    const command = systemNodePath || options.electronNodePath;
    const env = {
        [exports.ComputerUseMcpEnv.AskUserUrl]: options.askUserCallbackUrl,
        [exports.ComputerUseMcpEnv.BridgeSecret]: options.bridgeSecret,
        [exports.ComputerUseMcpEnv.ClientModulePath]: runtimePaths.clientModulePath,
        [exports.ComputerUseMcpEnv.ExePath]: runtimePaths.helperExePath,
        [exports.ComputerUseMcpEnv.HelperStateHome]: (0, computerUseRuntime_1.ensureComputerUseHelperStateHome)(),
        [exports.ComputerUseMcpEnv.LogDir]: (0, computerUseLogs_1.ensureComputerUseLogDir)(),
        [exports.ComputerUseMcpEnv.LogLevel]: 'info',
        [exports.ComputerUseMcpEnv.LogRetentionDays]: String((0, computerUseLogs_1.getComputerUseLogRetentionDays)()),
        [exports.ComputerUseMcpEnv.RuntimePackageRoot]: runtimePaths.runtimePackageRoot,
        [exports.ComputerUseMcpEnv.SdkRoot]: sdkRoot,
        [exports.ComputerUseMcpEnv.ZodRoot]: zodRoot,
    };
    if (!systemNodePath) {
        env.ELECTRON_RUN_AS_NODE = '1';
    }
    return {
        name: exports.ComputerUseMcpServerName.BuiltIn,
        transportType: 'stdio',
        command,
...
`;
//# sourceMappingURL=computerUseMcpServer.js.map