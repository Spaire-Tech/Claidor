...
exports.migrateScheduledTaskAnnounceJobs = migrateScheduledTaskAnnounceJobs;
exports.registerScheduledTaskHandlers = registerScheduledTaskHandlers;
const electron_1 = require("electron");
const constants_1 = require("../../../scheduledTask/constants");
const constants_2 = require("../../../shared/agent/constants");
const constants_3 = require("../../../shared/openclawEngine/constants");
...
const DINGTALK_PLATFORM = 'dingtalk';
const WECOM_PLATFORM = 'wecom';
const CASE_SENSITIVE_GROUP_TARGET_PLATFORMS = new Set([
    DINGTALK_PLATFORM,
    WECOM_PLATFORM,
]);
function normalizeImAnnounceDeliveryTo(rawTo, mappings, platform) {
    const parsed = (0, platform_1.parseImConversationId)(rawTo);
    if (parsed.peerKind === platform_1.ImPeerKind.Direct ||
        parsed.peerKind === platform_1.ImPeerKind.Group ||
    ...
    if (peer) {
        for (const mapping of mappings) {
            const mappingParsed = (0, platform_1.parseImConversationId)(mapping.imConversationId);
            if (mappingParsed.peerId.trim().toLowerCase() !== peer)
                continue;
...
function summarizeRelevantBindings(platformAgentBindings, platform, selectedAccountId) {
...
function logChannelConversationList(params) {
    ...
    console.debug('[ScheduledTask] listed channel conversations:', JSON.stringify({
        ...
        relevantBindings: summarizeRelevantBindings(params.platformAgentBindings, params.platform, params.selectedAccountId),
    }));
}
/**
 ...
 */
function applyLocalAnnounceDeliveryNormalization(normalizedInput, deps) {
    const { getIMGatewayManager } = deps;
    const delivery = normalizedInput.delivery;
    if (!(delivery && delivery.mode === constants_1.DeliveryMode.Announce && delivery.channel && delivery.to)) {
        return null;
    }
    ...
    normalizedInput.sessionTarget = constants_1.SessionTarget.Isolated;
    if (normalizedInput.payload?.kind === constants_1.PayloadKind.SystemEvent) {
    ...
    const parsedConversation = (0, platform_1.parseImConversationId)(rawTo);
    delivery.to = normalizeImAnnounceDeliveryTo(rawTo, mappings, platform);
    if (delivery.to !== rawTo) {
        console.debug('[ScheduledTask] normalized IM delivery.to:', rawTo, '->', delivery.to);
    }
    ...
    const existingAgentId = typeof normalizedInput.agentId === 'string'
        ...
        : '';
    if (!existingAgentId || existingAgentId === constants_2.AgentId.Main) {
        try {
            const boundAgentId = (0, helpers_1.resolveConversationAgentIdFromMappings)(mappings, rawTo, parsedConversation.accountId ?? delivery.accountId, {
                platform,
                platformAgentBindings: imSettings?.platformAgentBindings,
            });
            if (boundAgentId && boundAgentId !== constants_2.AgentId.Main) {
                normalizedInput.agentId = boundAgentId;
                console.log('[ScheduledTask] bound delivery job to conversation agent:', boundAgentId);
            }
        }
        catch (error) {
...
function describeRunFilter(filter) {
    if (!filter)
        return null;
    return {
...
async function restoreAnnounceDeliveryHintsFromGateway(normalizedInput, context, deps, options) {
    ...
    try {
        if (await ensureScheduledTaskGatewayClient(getOpenClawRuntimeAdapter)) {
            ...
            if (client) {
                const result = await client.request('sessions.list', { includeGlobal: true, includeUnknown: true, limit: 500 }, { timeoutMs: 10_000 });
                const sessions = Array.isArray(result?.sessions) ? result.sessions : [];
                const selectedAccountId = typeof delivery.accountId === 'string'
                    ? delivery.accountId
                    : undefined;
                if (!options?.casingOnly) {
                    const hints = (0, helpers_1.resolveImDeliveryHintsFromSessions)({
                        sessions,
                    ...
                    if (hints) {
                        if (hints.to !== delivery.to) {
                            console.log('[ScheduledTask] restored delivery.to casing from gateway session:', delivery.to, '->', hints.to);
                ...
                if (CASE_SENSITIVE_GROUP_TARGET_PLATFORMS.has(context.platform)) {
                    const nativeGroupTarget = (0, helpers_1.resolveGroupDeliveryTargetFromSessions)({
                        sessions,
                    ...
                    if (nativeGroupTarget && nativeGroupTarget !== delivery.to) {
                        console.log(`[ScheduledTask] restored ${context.platform} group delivery.to casing from gateway origin:`, delivery.to, '->', nativeGroupTarget);
...
async function primeAnnounceReplyRoute(context, deps) {
    if (context.platform === 'dingtalk') {
        const { getIMGatewayManager } = deps;
        const imStore = getIMGatewayManager()?.getIMStore();
        const mapping = imStore?.getSessionMapping(context.rawTo, context.platform);
        if (mapping) {
            await getIMGatewayManager().primeConversationReplyRoute(context.platform, context.rawTo, mapping.coworkSessionId);
        }
    }
}
/**
 ...
 */
async function applyAnnounceDeliveryNormalization(normalizedInput, deps) {
...
async function buildAnnounceNormalizationPatch(task, deps) {
...
async function migrateScheduledTaskAnnounceJobs(deps) {
...
async function ensureScheduledTaskGatewayClient(getOpenClawRuntimeAdapter) {
    const adapter = getOpenClawRuntimeAdapter();
    if (!adapter)
        return false;
    if (adapter.getGatewayClient())
        return true;
    ...
    if (adapter.getEngineStatusSnapshot().phase !== constants_3.OpenClawEnginePhase.Running) {
        return false;
    }
    await adapter.connectGatewayIfNeeded();
    return Boolean(adapter.getGatewayClient());
}
function registerScheduledTaskHandlers(deps) {
    const { getCronJobService, getIMGatewayManager, getOpenClawRuntimeAdapter, getCoworkSessionTitle } = deps;
    electron_1.ipcMain.handle(constants_1.IpcChannel.List, async () => {
        try {
            if (!(await ensureScheduledTaskGatewayClient(getOpenClawRuntimeAdapter))) {
    ...
    electron_1.ipcMain.handle(constants_1.IpcChannel.Get, async (_event, id) => {
        try {
    ...
    electron_1.ipcMain.handle(constants_1.IpcChannel.Create, async (_event, input) => {
        try {
    ...
    electron_1.ipcMain.handle(constants_1.IpcChannel.Update, async (_event, id, input) => {
        try {
            const normalizedInput = input && typeof input === 'object' ? { ...input } : {};
    ...
    electron_1.ipcMain.handle(constants_1.IpcChannel.Delete, async (_event, id) => {
        try {
    ...
    electron_1.ipcMain.handle(constants_1.IpcChannel.Toggle, async (_event, id, enabled) => {
        try {
    ...
    electron_1.ipcMain.handle(constants_1.IpcChannel.RunManually, async (_event, id) => {
        try {
    ...
    electron_1.ipcMain.handle(constants_1.IpcChannel.Stop, async (_event, _id) => {
        // OpenClaw doesn't expose a direct stop API for running cron jobs
        // The job will complete or timeout on its own
        return { success: true, result: false };
    });
    electron_1.ipcMain.handle(constants_1.IpcChannel.ListRuns, async (_event, taskId, limit, offset, filter) => {
        try {
    ...
    electron_1.ipcMain.handle(constants_1.IpcChannel.CountRuns, async (_event, taskId) => {
        try {
    ...
    electron_1.ipcMain.handle(constants_1.IpcChannel.ListAllRuns, async (_event, limit, offset, filter) => {
        try {
    ...
    electron_1.ipcMain.handle(constants_1.IpcChannel.ResolveSession, async (_event, input) => {
        try {
    ...
    electron_1.ipcMain.handle(constants_1.IpcChannel.ListChannels, async () => {
        try {
            return { success: true, channels: (0, helpers_1.listScheduledTaskChannels)() };
        }
        catch (error) {
            return {
                success: false,
    ...
    electron_1.ipcMain.handle(constants_1.IpcChannel.ListChannelConversations, async (_event, channel, accountId, filterAccountId) => {
        try {
            const platform = platform_1.PlatformRegistry.platformOfChannel(channel);
            if (!platform)
            ...
            const filteredMappings = (0, helpers_1.filterConversationMappingsForSelectedAccount)(rawMappings, platform, selectedAccountId, platformAgentBindings);
            const mappings = (0, helpers_1.dedupeConversationMappings)(filteredMappings);
            ...
            const conversations = mappings.map(m => {
                const parsed = (0, platform_1.parseImConversationId)(m.imConversationId);
                ...
                const customTitle = sessionTitle && !AUTO_CHANNEL_TITLE_RE.test(sessionTitle) ? sessionTitle : undefined;
                return {
                    ...
                    displayName: customTitle ?? (0, platform_1.imConversationDisplayName)(m.imConversationId),
                };
            });
            return { success: true, conversations };
        }
