...
const translations = {
    zh: {
        ...
        taskOutputTruncated: '[输出未完成] 模型已达到本次输出长度上限。部分结果已保留，但任务未确认完成；你可以继续对话以从中断处继续。',
        // Feishu bot install
        ...
        coworkErrorQuotaExhausted: '本月额度已用完，将于下月初重置。你可以在「设置 → 模型」中填入自己的 API Key 继续使用。',
        coworkErrorFreeQuotaExhausted: '本月额度已用完，将于下月初重置。你可以在「设置 → 模型」中填入自己的 API Key 继续使用。',
        ...
        coworkErrorMessageTooLarge: '本次消息过大，请减少附件、压缩图片或拆分提交。（单次整体需小于 30MB）',
        ...
        openClawConfigApplyOverdue: 'Caisra 正在等待活动任务结束后应用配置。请完成或停止活动任务，然后重试。',
        ...
        coworkErrorToolLoopBlocked: '检测到 AI 在重复执行同一个工具调用且没有新的进展（通常是在等待一个耗时较长的后台任务），本轮已被安全停止。后台任务可能仍在运行，可以继续发消息让 AI 接着处理。',
        ...
        coworkErrorModelStreamEmptySseData: '模型流式响应格式异常：模型服务返回了空的 SSE data 帧。请稍后重试，或检查当前模型代理配置。',
        coworkErrorModelStreamOnlyEmptySseData: '模型流式响应一直为空：模型服务连续返回空的 SSE data 帧。请稍后重试，或检查当前模型代理配置。',
        ...
        enterpriseAccountContextMismatchMessage: '当前团队身份与登录凭证不一致。相关图片和视频任务已停止轮询，请重新登录或重新选择团队身份后再试。',
        ...
        dataMigrationBackupBlockedByActiveWorkloads: '当前有正在运行的 Agent 或定时任务，请停止或等待任务完成后再备份。',
        ...
        imTelegramOpenClawHint: 'Telegram 由 Caisra 引擎运行，Bot 将在引擎启动后自动连接。',
        // Discord
        ...
        imDiscordOpenClawHint: 'Discord 由 Caisra 引擎运行，Bot 将在引擎启动后自动连接。',
        ...
        imFeishuOpenClawHint: '飞书由 Caisra 引擎运行，Bot 将在引擎启动后自动连接。',
        ...
        imDingtalkOpenClawHint: '钉钉由 Caisra 引擎运行，Bot 将在引擎启动后自动连接。',
        ...
        imWecomOpenClawHint: '企业微信由 Caisra 引擎运行，Bot 将在引擎启动后自动连接。',
        ...
        imWeixinOpenClawHint: '微信由 Caisra 引擎运行，Bot 将在引擎启动后自动连接。',
    ...
    en: {
        ...
        taskOutputTruncated: '[Output incomplete] The model reached the output limit for this response. The partial result was preserved, but the task is not confirmed complete. Continue the conversation to resume.',
        // Feishu bot install
        ...
        coworkErrorLobsterAILoginExpired: 'Your login session has expired. Sign in again to keep using your account\'s models.',
        ...
        coworkErrorQuotaExhausted: 'This month\'s allowance on your account is used up. It resets at the start of next month.\n\nYou can keep working now by using your own provider key — Settings → Models.',
        coworkErrorFreeQuotaExhausted: 'This month\'s allowance on your account is used up. It resets at the start of next month.\n\nYou can keep working now by using your own provider key — Settings → Models.',
        ...
        coworkErrorMessageTooLarge: 'This message is too large. Reduce attachments, compress images, or split it up. (Keep each message under about 30 MB.)',
        ...
        openClawConfigApplyOverdue: 'Caisra is waiting for active tasks to finish before applying configuration. Complete or stop the active tasks, then try again.',
        ...
        coworkErrorModelOverloaded: 'The model service is temporarily busy or at capacity. Please try again later.',
        coworkErrorContentFiltered: 'Content did not pass the safety review. Please modify and try again.',
        coworkErrorToolLoopBlocked: 'This turn was stopped safely because the AI kept repeating the same tool call with no new progress (usually while waiting on a slow background task). The background task may still be running — send another message to continue.',
        ...
        serverModelRuntimeProfileUnsupported: 'This package model task profile is not supported by the current version.',
        serverModelToolCallingUnavailable: 'Agent tool calling is not enabled for this model yet.',
        serverModelAgenticNotReady: 'This model is still undergoing agent capability validation. Please try again later.',
        coworkErrorModelStreamEmptySseData: 'Model stream format error: the model service returned an empty SSE data frame. Please retry later or check the current model proxy configuration.',
        coworkErrorModelStreamOnlyEmptySseData: 'Model stream stayed empty: the model service kept returning empty SSE data frames. Please retry later or check the current model proxy configuration.',
        ...
        enterpriseAccountContextMismatchMessage: 'Your team identity no longer matches the login credentials. Related image and video polling has stopped. Sign in again or choose a team identity before retrying.',
        ...
        dataMigrationBackupBlockedByActiveWorkloads: 'An agent or scheduled task is still running. Stop it or wait for it to finish before backing up.',
        dataMigrationRestoreProgressTitle: 'Importing Caisra data',
        dataMigrationRestoreProgressDesc: 'Restoring the backup and validating data. Caisra will restart automatically when finished.',
        dataMigrationRestoreProgressWarning: 'Do not close the app or restart the computer, or the migration may be interrupted.',
        // ── IM connectivity test messages ───────────────────────────────────
        // Common
        ...
        // QQ
        imQqOpenClawHint: 'QQ runs on the Caisra engine. The bot will connect automatically when the engine starts.',
        ...
        imTelegramOpenClawHint: 'Telegram runs on the Caisra engine. The bot will connect automatically when the engine starts.',
        // Discord
        ...
        imDiscordOpenClawHint: 'Discord runs on the Caisra engine. The bot will connect automatically when the engine starts.',
        ...
        imFeishuOpenClawHint: 'Feishu runs on the Caisra engine. The bot will connect automatically when the engine starts.',
        ...
        imDingtalkOpenClawHint: 'DingTalk runs on the Caisra engine. The bot will connect automatically when the engine starts.',
        ...
        imWecomOpenClawHint: 'WeCom runs on the Caisra engine. The bot will connect automatically when the engine starts.',
        imWecomConfigReadyOpenClaw: 'WeCom configuration is ready (Bot ID: {botId}), running on the Caisra engine.',
        // Weixin
        ...
        imWeixinOpenClawHint: 'WeChat runs on the Caisra engine. The bot will connect automatically when the engine starts.',
        ...
        imNimOpenClawHint: 'NIM runs on the Caisra engine. The bot will connect automatically when the engine starts.',
        ...
        imPopoOpenClawHint: 'POPO runs on the Caisra engine. The bot will connect automatically when the engine starts.',
        ...
        nodeDeploymentAccessStatusApplyFailed: 'The service was deployed, but its access settings could not be updated: {message}',
