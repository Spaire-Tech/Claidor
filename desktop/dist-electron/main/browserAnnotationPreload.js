...
let revision = 0;
let cleanup = null;
let activeCommandListener = null;
const cleanText = (value, max = browserAnnotations_1.BrowserAnnotationLimit.MaxExcerptLength) => (value || '').replace(/\s+/g, ' ').trim().slice(0, max);
const ELEMENT_STYLE_DEFINITIONS = [
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.Color, cssProperty: 'color' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.BackgroundColor, cssProperty: 'background-color' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.FontSize, cssProperty: 'font-size' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.FontFamily, cssProperty: 'font-family' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.FontWeight, cssProperty: 'font-weight' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.BorderRadius, cssProperty: 'border-radius' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.BorderColor, cssProperty: 'border-color' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.BorderWidth, cssProperty: 'border-width' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.PaddingTop, cssProperty: 'padding-top' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.PaddingRight, cssProperty: 'padding-right' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.PaddingBottom, cssProperty: 'padding-bottom' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.PaddingLeft, cssProperty: 'padding-left' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.MarginTop, cssProperty: 'margin-top' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.MarginRight, cssProperty: 'margin-right' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.MarginBottom, cssProperty: 'margin-bottom' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.MarginLeft, cssProperty: 'margin-left' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.Width, cssProperty: 'width' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.Height, cssProperty: 'height' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.Opacity, cssProperty: 'opacity' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.FlexDirection, cssProperty: 'flex-direction' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.JustifyContent, cssProperty: 'justify-content' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.AlignItems, cssProperty: 'align-items' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.Gap, cssProperty: 'gap' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.RowGap, cssProperty: 'row-gap' },
    { key: browserAnnotations_1.BrowserAnnotationElementStyleProperty.ColumnGap, cssProperty: 'column-gap' },
];
const cloneElementEdit = (edit) => ({
...
function selectorFor(element) {
    if (element.id)
        return `#${CSS.escape(element.id)}`.slice(0, browserAnnotations_1.BrowserAnnotationLimit.MaxSelectorLength);
    const parts = [];
    let current = element;
    ...
    return parts.join(' > ').slice(0, browserAnnotations_1.BrowserAnnotationLimit.MaxSelectorLength);
}
...
function baseAnchor(rect, element) {
    return {
        pageUrl: location.href.slice(0, browserAnnotations_1.BrowserAnnotationLimit.MaxUrlLength),
        pageTitle: document.title.slice(0, browserAnnotations_1.BrowserAnnotationLimit.MaxTitleLength),
        framePath: [],
        rect,
        documentRect: {
            ...rect,
...
function elementAnchor(element) {
    ...
    return {
        ...
        kind: browserAnnotations_1.BrowserAnnotationAnchorKind.Element,
        tagName: element.tagName.toLowerCase(),
        color: computed.color,
        fontFamily: computed.fontFamily,
    };
}
function readElementEdit(element) {
    const computed = getComputedStyle(element);
    const opacity = Number.parseFloat(computed.opacity);
    const original = {
        text: (element.textContent || '').slice(0, browserAnnotations_1.BrowserAnnotationLimit.MaxCommentLength),
    };
    const originalValues = original;
    const originalInlineStyles = {};
    for (const definition of ELEMENT_STYLE_DEFINITIONS) {
        originalValues[definition.key] = definition.key === browserAnnotations_1.BrowserAnnotationElementStyleProperty.Opacity
...
function setOrRestoreStyle(element, property, current, original, originalInlineValue, originalPriority) {
    if (current === original) {
...
function applyElementEdit(element, edit) {
    if (edit.canEditText) {
    ...
    for (const definition of ELEMENT_STYLE_DEFINITIONS) {
        ...
        const legacyInlineStyle = definition.key === browserAnnotations_1.BrowserAnnotationElementStyleProperty.Color
            ? { value: edit.originalInlineStyle.color, priority: edit.originalInlineStyle.colorPriority }
            : definition.key === browserAnnotations_1.BrowserAnnotationElementStyleProperty.BackgroundColor
                ? {
                    value: edit.originalInlineStyle.backgroundColor,
                    priority: edit.originalInlineStyle.backgroundColorPriority,
                }
                : definition.key === browserAnnotations_1.BrowserAnnotationElementStyleProperty.Opacity
                    ? { value: edit.originalInlineStyle.opacity, priority: edit.originalInlineStyle.opacityPriority }
                    : definition.key === browserAnnotations_1.BrowserAnnotationElementStyleProperty.FontFamily
                        ? {
                            value: edit.originalInlineStyle.fontFamily,
                            priority: edit.originalInlineStyle.fontFamilyPriority,
                        }
                        : undefined;
        setOrRestoreStyle(element, definition.cssProperty, edit.current[definition.key], edit.original[definition.key], inlineStyle?.value ?? legacyInlineStyle?.value, inlineStyle?.priority ?? legacyInlineStyle?.priority);
    }
}
function restoreElementEdit(element, edit) {
    applyElementEdit(element, { ...edit, current: { ...edit.original } });
}
function sameElementChanges(first, second) {
    if (!first && !second)
        return true;
    if (!first || !second)
        return false;
...
function send(type, payload = {}) {
    if (!activeEnvelope)
        return;
    electron_1.ipcRenderer.sendToHost(browserAnnotations_1.BrowserAnnotationGuestChannel.Event, {
        ...activeEnvelope,
        type,
        revision,
        ...payload,
...
function start(envelope) {
    if (activeCommandListener) {
        electron_1.ipcRenderer.removeListener(browserAnnotations_1.BrowserAnnotationGuestChannel.Command, activeCommandListener);
  ...
  `;
    ...
    textarea.rows = 1;
    textarea.maxLength = browserAnnotations_1.BrowserAnnotationLimit.MaxCommentLength;
    ...
    textInput.type = 'text';
    textInput.maxLength = browserAnnotations_1.BrowserAnnotationLimit.MaxCommentLength;
    ...
    const createNumberInput = (min, max, step = 1) => {
    ...
    const appendProperty = (label, control) => {
    ...
    const createUnitControl = (input, unit = 'px') => {
    ...
    const createMultiControl = (inputs, className = '') => {
        const control = document.createElement('div');
    ...
    paddingInputs.forEach((input, index) => input.setAttribute('aria-label', `${labels.padding || 'Padding'} ${insetNames[index]}`));
    marginInputs.forEach((input, index) => input.setAttribute('aria-label', `${labels.margin || 'Margin'} ${insetNames[index]}`));
    ...
    let draftElementEdit = null;
    let initialElementEdit = null;
    let regionStart = null;
    let captureMode = false;
    let markerRenderFrame = null;
    let editorExpanded = false;
    let ignoreOutsideClickUntil = 0;
    const resolveAnchorRect = (anchor, capture) => (0, browserAnnotations_1.resolveBrowserAnnotationViewportRect)(anchor, capture, {
        x: window.scrollX,
        y: window.scrollY,
    });
    const isMarkerVisible = (left, top) => (left + 26 > 0
        && top + 26 > 0
    ...
    const positionPendingMarker = (rect) => {
        if (editingId) {
    ...
    const refreshEditingGeometry = () => {
        ...
        if (pendingElement?.isConnected)
            pendingAnchor = elementAnchor(pendingElement);
    ...
    const updateSaveDisabled = () => {
        const disabled = !(0, browserAnnotations_1.hasBrowserAnnotationContent)(textarea.value, draftElementEdit || undefined);
    ...
    const fillNumberInput = (input, value) => {
        const number = Number.parseFloat(value || '');
        input.value = Number.isFinite(number) ? String(Number(number.toFixed(3))) : '';
    };
    const fillSelect = (select, currentValue, options) => {
        const values = [
    ...
    const fillPropertyControls = (anchor) => {
        tagName.textContent = anchor.kind === browserAnnotations_1.BrowserAnnotationAnchorKind.Element ? anchor.tagName : anchor.kind;
    ...
    const rollbackPendingEdit = () => {
        if (!pendingElement || !initialElementEdit)
            return;
    ...
    const openEditor = (anchor, id, targetElement, suppressImmediateOutsideClick = false) => {
        const annotation = id ? annotations.find(item => item.id === id) : undefined;
        pendingAnchor = anchor;
        editingId = id || null;
        pendingElement = targetElement
            || (anchor.kind === browserAnnotations_1.BrowserAnnotationAnchorKind.Element ? resolveElement(anchor.selector) : null);
        if (pendingElement && anchor.kind === browserAnnotations_1.BrowserAnnotationAnchorKind.Element) {
    ...
    const renderMarkers = () => {
        ...
        annotations.slice().sort((a, b) => a.order - b.order).forEach((annotation, index) => {
            ...
            const markerPoint = (0, browserAnnotations_1.resolveBrowserAnnotationMarkerViewportPoint)(annotation.anchor, annotation.capture, { x: window.scrollX, y: window.scrollY });
            ...
            marker.addEventListener('click', event => {
                ...
                if (pendingAnchor)
                    closeEditor();
                openEditor({ ...annotation.anchor, rect }, annotation.id, annotation.anchor.kind === browserAnnotations_1.BrowserAnnotationAnchorKind.Element
    ...
    const emitChanged = () => {
        revision += 1;
        send(browserAnnotations_1.BrowserAnnotationGuestEventType.Changed, { annotations });
        renderMarkers();
    };
    const applyAnnotationEdits = (nextAnnotations) => {
        for (const current of annotations) {
            ...
            if (current.elementEdit && (!next || !sameElementChanges(current.elementEdit, next.elementEdit))) {
                ...
                if (element)
                    restoreElementEdit(element, current.elementEdit);
            }
        }
        ...
        for (const annotation of annotations) {
            ...
            if (element)
                applyElementEdit(element, annotation.elementEdit);
        }
    };
    const updateDraft = (patch) => {
        if (!draftElementEdit || !pendingElement)
            return;
    ...
    const selectedTextAnchor = () => {
        ...
        if (!selection || selection.isCollapsed || !cleanText(selection.toString()))
            return null;
        ...
        return {
            ...
            kind: browserAnnotations_1.BrowserAnnotationAnchorKind.Text,
    ...
    const onMouseMove = (event) => {
        if (captureMode || pendingAnchor || regionStart)
            return;
    ...
    const onMouseDown = (event) => {
        if (!event.shiftKey || pendingAnchor || captureMode)
            return;
        event.preventDefault();
        event.stopPropagation();
    ...
    const onMouseUp = (event) => {
        ...
        if (regionStart) {
            ...
            if (rect.width >= 8 && rect.height >= 8) {
                openEditor({
                    ...baseAnchor(rect),
                    kind: browserAnnotations_1.BrowserAnnotationAnchorKind.Region,
        ...
        if (anchor)
            openEditor(anchor, undefined, null, true);
    };
    const onClick = (event) => {
        if (captureMode || event.shiftKey)
            return;
        if (pendingAnchor) {
        ...
        const existing = annotations.find(item => (item.anchor.kind === browserAnnotations_1.BrowserAnnotationAnchorKind.Element
    ...
    const saveAnnotation = () => {
        const comment = textarea.value.trim().slice(0, browserAnnotations_1.BrowserAnnotationLimit.MaxCommentLength);
        const elementEdit = draftElementEdit
            && (0, browserAnnotations_1.getBrowserAnnotationElementChanges)(draftElementEdit).length > 0
            ? cloneElementEdit(draftElementEdit)
            : undefined;
        if (!pendingAnchor || !(0, browserAnnotations_1.hasBrowserAnnotationContent)(comment, elementEdit))
            return;
        const now = Date.now();
            ...
            annotations = annotations.map(item => {
                ...
                return {
                    ...
                    screenshot: visualChange
                        ? {
                            status: browserAnnotations_1.BrowserAnnotationScreenshotStatus.Capturing,
        ...
        else if (annotations.length < browserAnnotations_1.BrowserAnnotationLimit.MaxAnnotations) {
    ...
    const onKeyDown = (event) => {
        if (event.key === 'Escape') {
            if (pendingAnchor)
                ...
                send(browserAnnotations_1.BrowserAnnotationGuestEventType.CloseRequested);
        }
        if ((event.metaKey || event.ctrlKey) && event.key === 'Enter' && pendingAnchor)
            saveAnnotation();
    };
    ...
    colorInput.addEventListener('change', () => {
        if (CSS.supports('color', colorInput.value)) {
            ...
            colorSwatch.value = colorToHex(getComputedStyle(pendingElement).color);
        }
    });
    ...
    backgroundInput.addEventListener('change', () => {
        if (CSS.supports('background-color', backgroundInput.value)) {
            ...
            backgroundSwatch.value = colorToHex(getComputedStyle(pendingElement).backgroundColor);
        }
    });
    ...
    borderColorInput.addEventListener('change', () => {
        if (CSS.supports('border-color', borderColorInput.value)) {
            ...
            borderColorSwatch.value = colorToHex(getComputedStyle(pendingElement).borderColor);
        }
    });
    ...
    opacityInput.addEventListener('input', () => {
        ...
        if (Number.isFinite(opacity))
            updateDraft({ opacity: Math.max(0, Math.min(1, opacity)) });
    });
    const bindPixelInput = (input, property, min, max) => input.addEventListener('input', () => {
    ...
    bindPixelInput(fontSizeInput, browserAnnotations_1.BrowserAnnotationElementStyleProperty.FontSize, 0, 512);
    bindPixelInput(borderRadiusInput, browserAnnotations_1.BrowserAnnotationElementStyleProperty.BorderRadius, 0, 9_999);
    bindPixelInput(borderWidthInput, browserAnnotations_1.BrowserAnnotationElementStyleProperty.BorderWidth, 0, 128);
    bindPixelInput(widthInput, browserAnnotations_1.BrowserAnnotationElementStyleProperty.Width, 0, 100_000);
    bindPixelInput(heightInput, browserAnnotations_1.BrowserAnnotationElementStyleProperty.Height, 0, 100_000);
    const paddingProperties = [
        browserAnnotations_1.BrowserAnnotationElementStyleProperty.PaddingTop,
        browserAnnotations_1.BrowserAnnotationElementStyleProperty.PaddingRight,
        browserAnnotations_1.BrowserAnnotationElementStyleProperty.PaddingBottom,
        browserAnnotations_1.BrowserAnnotationElementStyleProperty.PaddingLeft,
    ];
    paddingInputs.forEach((input, index) => {
        bindPixelInput(input, paddingProperties[index], 0, 10_000);
    });
    const marginProperties = [
        browserAnnotations_1.BrowserAnnotationElementStyleProperty.MarginTop,
        browserAnnotations_1.BrowserAnnotationElementStyleProperty.MarginRight,
        browserAnnotations_1.BrowserAnnotationElementStyleProperty.MarginBottom,
        browserAnnotations_1.BrowserAnnotationElementStyleProperty.MarginLeft,
    ];
    ...
    bindPixelInput(gapInputs[0], browserAnnotations_1.BrowserAnnotationElementStyleProperty.RowGap, 0, 10_000);
    bindPixelInput(gapInputs[1], browserAnnotations_1.BrowserAnnotationElementStyleProperty.ColumnGap, 0, 10_000);
    ...
    const commandListener = (_event, command) => {
        if (!activeEnvelope || command.protocolVersion !== browserAnnotations_1.BrowserAnnotationProtocolVersion)
            return;
        if (command.browserTabId !== activeEnvelope.browserTabId
            ...
            return;
        if (command.type === browserAnnotations_1.BrowserAnnotationGuestCommandType.Sync) {
            applyAnnotationEdits(command.annotations ? [...command.annotations] : []);
            revision = Math.max(revision, command.revision);
            renderMarkers();
        }
        else if (command.type === browserAnnotations_1.BrowserAnnotationGuestCommandType.Focus) {
        ...
        else if (command.type === browserAnnotations_1.BrowserAnnotationGuestCommandType.PrepareCapture) {
            const annotation = annotations.find(item => item.id === command.annotationId);
            ...
            if (resolvedRect)
                showHighlight(resolvedRect);
            requestAnimationFrame(() => requestAnimationFrame(() => {
                if (!annotation || !resolvedRect)
                    return;
                send(browserAnnotations_1.BrowserAnnotationGuestEventType.CaptureReady, {
        ...
        else if (command.type === browserAnnotations_1.BrowserAnnotationGuestCommandType.ResumeAfterCapture) {
            captureMode = false;
            highlight.style.display = 'none';
        ...
        else if (command.type === browserAnnotations_1.BrowserAnnotationGuestCommandType.Clear) {
            closeEditor();
            for (const annotation of annotations) {
                const element = annotation.elementEdit ? resolveElement(annotation.anchor.selector) : null;
                if (annotation.elementEdit && element)
                    restoreElementEdit(element, annotation.elementEdit);
            }
            annotations = [];
            emitChanged();
        }
        else if (command.type === browserAnnotations_1.BrowserAnnotationGuestCommandType.Stop) {
            electron_1.ipcRenderer.removeListener(browserAnnotations_1.BrowserAnnotationGuestChannel.Command, commandListener);
            activeCommandListener = null;
            cleanup?.();
            cleanup = null;
            activeEnvelope = null;
    ...
    electron_1.ipcRenderer.on(browserAnnotations_1.BrowserAnnotationGuestChannel.Command, commandListener);
    send(browserAnnotations_1.BrowserAnnotationGuestEventType.Ready);
}
electron_1.ipcRenderer.on(browserAnnotations_1.BrowserAnnotationGuestChannel.Command, (_event, envelope) => {
    if (envelope?.protocolVersion === browserAnnotations_1.BrowserAnnotationProtocolVersion
        && envelope.type === browserAnnotations_1.BrowserAnnotationGuestCommandType.Start)
        start(envelope);
});
//# sourceMappingURL=browserAnnotationPreload.js.map