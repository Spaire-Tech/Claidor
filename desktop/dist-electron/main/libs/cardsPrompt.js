...
exports.buildManagedCardsPrompt = buildManagedCardsPrompt;
const library_1 = require("../../shared/cards/library");
const prompt_generated_1 = require("../../shared/cards/prompt.generated");
/**
...
const MARKERS = { preamble: '@@CAISRA_PREAMBLE@@', rules: '@@CAISRA_RULES@@' };
...
const RULES = [
    `The fence is \`\`\`${library_1.CARD_FENCE} on its own line, the program, then \`\`\` on its own line. One card block per reply; a reply may also carry one artifact block after it, which is a different thing (see Artifacts).`,
...
function buildManagedCardsPrompt() {
    for (const marker of Object.values(MARKERS)) {
        if (!prompt_generated_1.CARD_PROMPT.includes(marker))
            throw new Error(`The generated card prompt lost its ${marker} marker; regenerate it.`);
    }
    const check = prompt_generated_1.CARD_PROMPT.match(FINAL_VERIFICATION);
    if (!check)
        throw new Error('OpenUI moved its Final Verification checklist; update this file.');
    const prompt = prompt_generated_1.CARD_PROMPT
...
//# sourceMappingURL=cardsPrompt.js.map