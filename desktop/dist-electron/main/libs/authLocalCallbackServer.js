...
exports.appendLoginParams = appendLoginParams;
exports.appendCallbackReturnTo = appendCallbackReturnTo;
exports.startAuthLocalCallback = startAuthLocalCallback;
...
const renderCallbackHtml = (success, message) => {
    return renderCallbackHtmlWithRedirect(success, message, null);
};
const renderCallbackHtmlWithRedirect = (success, message, redirectUrl) => {
    ...
    const redirectHint = redirectUrl
        ? '<p class="hint">页面将自动返回 Caisra 登录页。</p>'
        : '';
    const redirectAction = redirectUrl
        ? `<a class="action" href="${escapeHtml(redirectUrl)}">立即返回</a>`
        : '';
    return `<!doctype html>
<html lang="zh-CN"><head><meta charset="utf-8"><title>Caisra 登录</title>
<style>
...
const sendHtml = (res, statusCode, success, message) => {
...
const sendHtmlWithRedirect = (res, statusCode, success, message, redirectUrl) => {
...
function appendLoginParams(baseUrl, params) {
...
function appendCallbackReturnTo(redirectUri, returnTo) {
...
function resolveSafeReturnTo(value) {
    ...
    try {
        ...
        if (!isYoudaoHost && !isLoopbackHost)
            return null;
        return url.toString();
    }
    catch {
...
async function createAuthLocalCallback(options) {
    const state = crypto_1.default.randomBytes(24).toString('base64url');
    ...
    server.on('request', (req, res) => {
        if (req.method !== 'GET') {
            sendHtml(res, 405, false, '登录回调请求方法不支持，请返回 Caisra 后重试。');
            return;
        }
        ...
        if (!code) {
            ...
            sendHtml(res, 400, false, '登录回调缺少授权码，请返回 Caisra 后重试。');
            void callback.close();
            return;
        }
        if (returnedState !== state) {
            console.warn('[AuthLocalCallback] callback was rejected because the state did not match');
            sendHtml(res, 400, false, '登录状态校验失败，请返回 Caisra 后重试。');
            void callback.close();
            return;
        }
        try {
            ...
            options.onCode(code);
            sendHtmlWithRedirect(res, 200, true, '登录已完成，正在返回 Caisra 登录页。', returnTo);
        }
        catch (error) {
            console.error('[AuthLocalCallback] failed to deliver auth code:', error);
            sendHtml(res, 500, false, '登录回调处理失败，请返回 Caisra 后重试。');
        }
        finally {
            void callback.close();
        }
...
async function startAuthLocalCallback(options) {
...
//# sourceMappingURL=authLocalCallbackServer.js.map