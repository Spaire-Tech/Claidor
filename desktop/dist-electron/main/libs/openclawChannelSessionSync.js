...
exports.OpenClawChannelSessionSync = exports.DEFAULT_MANAGED_AGENT_ID = void 0;
exports.buildManagedSessionKey = buildManagedSessionKey;
exports.parseManagedSessionKey = parseManagedSessionKey;
exports.isManagedSessionKey = isManagedSessionKey;
exports.parseChannelSessionKey = parseChannelSessionKey;
exports.extractAgentIdFromKey = extractAgentIdFromKey;
exports.extractAccountIdFromKey = extractAccountIdFromKey;
exports.resolveAgentBinding = resolveAgentBinding;
exports.isCronSessionKey = isCronSessionKey;
exports.buildChannelDisplayName = buildChannelDisplayName;
...
exports.DEFAULT_MANAGED_AGENT_ID = 'main';
function buildManagedSessionKey(sessionId, agentId = exports.DEFAULT_MANAGED_AGENT_ID) {
    const normalizedSessionId = sessionId.trim();
    const normalizedAgentId = agentId.trim() || exports.DEFAULT_MANAGED_AGENT_ID;
    return `agent:${normalizedAgentId}:lobsterai:${normalizedSessionId}`;
}
function parseManagedSessionKey(sessionKey) {
...
function isManagedSessionKey(sessionKey) {
 ...
 */
function parseChannelSessionKey(sessionKey) {
    ...
    if (sessionKey.startsWith('agent:')) {
        ...
        if (jsonIdx > 0) {
            ...
            try {
                ...
                if (ctx && typeof ctx.channel === 'string') {
                    const platform = platform_1.PlatformRegistry.platformOfChannel(ctx.channel);
                    if (platform) {
        ...
        if (parts.length >= 4) {
            let platform = platform_1.PlatformRegistry.platformOfChannel(parts[2]);
            if (platform) {
            ...
            if (!platform && parts.length >= 5) {
                platform = platform_1.PlatformRegistry.platformOfChannel(parts[3]);
                if (platform) {
    ...
    const platform = platform_1.PlatformRegistry.platformOfChannel(channelName);
    if (!platform)
        return null;
 ...
 */
function extractAgentIdFromKey(sessionKey) {
    if (!sessionKey.startsWith('agent:'))
        return null;
 ...
 */
function extractAccountIdFromKey(sessionKey) {
    if (!sessionKey.startsWith('agent:'))
        return null;
    // Try JSON SessionContext format first
    const jsonIdx = sessionKey.indexOf(':{');
...
const MULTI_INSTANCE_PLATFORMS = new Set(['dingtalk', 'feishu', 'qq', 'email', 'nim', 'wecom', 'telegram', 'discord', 'popo']);
/**
 ...
 */
function resolveAgentBinding(bindings, platform, accountId) {
    if (!bindings)
        return 'main';
...
function isCronSessionKey(sessionKey) {
    return (0, openclawCronSessionKey_1.isOpenClawCronSessionKey)(sessionKey);
}
function getChannelTitlePrefix(platform) {
    const i18nMap = {
        feishu: (0, i18n_1.t)('channelPrefixFeishu'),
        dingtalk: (0, i18n_1.t)('channelPrefixDingtalk'),
        wecom: (0, i18n_1.t)('channelPrefixWecom'),
        'wecom-openclaw-plugin': (0, i18n_1.t)('channelPrefixWecom'),
        nim: (0, i18n_1.t)('channelPrefixNim'),
        weixin: (0, i18n_1.t)('channelPrefixWeixin'),
        'netease-bee': (0, i18n_1.t)('channelPrefixNeteaseBee'),
    };
    const staticMap = {
        telegram: 'TG',
        discord: 'Discord',
        qq: 'QQ',
        popo: 'POPO',
        email: (0, i18n_1.t)('channelPrefixEmail'),
    };
    const label = i18nMap[platform] ?? staticMap[platform] ?? platform;
    return `[${label}]`;
}
const PEER_KIND_LABELS = {
    direct: '',
    group: 'group:',
    channel: 'ch:',
};
 ...
 */
function buildChannelDisplayName(conversationId) {
...
class OpenClawChannelSessionSync {
    coworkStore;
    imStore;
    getDefaultCwd;
    resolveJobName;
     ...
     */
    agentChangedSessionIds = new Set();
    constructor(deps) {
    ...
    updateLocalSessionCwdIfNeeded(session, agentId) {
        const resolvedCwd = this.getDefaultCwd(agentId).trim();
        if (!resolvedCwd || session.cwd === resolvedCwd) {
            return;
        }
        const updateSession = this.coworkStore.updateSession;
        if (!updateSession) {
            return;
        }
        updateSession.call(this.coworkStore, session.id, { cwd: resolvedCwd }, { touchUpdatedAt: false });
        console.debug(`[ChannelSessionSync] corrected local session ${session.id} cwd for agent ${agentId} to ${resolvedCwd}`);
    }
    resolveAgentIdForSessionKey(sessionKey, platform) {
    ...
    getMappingForSessionKey(sessionKey, parsed, agentId) {
    ...
    createChannelSession(parsed, agentId) {
        ...
        console.log(`[ChannelSessionSync] Created session for ${parsed.platform} conversation ${parsed.conversationId}: ${session.id}`);
        return session;
    }
    /**
     ...
     */
    isCurrentBindingKey(sessionKey) {
        const parsed = parseChannelSessionKey(sessionKey);
        if (!parsed)
            return true; // Not a channel key — let other logic handle it
        const keyAgentId = extractAgentIdFromKey(sessionKey);
        if (!keyAgentId)
            return true; // Legacy key without agentId — allow
        const accountId = extractAccountIdFromKey(sessionKey);
        if (!accountId)
            return true; // Account-less group keys are already scoped by agentId.
        const imSettings = this.imStore.getIMSettings();
     ...
     */
    isAgentChangedSession(sessionId) {
     ...
     */
    resolveOrCreateSession(sessionKey) {
        ...
        console.log('[ChannelSessionSync] existing mapping:', existingMapping
        ...
        if (existingMapping) {
            ...
            if (session) {
                ...
                if (existingMapping.agentId !== currentAgentId) {
                    ...
                    this.imStore.updateSessionMappingTarget(existingMapping.imConversationId, parsed.platform, newSession.id, currentAgentId, sessionKey, existingMapping.agentId);
                ...
                if (existingMapping.imConversationId === parsed.conversationId &&
                    existingMapping.openClawSessionKey !== sessionKey) {
                    this.imStore.updateSessionOpenClawSessionKey(parsed.conversationId, parsed.platform, sessionKey, existingMapping.agentId);
                }
                this.imStore.updateSessionLastActive(existingMapping.imConversationId, parsed.platform, existingMapping.agentId);
            ...
            this.imStore.deleteSessionMapping(existingMapping.imConversationId, parsed.platform, existingMapping.agentId);
        }
    ...
    resolveSession(sessionKey) {
        ...
        const cronKey = (0, openclawCronSessionKey_1.parseOpenClawCronSessionKey)(sessionKey);
     ...
     */
    resolveConversationByDeliveryTarget(channel, to, accountId) {
        ...
        for (const mapping of this.imStore.listSessionMappings(platform)) {
            ...
            if (!sessionKey)
                continue;
            if (!this.coworkStore.getSession(mapping.coworkSessionId))
                continue;
     ...
     */
    resolveOrCreateConversationForDeliveryMirror(channel, to, accountId, agentId) {
        const platform = platform_1.PlatformRegistry.platformOfChannel(channel);
        if (!platform)
            return null;
        const peerId = (0, platform_1.parseImConversationId)(to).peerId.trim();
        if (!peerId)
            return null;
        ...
        if (platform === 'feishu' && normalizedAccountId && FEISHU_GROUP_CHAT_ID_RE.test(peerId)) {
            ...
            const mirrorAgentId = agentId?.trim()
                || resolveAgentBinding(imSettings?.platformAgentBindings, platform, normalizedAccountId);
            const sessionKey = [
                'agent',
                mirrorAgentId,
                platform_1.PlatformRegistry.channelOf(platform),
                normalizedAccountId,
                'direct',
                peerId,
            ].join(':');
    ...
    isChannelSessionKey(sessionKey) {
        if (!sessionKey || isManagedSessionKey(sessionKey))
            return false;
     ...
     */
    resolveOrCreateMainAgentSession(sessionKey) {
        if (isManagedSessionKey(sessionKey))
            return null;
        if (!MAIN_AGENT_SESSION_RE.test(sessionKey))
            return null;
        ...
        const session = this.coworkStore.createSession('[Caisra]', cwd, '', 'local');
     ...
     */
    resolveOrCreateCronSession(sessionKey) {
        const cronKey = (0, openclawCronSessionKey_1.parseOpenClawCronSessionKey)(sessionKey);
        if (!cronKey)
            return null;
        ...
        if (delivery?.mode === constants_1.DeliveryMode.Announce &&
            delivery.channel &&
            platform_1.PlatformRegistry.isIMChannel(delivery.channel)) {
            return null;
        }
        ...
        const agentId = cronKey.agentId || exports.DEFAULT_MANAGED_AGENT_ID;
        ...
        if (persistedSessionId) {
            ...
            if (persistedSession) {
                ...
                console.debug('[ChannelSessionSync] reusing persisted cron session:', { jobId, agentId, sessionId: persistedSessionId });
                return persistedSessionId;
            }
            console.warn('[ChannelSessionSync] ignored stale persisted cron session lookup:', { jobId, agentId, sessionId: persistedSessionId });
        }
        ...
        const cronLabel = (0, i18n_1.t)('cronSessionPrefix');
        const title = jobName
            ? `[${cronLabel}] ${jobName}`
        ...
        const session = this.coworkStore.createSession(title, cwd, '', 'local', [], agentId, '', { scheduledTaskId: jobId });
...
exports.OpenClawChannelSessionSync = OpenClawChannelSessionSync;
//# sourceMappingURL=openclawChannelSessionSync.js.map