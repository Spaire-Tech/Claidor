...
exports.reconcileOpenClawThinkingBlocks = void 0;
...
const selectCanonicalBlocks = (historyMessages, toolUseMessageIdByToolCallId, includeUnanchored) => {
    const extractedBlocks = (0, blocks_1.extractCurrentTurnThinkingBlocks)(historyMessages);
    ...
    const lastUnanchoredAssistantOrdinal = unanchoredBlocks.reduce((latest, block) => Math.max(latest, block.assistantOrdinal), -1);
    return [
        ...anchoredBlocks,
        ...(includeUnanchored
 ...
 */
const reconcileOpenClawThinkingBlocks = (options) => {
    const canonicalBlocks = selectCanonicalBlocks(options.historyMessages, options.toolUseMessageIdByToolCallId, options.includeUnanchored);
    ...
    for (const block of canonicalBlocks) {
        ...
        if (!messageId) {
            message = currentTurnMessages.find((candidate) => {
                return candidate.type === 'assistant'
                    ...
                    && candidate.metadata?.[blocks_1.OpenClawThinkingMetadata.Key] === block.key
        ...
        if (!messageId) {
            ...
            const reusableCandidates = currentTurnMessages.filter((candidate, index) => {
                return candidate.type === 'assistant'
                    && candidate.metadata?.isThinking === true
                    && !candidate.metadata?.[blocks_1.OpenClawThinkingMetadata.Key]
        ...
        const metadata = {
            ...
            isStreaming: false,
            isFinal: true,
            [blocks_1.OpenClawThinkingMetadata.Key]: block.key,
            ...(block.anchorToolCallId
                ? { [blocks_1.OpenClawThinkingMetadata.AnchorToolCallId]: block.anchorToolCallId }
                : {}),
        };
        if (messageId) {
            ...
            if (!message
                ...
                || message.metadata?.[blocks_1.OpenClawThinkingMetadata.Key] !== block.key
...
exports.reconcileOpenClawThinkingBlocks = reconcileOpenClawThinkingBlocks;
