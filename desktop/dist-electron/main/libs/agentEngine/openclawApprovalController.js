...
exports.OpenClawApprovalController = void 0;
const openclawApprovalBridge_1 = require("./openclawApprovalBridge");
class OpenClawApprovalController {
    options;
    ...
    respondToPermission(requestId, result) {
        ...
        const decision = (0, openclawApprovalBridge_1.resolveApprovalDecision)(pending, result);
        ...
        const method = (0, openclawApprovalBridge_1.getApprovalResolveMethod)(pending);
    ...
    handleExecApprovalRequested(payload) {
        const approval = (0, openclawApprovalBridge_1.parseExecApprovalRequestedPayload)(payload);
        if (!approval)
            return;
        ...
        if (!sessionId) {
            ...
            console.warn('[EngineRuntime] approval dropped: no session for key, requestId:', requestId, 'sessionKey:', sessionKey, 'kind:', kind);
            return;
        }
        ...
        this.options.emitPermissionRequest(sessionId, (0, openclawApprovalBridge_1.buildExecApprovalPermissionRequest)(requestId, request, command, fileAccess, claudeTool));
    }
    handleExecApprovalResolved(payload) {
        const requestId = (0, openclawApprovalBridge_1.parseApprovalResolvedPayload)(payload);
        if (!requestId)
            return;
        const pending = this.pendingApprovals.get(requestId);
        this.pendingApprovals.delete(requestId);
    ...
    handlePluginApprovalRequested(payload) {
        const approval = (0, openclawApprovalBridge_1.parsePluginApprovalRequestedPayload)(payload);
        if (!approval)
            return;
        ...
        this.options.emitPermissionRequest(sessionId, (0, openclawApprovalBridge_1.buildPluginApprovalPermissionRequest)(requestId, request, allowedDecisions));
    }
    handlePluginApprovalResolved(payload) {
        const requestId = (0, openclawApprovalBridge_1.parseApprovalResolvedPayload)(payload);
        if (!requestId)
            return;
        const pending = this.pendingApprovals.get(requestId);
...
exports.OpenClawApprovalController = OpenClawApprovalController;
//# sourceMappingURL=openclawApprovalController.js.map