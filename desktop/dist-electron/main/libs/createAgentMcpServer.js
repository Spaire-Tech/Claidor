...
exports.resolveCreateAgentMcpStdioLaunch = void 0;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const script_1 = require("../../shared/onboarding/script");
const constants_1 = require("../../shared/staffing/constants");
...
const PROPOSE_TEAM_DESCRIPTION = [
    ...
    'know their job. Give the work type the person chose (one of: ' + script_1.ROLES.map(one => one.label).join(', ') + ')',
    ...
    strongs_1.STRONGS.map(one => `${one.slug} (${one.name})`).join(', ') + '.',
].join('\n');
...
const tools = [{
  name: '${roster_1.PROPOSE_TEAM_TOOL}',
  ...
  inputSchema: {
    ...
    properties: {
      ...
      picks: {
        type: 'array',
        description: 'Your own ${roster_1.ROSTER_LIMITS.min} or ${roster_1.ROSTER_LIMITS.max} picks, by slug, when the table\'s default is not the right one.',
        items: { type: 'string' },
        minItems: ${roster_1.ROSTER_LIMITS.min},
        maxItems: ${roster_1.ROSTER_LIMITS.max},
      },
    },
    required: ['workType'],
  },
}, {
  name: '${constants_1.CREATE_AGENT_TOOL}',
  ...
  inputSchema: {
    ...
    properties: {
      name: {
        type: 'string',
        description: 'The agent\'s name, e.g. "Projects Manager". Up to ${constants_1.CREATE_AGENT_LIMITS.name} characters.',
      },
      label: {
        type: 'string',
...
async function handleRequest(message) {
  ...
  if (message.method === 'initialize') {
    ...
    result = message.params?.name === '${constants_1.CREATE_AGENT_TOOL}'
      ? await createAgent(message.params?.arguments || {})
      : message.params?.name === '${roster_1.PROPOSE_TEAM_TOOL}'
...
const prepare = (baseDir, options) => {
    ...
    const serverDir = path_1.default.join(baseDir, 'create-agent-mcp');
...
const resolveCreateAgentMcpStdioLaunch = (baseDir, options) => {
...
exports.resolveCreateAgentMcpStdioLaunch = resolveCreateAgentMcpStdioLaunch;
//# sourceMappingURL=createAgentMcpServer.js.map