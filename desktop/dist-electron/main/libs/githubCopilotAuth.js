...
exports.getCopilotToken = getCopilotToken;
exports.getGitHubUser = getGitHubUser;
exports.cancelPolling = cancelPolling;
exports.authenticateWithDeviceFlow = authenticateWithDeviceFlow;
...
async function fetchJson(url, options) {
    const response = await electron_1.session.defaultSession.fetch(url, options);
    if (!response.ok) {
...
async function requestDeviceCode() {
    ...
    const response = await electron_1.session.defaultSession.fetch(DEVICE_CODE_URL, {
        method: 'POST',
        headers: {
 ...
 */
async function pollForAccessToken(deviceCode, interval, expiresIn, onStatusChange) {
    ...
    while (Date.now() < deadline) {
        ...
        try {
            ...
            const response = await electron_1.session.defaultSession.fetch(ACCESS_TOKEN_URL, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/x-www-form-urlencoded',
 ...
 */
async function getCopilotToken(githubAccessToken) {
    const response = await electron_1.session.defaultSession.fetch(COPILOT_TOKEN_URL, {
        method: 'GET',
        headers: {
    ...
    const baseUrl = deriveCopilotApiBaseUrlFromToken(token) ?? exports.DEFAULT_COPILOT_API_BASE_URL;
 ...
 */
async function authenticateWithDeviceFlow(onDeviceCode, onStatusChange) {
    // Step 1: Get device code
    const deviceCodeResponse = await requestDeviceCode();
    onDeviceCode(deviceCodeResponse.user_code, deviceCodeResponse.verification_uri);
    // Step 2: Poll for access token
    const githubAccessToken = await pollForAccessToken(deviceCodeResponse.device_code, deviceCodeResponse.interval, deviceCodeResponse.expires_in, onStatusChange);
