...
exports.TurnRoute = void 0;
exports.routeTurn = routeTurn;
exports.lastTurnUsedTools = lastTurnUsedTools;
exports.TurnRoute = {
...
const ACTION_PATTERN = new RegExp(`(^|[^a-z])(${ACTION_VERBS.map(verb => verb.replace(/ /g, '\\s+')).join('|')})([^a-z]|$)`, 'i');
 ...
 */
function routeTurn(signals) {
    if (signals.inRoom || signals.lastTurnUsedTools || signals.hasAttachments)
        return exports.TurnRoute.Strong;
    const text = signals.text.trim();
    if (!text || text.length > FAST_MAX_CHARS)
        return exports.TurnRoute.Strong;
    if (LINK_PATTERN.test(text) || PATH_PATTERN.test(text))
        return exports.TurnRoute.Strong;
    ...
    return asksAQuestion ? exports.TurnRoute.Fast : exports.TurnRoute.Strong;
}
/**
 * Whether the reply before this message used a tool: any tool use after
 * the last thing the person said.
 */
function lastTurnUsedTools(messages) {
    let sawTool = false;
    for (let i = messages.length - 1; i >= 0; i -= 1) {
        const one = messages[i];
        if (one.type === 'user')
            break;
        if (one.type === 'tool_use' || one.type === 'tool_result')
            sawTool = true;
    }
    return sawTool;
}
//# sourceMappingURL=turnRouting.js.map