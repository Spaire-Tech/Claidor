...
exports.getElectronNodeRuntimePath = getElectronNodeRuntimePath;
exports.ensureElectronNodeShim = ensureElectronNodeShim;
exports.getSkillsRoot = getSkillsRoot;
exports.getNodeToolEnv = getNodeToolEnv;
exports.getEnhancedEnv = getEnhancedEnv;
exports.ensureCoworkTempDir = ensureCoworkTempDir;
exports.probeCoworkModelReadiness = probeCoworkModelReadiness;
...
const coworkLogger_1 = require("./coworkLogger");
const coworkModelApi_1 = require("./coworkModelApi");
...
function appendEnvPath(current, additions) {
    ...
    if (current) {
        for (const entry of current.split(path_1.delimiter)) {
            if (entry) {
                items.add(entry);
            }
        }
    }
    return items.size > 0 ? Array.from(items).join(path_1.delimiter) : current;
}
function hasCommandInEnv(command, env) {
    const whichCmd = process.platform === 'win32' ? 'where' : 'which';
    try {
        const result = (0, child_process_1.spawnSync)(whichCmd, [command], {
            env: { ...env },
            encoding: 'utf-8',
            timeout: 5000,
...
function resolveElectronNodeRuntimePath() {
    ...
    try {
        ...
        const frameworksDir = (0, path_1.join)(process.resourcesPath, '..', 'Frameworks');
        if (!(0, fs_1.existsSync)(frameworksDir)) {
            return process.execPath;
        }
        const helperApps = (0, fs_1.readdirSync)(frameworksDir)
            .filter((entry) => entry.startsWith(`${appName} Helper`) && entry.endsWith('.app'))
            .sort((a, b) => {
            const score = (name) => {
                if (name === `${appName} Helper.app`)
                    return 0;
                if (name === `${appName} Helper (Renderer).app`)
                    return 1;
                if (name === `${appName} Helper (Plugin).app`)
                    return 2;
                if (name === `${appName} Helper (GPU).app`)
                    return 3;
                return 10;
            };
        ...
        for (const helperApp of helperApps) {
            ...
            const helperExePath = (0, path_1.join)(frameworksDir, helperApp, 'Contents', 'MacOS', helperExeName);
            if ((0, fs_1.existsSync)(helperExePath)) {
                (0, coworkLogger_1.coworkLog)('INFO', 'resolveNodeShim', `Using Electron helper runtime for node shim: ${helperExePath}`);
                return helperExePath;
            }
        }
    }
    catch (error) {
        (0, coworkLogger_1.coworkLog)('WARN', 'resolveNodeShim', `Failed to resolve Electron helper runtime: ${error instanceof Error ? error.message : String(error)}`);
    }
    return process.execPath;
}
...
function resolveUserShellPath() {
    ...
    try {
        ...
        for (const probe of pathProbes) {
            try {
                const result = (0, child_process_1.execSync)(probe, {
                    encoding: 'utf-8',
                    timeout: 5000,
                    env: { ...process.env },
                });
...
function readWindowsRegistryPathValue(registryKey) {
    try {
        const output = (0, child_process_1.execSync)(`reg query "${registryKey}" /v Path`, {
            encoding: 'utf-8',
            timeout: 8000,
            windowsHide: true,
        });
...
function resolveWindowsRegistryPath() {
    if (cachedWindowsRegistryPath !== undefined)
    ...
    try {
        ...
        if (registryPath.trim()) {
            ...
            (0, coworkLogger_1.coworkLog)('INFO', 'resolveWindowsRegistryPath', `Resolved ${unique.length} PATH entries from Windows registry`);
        }
        else {
            cachedWindowsRegistryPath = null;
        }
        ...
        (0, coworkLogger_1.coworkLog)('WARN', 'resolveWindowsRegistryPath', `Failed to read PATH from Windows registry: ${error instanceof Error ? error.message : String(error)}`);
        cachedWindowsRegistryPath = null;
    }
    return cachedWindowsRegistryPath;
}
...
function ensureWindowsRegistryPathEntries(env) {
    ...
    const currentEntriesLower = new Set(currentPath.split(path_1.delimiter).map((entry) => entry.toLowerCase().replace(/\\$/, '')));
    ...
    if (missingEntries.length > 0) {
        ...
        env.PATH = currentPath ? `${currentPath}${path_1.delimiter}${missingEntries.join(path_1.delimiter)}` : missingEntries.join(path_1.delimiter);
        (0, coworkLogger_1.coworkLog)('INFO', 'ensureWindowsRegistryPathEntries', `Appended ${missingEntries.length} missing PATH entries from Windows registry: ${missingEntries.join(', ')}`);
    }
}
/**
...
function listWindowsCommandPaths(command) {
    try {
        ...
        const parsed = output
            ...
            .filter((line) => Boolean(line && (0, fs_1.existsSync)(line)));
...
function listGitInstallPathsFromRegistry() {
    ...
    for (const key of registryKeys) {
        try {
            const output = (0, child_process_1.execSync)(`reg query "${key}" /v InstallPath`, { encoding: 'utf-8', timeout: 5000 });
            for (const line of output.split(/\r?\n/)) {
...
function getBundledGitBashCandidates() {
    const bundledRoots = electron_1.app.isPackaged
        ? [(0, path_1.join)(process.resourcesPath, 'mingit')]
        : [
            (0, path_1.join)(__dirname, '..', '..', 'resources', 'mingit'),
            (0, path_1.join)(process.cwd(), 'resources', 'mingit'),
        ];
    const candidates = [];
    for (const root of bundledRoots) {
        // Prefer bin/bash.exe on Windows; invoking usr/bin/bash.exe directly may miss Git toolchain PATH.
        candidates.push((0, path_1.join)(root, 'bin', 'bash.exe'));
        candidates.push((0, path_1.join)(root, 'usr', 'bin', 'bash.exe'));
    }
    return candidates;
}
function checkWindowsGitBashHealth(bashPath) {
    try {
        if (!(0, fs_1.existsSync)(bashPath)) {
        ...
        const fastResult = (0, child_process_1.spawnSync)(bashPath, ['-c', 'cygpath -u "C:\\\\Windows"'], {
            encoding: 'utf-8',
            timeout: 5000,
            windowsHide: true,
            env: healthEnv,
        ...
        const result = (fastResult.error || (typeof fastResult.status === 'number' && fastResult.status !== 0))
            ...
            ? (0, child_process_1.spawnSync)(bashPath, ['-lc', 'cygpath -u "C:\\\\Windows"'], {
                encoding: 'utf-8',
                timeout: 15000,
                windowsHide: true,
                env: healthEnv,
...
function getWindowsGitToolDirs(bashPath) {
    ...
    if (!gitRoot) {
        const bashDir = (0, path_1.dirname)(normalized);
        return [bashDir].filter((dir) => (0, fs_1.existsSync)(dir));
    }
    const candidates = [
        (0, path_1.join)(gitRoot, 'cmd'),
        (0, path_1.join)(gitRoot, 'mingw64', 'bin'),
        (0, path_1.join)(gitRoot, 'usr', 'bin'),
        (0, path_1.join)(gitRoot, 'bin'),
    ];
    return candidates.filter((dir) => (0, fs_1.existsSync)(dir));
}
function ensureElectronNodeShim(electronPath, npmBinDir) {
    try {
        const shimDir = (0, path_1.join)(electron_1.app.getPath('userData'), 'cowork', 'bin');
        (0, fs_1.mkdirSync)(shimDir, { recursive: true });
        (0, coworkLogger_1.coworkLog)('INFO', 'resolveNodeShim', `Shim directory: ${shimDir}, electronPath: ${electronPath}, npmBinDir: ${npmBinDir || '(none)'}`);
        // --- node shim ---
        // Shell script (macOS/Linux/Windows git-bash)
        const nodeSh = (0, path_1.join)(shimDir, 'node');
        const nodeShContent = [
            '#!/usr/bin/env bash',
        ...
        (0, fs_1.writeFileSync)(nodeSh, nodeShContent, 'utf8');
        try {
            (0, fs_1.chmodSync)(nodeSh, 0o755);
        }
        catch {
            // Ignore chmod errors on file systems that do not support POSIX modes.
        }
        (0, coworkLogger_1.coworkLog)('INFO', 'resolveNodeShim', `Created node bash shim: ${nodeSh}`);
        // Windows .cmd wrapper (only needed on Windows)
        if (process.platform === 'win32') {
            const nodeCmd = (0, path_1.join)(shimDir, 'node.cmd');
            ...
            (0, fs_1.writeFileSync)(nodeCmd, nodeCmdContent, 'utf8');
            (0, coworkLogger_1.coworkLog)('INFO', 'resolveNodeShim', `Created node.cmd shim: ${nodeCmd}`);
        }
        // --- npx / npm shims ---
        ...
        if (npmBinDir && (0, fs_1.existsSync)(npmBinDir)) {
            const npxCliJs = (0, path_1.join)(npmBinDir, 'npx-cli.js');
            const npmCliJs = (0, path_1.join)(npmBinDir, 'npm-cli.js');
            ...
            (0, coworkLogger_1.coworkLog)('INFO', 'resolveNodeShim', `npmBinDir exists: true, npx-cli.js exists: ${(0, fs_1.existsSync)(npxCliJs)}, npm-cli.js exists: ${(0, fs_1.existsSync)(npmCliJs)}`);
            if ((0, fs_1.existsSync)(npxCliJs)) {
                // npx bash shim
                const npxSh = (0, path_1.join)(shimDir, 'npx');
                const npxShContent = [
                    '#!/usr/bin/env bash',
                ...
                (0, fs_1.writeFileSync)(npxSh, npxShContent, 'utf8');
                try {
                    (0, fs_1.chmodSync)(npxSh, 0o755);
                }
                catch { /* ignore */ }
                (0, coworkLogger_1.coworkLog)('INFO', 'resolveNodeShim', `Created npx bash shim: ${npxSh} -> ${npxCliJsPosix}`);
                ...
                if (process.platform === 'win32') {
                    const npxCmd = (0, path_1.join)(shimDir, 'npx.cmd');
                    const npxCmdContent = [
                        '@echo off',
                        '"%~dp0node.cmd" "%LOBSTERAI_NPM_BIN_DIR%\\npx-cli.js" %*',
                        '',
                    ].join('\r\n');
                    (0, fs_1.writeFileSync)(npxCmd, npxCmdContent, 'utf8');
                    (0, coworkLogger_1.coworkLog)('INFO', 'resolveNodeShim', `Created npx.cmd shim: ${npxCmd} (using env var LOBSTERAI_NPM_BIN_DIR)`);
                }
            }
            else {
                (0, coworkLogger_1.coworkLog)('WARN', 'resolveNodeShim', `npx-cli.js not found at: ${npxCliJs}`);
            }
            if ((0, fs_1.existsSync)(npmCliJs)) {
                // npm bash shim
                const npmSh = (0, path_1.join)(shimDir, 'npm');
                const npmShContent = [
                    '#!/usr/bin/env bash',
                    'SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"',
                ...
                (0, fs_1.writeFileSync)(npmSh, npmShContent, 'utf8');
                try {
                    (0, fs_1.chmodSync)(npmSh, 0o755);
                }
                catch { /* ignore */ }
                (0, coworkLogger_1.coworkLog)('INFO', 'resolveNodeShim', `Created npm bash shim: ${npmSh} -> ${npmCliJsPosix}`);
                // npm.cmd for Windows — uses %LOBSTERAI_NPM_BIN_DIR% env var to avoid
                // hardcoding paths that may contain non-ASCII chars (breaks GBK cmd.exe).
                if (process.platform === 'win32') {
                    const npmCmd = (0, path_1.join)(shimDir, 'npm.cmd');
                    const npmCmdContent = [
                        '@echo off',
                        '"%~dp0node.cmd" "%LOBSTERAI_NPM_BIN_DIR%\\npm-cli.js" %*',
                        '',
                    ].join('\r\n');
                    (0, fs_1.writeFileSync)(npmCmd, npmCmdContent, 'utf8');
                    (0, coworkLogger_1.coworkLog)('INFO', 'resolveNodeShim', `Created npm.cmd shim: ${npmCmd} (using env var LOBSTERAI_NPM_BIN_DIR)`);
                }
            }
            else {
                (0, coworkLogger_1.coworkLog)('WARN', 'resolveNodeShim', `npm-cli.js not found at: ${npmCliJs}`);
            }
            (0, coworkLogger_1.coworkLog)('INFO', 'resolveNodeShim', `Created npx/npm shims pointing to: ${npmBinDir}`);
        }
        else {
            (0, coworkLogger_1.coworkLog)('WARN', 'resolveNodeShim', `npmBinDir not available: ${npmBinDir || '(not provided)'}, exists: ${npmBinDir ? (0, fs_1.existsSync)(npmBinDir) : 'N/A'}`);
        }
        // Verify shim files exist and are executable
        const shimFiles = ['node', 'npx', 'npm'];
        for (const name of shimFiles) {
            const shimPath = (0, path_1.join)(shimDir, name);
            const exists = (0, fs_1.existsSync)(shimPath);
            if (exists) {
                try {
                    const stat = (0, fs_1.statSync)(shimPath);
                    (0, coworkLogger_1.coworkLog)('INFO', 'resolveNodeShim', `Shim verify: ${name} exists, mode=0o${stat.mode.toString(8)}, size=${stat.size}`);
                }
                catch (e) {
                    (0, coworkLogger_1.coworkLog)('WARN', 'resolveNodeShim', `Shim verify: ${name} exists but stat failed: ${e instanceof Error ? e.message : String(e)}`);
                }
            }
            else {
                (0, coworkLogger_1.coworkLog)('WARN', 'resolveNodeShim', `Shim verify: ${name} NOT found at ${shimPath}`);
            }
        }
        return shimDir;
    }
    catch (error) {
        (0, coworkLogger_1.coworkLog)('WARN', 'resolveNodeShim', `Failed to prepare Electron Node shim: ${error instanceof Error ? error.message : String(error)}`);
        return null;
    }
}
/**
...
function resolveWindowsGitBashPath() {
    ...
    const installCandidates = [
        (0, path_1.join)(programFiles, 'Git', 'bin', 'bash.exe'),
        (0, path_1.join)(programFiles, 'Git', 'usr', 'bin', 'bash.exe'),
        (0, path_1.join)(programFilesX86, 'Git', 'bin', 'bash.exe'),
        (0, path_1.join)(programFilesX86, 'Git', 'usr', 'bin', 'bash.exe'),
        (0, path_1.join)(localAppData, 'Programs', 'Git', 'bin', 'bash.exe'),
        (0, path_1.join)(localAppData, 'Programs', 'Git', 'usr', 'bin', 'bash.exe'),
        (0, path_1.join)(userProfile, 'scoop', 'apps', 'git', 'current', 'bin', 'bash.exe'),
        (0, path_1.join)(userProfile, 'scoop', 'apps', 'git', 'current', 'usr', 'bin', 'bash.exe'),
        'C:\\Git\\bin\\bash.exe',
    ...
    for (const installRoot of registryInstallRoots) {
        const registryCandidates = [
            (0, path_1.join)(installRoot, 'bin', 'bash.exe'),
            (0, path_1.join)(installRoot, 'usr', 'bin', 'bash.exe'),
        ];
    ...
    for (const gitPath of gitPaths) {
        const gitRoot = (0, path_1.dirname)((0, path_1.dirname)(gitPath));
    ...
    for (const candidate of candidates) {
        if (!(0, fs_1.existsSync)(candidate.path)) {
            continue;
        }
        const health = checkWindowsGitBashHealth(candidate.path);
        if (health.ok) {
            (0, coworkLogger_1.coworkLog)('INFO', 'resolveGitBash', `Selected git-bash (${candidate.source}): ${candidate.path}`);
        ...
        (0, coworkLogger_1.coworkLog)('WARN', 'resolveGitBash', failure);
    }
    ...
    (0, coworkLogger_1.coworkLog)('WARN', 'resolveGitBash', diagnostic);
    cachedGitBashPath = null;
...
function ensureWindowsSystemEnvVars(env) {
    ...
    if (injected.length > 0) {
        (0, coworkLogger_1.coworkLog)('INFO', 'ensureWindowsSystemEnvVars', `Injected missing Windows system env vars: ${injected.join(', ')}`);
    }
}
/**
...
function ensureWindowsSystemPathEntries(env) {
    ...
    const currentPath = env.PATH || '';
    const currentEntries = currentPath.split(path_1.delimiter).map((entry) => entry.toLowerCase());
    const missingDirs = [];
    for (const relDir of WINDOWS_SYSTEM_PATH_ENTRIES) {
        const fullDir = (0, path_1.join)(systemRoot, relDir);
        if (!currentEntries.includes(fullDir.toLowerCase()) && (0, fs_1.existsSync)(fullDir)) {
            missingDirs.push(fullDir);
        }
    }
    // Also ensure the systemRoot itself (e.g. C:\windows) is in PATH
    if (!currentEntries.includes(systemRoot.toLowerCase()) && (0, fs_1.existsSync)(systemRoot)) {
        missingDirs.push(systemRoot);
    }
    if (missingDirs.length > 0) {
        // Append system dirs at the END so they don't override user tools
        env.PATH = currentPath ? `${currentPath}${path_1.delimiter}${missingDirs.join(path_1.delimiter)}` : missingDirs.join(path_1.delimiter);
        (0, coworkLogger_1.coworkLog)('INFO', 'ensureWindowsSystemPathEntries', `Appended missing Windows system PATH entries: ${missingDirs.join(', ')}`);
    }
}
/**
...
function ensureWindowsBashBootstrapPath(env) {
    ...
    const entries = currentPath.split(path_1.delimiter).map((entry) => entry.trim()).filter(Boolean);
    if (entries.some((entry) => entry === bootstrapToken)) {
        return;
    }
    env.PATH = `${bootstrapToken}${path_1.delimiter}${currentPath}`;
    (0, coworkLogger_1.coworkLog)('INFO', 'ensureWindowsBashBootstrapPath', `Prepended bash bootstrap PATH token: ${bootstrapToken}`);
}
/**
...
function ensureWindowsOriginalPath(env) {
    ...
    (0, coworkLogger_1.coworkLog)('INFO', 'ensureWindowsOriginalPath', `Set ORIGINAL_PATH with ${posixPath.split(':').length} POSIX-format entries`);
}
/**
...
function ensureWindowsBashUtf8InitScript() {
    try {
        const initDir = (0, path_1.join)(electron_1.app.getPath('userData'), 'cowork', 'bin');
        (0, fs_1.mkdirSync)(initDir, { recursive: true });
        const initScript = (0, path_1.join)(initDir, 'bash_utf8_init.sh');
        const content = [
            '#!/usr/bin/env bash',
            '# Auto-generated by Caisra – switch Windows console code page to UTF-8',
        ...
        (0, fs_1.writeFileSync)(initScript, content, 'utf8');
        try {
            (0, fs_1.chmodSync)(initScript, 0o755);
        }
        catch {
            // Ignore chmod errors on file systems that do not support POSIX modes.
        }
        ...
        (0, coworkLogger_1.coworkLog)('WARN', 'ensureWindowsBashUtf8InitScript', `Failed to create bash UTF-8 init script: ${error instanceof Error ? error.message : String(error)}`);
        return null;
    }
}
function applyPackagedEnvOverrides(env) {
    const electronNodeRuntimePath = getElectronNodeRuntimePath();
    if (electron_1.app.isPackaged && !env.LOBSTERAI_ELECTRON_PATH) {
    ...
    if (process.platform === 'win32') {
        ...
        if (!env.BASH_ENV) {
            ...
            if (initScript) {
                ...
                (0, coworkLogger_1.coworkLog)('INFO', 'applyPackagedEnvOverrides', `Set BASH_ENV for UTF-8 console code page: ${env.BASH_ENV}`);
            }
        }
        ...
        let bashPath = configuredBashPath && (0, fs_1.existsSync)(configuredBashPath)
            ? configuredBashPath
        ...
        if (configuredBashPath && bashPath === configuredBashPath) {
            ...
            if (!configuredHealth.ok) {
                ...
                if (fallbackPath && fallbackPath !== configuredBashPath) {
                    (0, coworkLogger_1.coworkLog)('WARN', 'resolveGitBash', `Configured bash is unhealthy (${configuredBashPath}): ${configuredHealth.reason || 'unknown reason'}. Falling back to: ${fallbackPath}`);
                    bashPath = fallbackPath;
                }
                else {
                    const diagnostic = truncateDiagnostic(`Configured bash is unhealthy (${configuredBashPath}): ${configuredHealth.reason || 'unknown reason'}`);
        ...
        if (bashPath) {
            ...
            (0, coworkLogger_1.coworkLog)('INFO', 'resolveGitBash', `Using Windows git-bash: ${bashPath}`);
            const gitToolDirs = getWindowsGitToolDirs(bashPath);
            env.PATH = appendEnvPath(env.PATH, gitToolDirs);
            (0, coworkLogger_1.coworkLog)('INFO', 'resolveGitBash', `Injected Windows Git toolchain PATH entries: ${gitToolDirs.join(', ')}`);
        ...
        if (!env.MSYS2_PATH_TYPE) {
            ...
            (0, coworkLogger_1.coworkLog)('INFO', 'applyPackagedEnvOverrides', 'Set MSYS2_PATH_TYPE=inherit to preserve PATH in git-bash');
        }
    ...
    if (!electron_1.app.isPackaged) {
        ...
        const devBinDir = (0, path_1.join)(electron_1.app.getAppPath(), 'node_modules', '.bin');
        if ((0, fs_1.existsSync)(devBinDir)) {
            env.PATH = [devBinDir, env.PATH].filter(Boolean).join(path_1.delimiter);
            (0, coworkLogger_1.coworkLog)('INFO', 'applyPackagedEnvOverrides', `Dev mode: prepended node_modules/.bin to PATH: ${devBinDir}`);
        }
        ...
        const devRuntimeNodeModules = (() => {
            const candidates = [
                (0, path_1.join)(electron_1.app.getAppPath(), 'vendor', 'openclaw-runtime', 'current', 'node_modules'),
                (0, path_1.join)(process.cwd(), 'vendor', 'openclaw-runtime', 'current', 'node_modules'),
            ];
            for (const c of candidates) {
                try {
                    const resolved = (0, fs_1.realpathSync)(c);
        ...
        if (devRuntimeNodeModules) {
            ...
            (0, coworkLogger_1.coworkLog)('INFO', 'applyPackagedEnvOverrides', `Dev mode: added openclaw runtime node_modules to NODE_PATH: ${devRuntimeNodeModules}`);
        }
        return;
    }
    if (!env.HOME) {
    ...
    if (userPath) {
        env.PATH = userPath;
        (0, coworkLogger_1.coworkLog)('INFO', 'applyPackagedEnvOverrides', `Resolved user shell PATH (${userPath.split(path_1.delimiter).length} entries)`);
        for (const entry of userPath.split(path_1.delimiter)) {
            (0, coworkLogger_1.coworkLog)('INFO', 'applyPackagedEnvOverrides', `  PATH entry: ${entry} (exists: ${(0, fs_1.existsSync)(entry)})`);
        }
    }
    else {
        // Fallback: append common node installation paths
        const home = env.HOME || electron_1.app.getPath('home');
        const commonPaths = [
            '/usr/local/bin',
            '/opt/homebrew/bin',
        ...
        env.PATH = [env.PATH, ...commonPaths].filter(Boolean).join(path_1.delimiter);
        (0, coworkLogger_1.coworkLog)('WARN', 'applyPackagedEnvOverrides', `Failed to resolve user shell PATH, using fallback common paths`);
    }
    const resourcesPath = process.resourcesPath;
    (0, coworkLogger_1.coworkLog)('INFO', 'applyPackagedEnvOverrides', `Packaged mode: resourcesPath=${resourcesPath}`);
    ...
    const npmBinDir = (0, path_1.join)(resourcesPath, 'app.asar.unpacked', 'node_modules', 'npm', 'bin');
    (0, coworkLogger_1.coworkLog)('INFO', 'applyPackagedEnvOverrides', `npmBinDir=${npmBinDir}, exists=${(0, fs_1.existsSync)(npmBinDir)}`);
    ...
    const shouldForcePackagedDarwinShim = electron_1.app.isPackaged && process.platform === 'darwin';
    ...
    if (shouldInjectShim) {
        ...
        if (shimDir) {
            env.PATH = [shimDir, env.PATH].filter(Boolean).join(path_1.delimiter);
            env.LOBSTERAI_NODE_SHIM_ACTIVE = '1';
            (0, coworkLogger_1.coworkLog)('INFO', 'resolveNodeShim', `Injected Electron Node/npx/npm shim PATH entry: ${shimDir}`);
            if (shouldForcePackagedDarwinShim) {
                (0, coworkLogger_1.coworkLog)('INFO', 'resolveNodeShim', 'Packaged macOS build: forcing bundled Electron node/npx/npm shims to avoid stale system Node versions');
            }
        ...
        (0, coworkLogger_1.coworkLog)('INFO', 'resolveNodeShim', 'System node/npx/npm detected; skipped Electron node shim injection');
    }
    const nodePaths = [
        (0, path_1.join)(resourcesPath, 'app.asar', 'node_modules'),
        (0, path_1.join)(resourcesPath, 'app.asar.unpacked', 'node_modules'),
        (0, path_1.join)(resourcesPath, 'cfmind', 'node_modules'),
    ].filter((nodePath) => (0, fs_1.existsSync)(nodePath));
...
function verifyNodeEnvironment(env) {
    ...
    (0, coworkLogger_1.coworkLog)('INFO', tag, `Final PATH has ${pathEntries.length} entries:`);
    for (let i = 0; i < pathEntries.length; i++) {
        const entry = pathEntries[i];
        const exists = entry ? (0, fs_1.existsSync)(entry) : false;
        (0, coworkLogger_1.coworkLog)('INFO', tag, `  [${i}] ${entry} (exists: ${exists})`);
    }
    // Try to resolve node, npx, npm using 'which' (macOS/Linux) or 'where' (Windows)
    const whichCmd = process.platform === 'win32' ? 'where' : 'which';
    for (const tool of ['node', 'npx', 'npm']) {
        try {
            const result = (0, child_process_1.spawnSync)(whichCmd, [tool], {
                env: { ...env },
                encoding: 'utf-8',
                timeout: 5000,
                windowsHide: process.platform === 'win32',
            ...
            if (result.status === 0 && result.stdout) {
                ...
                (0, coworkLogger_1.coworkLog)('INFO', tag, `${whichCmd} ${tool} => ${resolved}`);
                ...
                if (tool === 'node' && resolvedForExec) {
                    try {
                        ...
                        const versionResult = (0, child_process_1.spawnSync)(execTarget, ['--version'], {
                            env: { ...env, ELECTRON_RUN_AS_NODE: '1' },
                            encoding: 'utf-8',
                            timeout: 5000,
                            windowsHide: process.platform === 'win32',
                        });
                        (0, coworkLogger_1.coworkLog)('INFO', tag, `node --version (${execTarget}) => ${(versionResult.stdout || '').trim()} (exit: ${versionResult.status})`);
                        if (versionResult.error) {
                            (0, coworkLogger_1.coworkLog)('WARN', tag, `node --version spawn error: ${versionResult.error.message}`);
                        }
                        if (versionResult.stderr) {
                            (0, coworkLogger_1.coworkLog)('WARN', tag, `node --version stderr: ${versionResult.stderr.trim()}`);
                        }
                    }
                    catch (e) {
                        (0, coworkLogger_1.coworkLog)('WARN', tag, `node --version failed: ${e instanceof Error ? e.message : String(e)}`);
                    }
                }
            }
            else {
                (0, coworkLogger_1.coworkLog)('WARN', tag, `${whichCmd} ${tool} => NOT FOUND (exit: ${result.status}, stderr: ${(result.stderr || '').trim()})`);
            }
        }
        catch (e) {
            (0, coworkLogger_1.coworkLog)('WARN', tag, `${whichCmd} ${tool} threw: ${e instanceof Error ? e.message : String(e)}`);
        }
    }
    // Log key env vars
    (0, coworkLogger_1.coworkLog)('INFO', tag, `NODE_PATH=${env.NODE_PATH || '(not set)'}`);
    (0, coworkLogger_1.coworkLog)('INFO', tag, `LOBSTERAI_ELECTRON_PATH=${env.LOBSTERAI_ELECTRON_PATH || '(not set)'}`);
    (0, coworkLogger_1.coworkLog)('INFO', tag, `LOBSTERAI_NPM_BIN_DIR=${env.LOBSTERAI_NPM_BIN_DIR || '(not set)'}`);
    (0, coworkLogger_1.coworkLog)('INFO', tag, `HOME=${env.HOME || '(not set)'}`);
}
/**
...
function getSkillsRoot() {
    if (electron_1.app.isPackaged) {
        ...
        return (0, path_1.join)(electron_1.app.getPath('userData'), 'SKILLs');
    }
    ...
    const candidates = [
        ...envRoots,
        (0, path_1.join)(electron_1.app.getAppPath(), 'SKILLs'),
        (0, path_1.join)(process.cwd(), 'SKILLs'),
        (0, path_1.join)(__dirname, '..', 'SKILLs'),
        (0, path_1.join)(__dirname, '..', '..', 'SKILLs'),
    ];
    for (const candidate of candidates) {
    ...
    return (0, path_1.join)(electron_1.app.getAppPath(), 'SKILLs');
}
function finalizeNodeToolEnv(env) {
...
async function injectSystemProxyForSubprocess(env) {
...
async function getEnhancedEnv(target = 'local') {
    const config = (0, claudeSettings_1.getCurrentApiConfig)(target);
    const env = config
        ? (0, claudeSettings_1.buildEnvForConfig)(config)
        : { ...process.env };
    applyPackagedEnvOverrides(env);
    ...
    env.LOBSTERAI_SKILLS_ROOT = skillsRoot; // Legacy name; bundled skill scripts still read it
    env.CAISRA_SKILLS_ROOT = skillsRoot;
    finalizeNodeToolEnv(env);
    await injectSystemProxyForSubprocess(env);
    return env;
}
...
function ensureCoworkTempDir(cwd) {
    const tempDir = (0, path_1.join)(cwd, constants_1.COWORK_TEMP_DIR_NAME);
...
function resolveSessionTitleApiConfig() {
    const rawResolution = (0, claudeSettings_1.resolveRawApiConfig)();
    if (rawResolution.config && rawResolution.providerMetadata?.providerName === 'gemini') {
        return {
            config: {
                protocol: coworkModelApi_1.CoworkModelProtocol.GeminiNative,
    ...
    const resolution = (0, claudeSettings_1.resolveCurrentApiConfig)();
    ...
    return {
        config: {
            protocol: coworkModelApi_1.CoworkModelProtocol.Anthropic,
...
async function probeCoworkModelReadiness(timeoutMs = COWORK_MODEL_PROBE_TIMEOUT_MS) {
    ...
    try {
        const response = await fetch(config.protocol === coworkModelApi_1.CoworkModelProtocol.GeminiNative
            ? (0, coworkModelApi_1.buildGeminiGenerateContentUrl)(config.baseURL, config.model)
            : (0, coworkModelApi_1.buildAnthropicMessagesUrl)(config.baseURL), {
            method: 'POST',
            headers: config.protocol === coworkModelApi_1.CoworkModelProtocol.GeminiNative
                ? {
            ...
            body: JSON.stringify(config.protocol === coworkModelApi_1.CoworkModelProtocol.GeminiNative
                ? {
        ...
        if (!response.ok) {
            ...
            const errorSnippet = (0, coworkModelApi_1.extractApiErrorSnippet)(errorText);
            return {
                ok: false,
                error: errorSnippet
...
async function generateSessionTitle(userIntent, defaultTitle = SESSION_TITLE_FALLBACK) {
    return (0, sessionTitle_1.buildSessionTitleFromInput)(userIntent, defaultTitle);
}
//# sourceMappingURL=coworkUtil.js.map