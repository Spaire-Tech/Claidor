...
exports.buildManagedArtifactsPrompt = buildManagedArtifactsPrompt;
const constants_1 = require("../../shared/artifacts/constants");
const prompts_generated_1 = require("../../shared/artifacts/prompts.generated");
 ...
 */
function buildManagedArtifactsPrompt() {
    const deck = constants_1.ARTIFACT_ROOTS[constants_1.ArtifactKind.Presentation];
    const report = constants_1.ARTIFACT_ROOTS[constants_1.ArtifactKind.Report];
    return [
        '## Artifacts',
        '',
        `Two answers are not cards but things that stand on their own: a slide deck and a report. They use the same fenced \`${library_1.CARD_FENCE}\` block and the same syntax as the cards, with a different root: \`root = ${deck}(...)\` for a deck, \`root = ${report}(...)\` for a report. The app shows a small chip in the conversation with the title on it; the person opens it into the full deck or report.`,
        '',
        '### When',
        ...
        '### Deck',
        '',
        prompts_generated_1.PRESENTATION_SIGNATURES,
        '',
        '### Report',
        '',
        prompts_generated_1.REPORT_SIGNATURES,
    ].join('\n');
}
//# sourceMappingURL=artifactsPrompt.js.map