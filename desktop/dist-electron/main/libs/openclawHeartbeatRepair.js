...
exports.stripProactiveHeartbeatSection = exports.repairHeartbeatFile = exports.isLegacyHeartbeatTemplate = exports.HEARTBEAT_EMPTY_TEMPLATE = exports.HEARTBEAT_EMPTY_TEMPLATE_LINES = void 0;
...
exports.HEARTBEAT_EMPTY_TEMPLATE_LINES = [
    '# Keep this file empty (or with only comments) to skip heartbeat API calls.',
    '# Add tasks below when you want the agent to check something periodically.',
];
exports.HEARTBEAT_EMPTY_TEMPLATE = `${exports.HEARTBEAT_EMPTY_TEMPLATE_LINES.join('\n')}\n`;
const normalizeLines = (content) => content
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter((line) => line.length > 0);
const linesEqual = (left, right) => left.length === right.length && left.every((line, index) => line === right[index]);
/**
 ...
 */
const isLegacyHeartbeatTemplate = (content) => {
...
exports.isLegacyHeartbeatTemplate = isLegacyHeartbeatTemplate;
/**
 ...
 */
const repairHeartbeatFile = (workspaceDir) => {
    ...
    const heartbeatPath = path_1.default.join(workspaceDir, HEARTBEAT_FILENAME);
    let content = null;
    try {
        content = fs_1.default.readFileSync(heartbeatPath, 'utf8');
    }
    catch (error) {
    ...
    if (content !== null && !(0, exports.isLegacyHeartbeatTemplate)(content)) {
        return false;
    }
    const tmpPath = `${heartbeatPath}.tmp-${Date.now()}`;
    fs_1.default.writeFileSync(tmpPath, exports.HEARTBEAT_EMPTY_TEMPLATE, 'utf8');
    fs_1.default.renameSync(tmpPath, heartbeatPath);
    return true;
};
exports.repairHeartbeatFile = repairHeartbeatFile;
 ...
 */
const stripProactiveHeartbeatSection = (content) => {
...
exports.stripProactiveHeartbeatSection = stripProactiveHeartbeatSection;
//# sourceMappingURL=openclawHeartbeatRepair.js.map