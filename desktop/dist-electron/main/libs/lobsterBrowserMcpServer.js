...
exports.resolveLobsterBrowserMcpStdioLaunch = exports.resolveLobsterBrowserMcpCommand = void 0;
...
const credentialOnly = process.argv.includes('${constants_1.BrowserCredentialMcpServer.ToolSetArgument}');
writeDiagnostic(
...
const toolDefinitions = [
  ...
  ['${constants_1.BrowserCredentialLoginTool.Name}', {
    pageId: { type: 'number' },
...
const tools = toolDefinitions
  .filter(([name]) => !credentialOnly || name === '${constants_1.BrowserCredentialLoginTool.Name}')
...
`;
const formatBridgeEndpointForLog = (bridgeUrl) => {
    try {
...
const prepareLobsterBrowserMcpRuntime = (baseDir, options) => {
    ...
    const serverDir = path_1.default.join(baseDir, 'lobster-browser-mcp');
    fs_1.default.mkdirSync(serverDir, { recursive: true, mode: 0o700 });
    if (process.platform !== 'win32') {
        fs_1.default.chmodSync(serverDir, 0o700);
    }
    const serverPath = path_1.default.join(serverDir, SERVER_FILE_NAME);
...
const resolveLobsterBrowserMcpCommand = (baseDir, options) => {
    const { serverDir } = prepareLobsterBrowserMcpRuntime(baseDir, options);
    if ((options.platform ?? process.platform) === 'win32') {
        const launcherPath = path_1.default.join(serverDir, WINDOWS_LAUNCHER_FILE_NAME);
        writeFileIfChanged(launcherPath, buildWindowsLauncherSource(options.electronNodeRuntimePath), 0o700);
        console.log('[LobsterBrowserMcp] Prepared browser MCP launcher', {
            platform: options.platform ?? process.platform,
            launcherPath,
            launcherExists: fs_1.default.existsSync(launcherPath),
            electronNodeRuntimePath: options.electronNodeRuntimePath,
            electronNodeRuntimeExists: fs_1.default.existsSync(options.electronNodeRuntimePath),
            bridgeEndpoint: formatBridgeEndpointForLog(options.bridgeUrl),
        });
        return launcherPath;
    }
    const launcherPath = path_1.default.join(serverDir, POSIX_LAUNCHER_FILE_NAME);
    writeFileIfChanged(launcherPath, buildPosixLauncherSource(options.electronNodeRuntimePath), 0o700);
    console.log('[LobsterBrowserMcp] Prepared browser MCP launcher', {
        platform: options.platform ?? process.platform,
        launcherPath,
        launcherExists: fs_1.default.existsSync(launcherPath),
...
exports.resolveLobsterBrowserMcpCommand = resolveLobsterBrowserMcpCommand;
const resolveLobsterBrowserMcpStdioLaunch = (baseDir, options) => {
    ...
    console.log('[LobsterBrowserMcp] Prepared browser MCP stdio launch', {
        platform: options.platform ?? process.platform,
        serverPath,
        serverExists: fs_1.default.existsSync(serverPath),
        electronNodeRuntimePath: options.electronNodeRuntimePath,
        electronNodeRuntimeExists: fs_1.default.existsSync(options.electronNodeRuntimePath),
        bridgeEndpoint: formatBridgeEndpointForLog(options.bridgeUrl),
    });
...
exports.resolveLobsterBrowserMcpStdioLaunch = resolveLobsterBrowserMcpStdioLaunch;
//# sourceMappingURL=lobsterBrowserMcpServer.js.map