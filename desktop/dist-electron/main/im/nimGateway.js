...
const nimMedia_1 = require("./nimMedia");
const dingtalkMediaParser_1 = require("./dingtalkMediaParser");
const nimQChatClient_1 = require("./nimQChatClient");
...
function parseConversationId(conversationId) {
    ...
    if (parts.length >= 3) {
        ...
        const sessionType = typeNum === 1 ? 'p2p'
            : typeNum === 2 ? 'team'
                : typeNum === 3 ? 'superTeam'
                    : 'p2p';
 ...
 */
function buildConversationId(conversationIdUtil, accountId, sessionType = 'p2p') {
 ...
 */
function getSdkDataPath(account) {
    let baseDir;
    try {
...
class NimGateway extends events_1.EventEmitter {
    v2Client = null;
    config = null;
    status = { ...types_1.DEFAULT_NIM_STATUS };
    onMessageCallback;
    lastSenderId = null;
    log = () => { };
    reconnectTimer = null;
     ...
     */
    boundOnReceiveMessages = null;
    boundOnLoginStatus = null;
    boundOnKickedOffline = null;
    boundOnLoginFailed = null;
    boundOnDisconnected = null;
    /**
    ...
    scheduleReconnect(delayMs) {
        ...
        this.reconnectTimer = setTimeout(async () => {
            ...
            if (!savedConfig || this.stopRequested)
                return;
            try {
                ...
                const nextDelay = Math.min((this.reconnectAttempts <= 1 ? 2000 : delayMs * 2), this.MAX_RECONNECT_DELAY_MS);
    ...
    async start(config, options) {
        ...
        let resolve;
        this.lifecyclePromise = new Promise(r => { resolve = r; });
        try {
     ...
     */
    async startInternal(config, options) {
        if (this.v2Client) {
        ...
        try {
            ...
            this.boundOnReceiveMessages = (messages) => {
                if (!this.v2Client)
            ...
            this.boundOnLoginStatus = (loginStatus) => {
                if (!this.v2Client)
                    return; // stale callback guard
            ...
            this.boundOnKickedOffline = (detail) => {
                if (!this.v2Client)
                    return; // stale callback guard
            ...
            this.boundOnLoginFailed = (error) => {
                ...
                const delay = Math.min(this.reconnectAttempts <= 1 ? 3000 : 3000 * Math.pow(2, this.reconnectAttempts - 1), this.MAX_RECONNECT_DELAY_MS);
                this.scheduleReconnect(delay);
            };
            this.boundOnDisconnected = (error) => {
                if (!this.v2Client)
                    return; // stale callback guard
    ...
    async stopInternal() {
        ...
        try {
            ...
            if (this.v2Client?.V2NIMLoginService) {
                if (this.boundOnLoginStatus)
                    this.v2Client.V2NIMLoginService.off('onLoginStatus', this.boundOnLoginStatus);
                if (this.boundOnKickedOffline)
                    this.v2Client.V2NIMLoginService.off('onKickedOffline', this.boundOnKickedOffline);
                if (this.boundOnLoginFailed)
                    this.v2Client.V2NIMLoginService.off('onLoginFailed', this.boundOnLoginFailed);
                if (this.boundOnDisconnected)
                    this.v2Client.V2NIMLoginService.off('onDisconnected', this.boundOnDisconnected);
            }
        }
        catch (listenerErr) {
     ...
     */
    async fetchTeamName(teamId, teamType, fallbackId) {
        try {
            const nim = this.v2Client;
    ...
    async handleIncomingMessage(msg) {
        try {
            ...
            const supportedTypes = ['text', 'image', 'audio', 'video', 'file'];
            ...
            if (isTeam) {
                ...
                const forcePushIds = msg.pushConfig?.forcePushAccountIds ?? [];
                const teamPolicy = this.config?.team?.policy || 'disabled';
            ...
            if (msgType === 'text') {
                ...
                const placeholder = (0, nimMedia_1.inferMediaPlaceholder)(msgType);
                const attachUrl = attach?.url;
                content = attachUrl ? `${placeholder} ${attachUrl}` : placeholder;
                // 下载媒体文件
                if (attachUrl) {
                    const nimMediaType = msgType;
                    const downloaded = await (0, nimMedia_1.downloadNimMedia)(attachUrl, {
                        name: attach?.name,
                        size: attach?.size,
                        width: attach?.width,
                        height: attach?.height,
                        duration: attach?.duration,
                    }, nimMediaType, this.log);
                    if (downloaded) {
            ...
            if (isP2P && this.v2Client?.V2NIMMessageService) {
                this.v2Client.V2NIMMessageService.sendP2PMessageReceipt(msg).catch((err) => {
            ...
            const replyFn = async (text) => {
                ...
                if (isTeam) {
                    await this.sendTeamReply(targetId, text, msg, senderId, sessionType);
                }
                else {
     ...
     */
    async sendMedia(to, filePath) {
        if (!this.v2Client?.V2NIMMessageService || !this.v2Client?.V2NIMMessageCreator) {
            throw new Error('NIM SDK not ready');
        }
        const conversationId = buildConversationId(this.v2Client.V2NIMConversationIdUtil, to, 'p2p');
        await (0, nimMedia_1.sendNimMediaMessage)(this.v2Client.V2NIMMessageService, this.v2Client.V2NIMMessageCreator, conversationId, filePath, this.log);
    }
    /**
     ...
     */
    async sendReplyWithMedia(to, text) {
        // 1. 解析媒体标记
        const markers = (0, dingtalkMediaParser_1.parseMediaMarkers)(text);
        if (markers.length === 0) {
            // 没有媒体标记，纯文本发送
        ...
        // 2. 先发送去除标记后的文本（如果有）
        const strippedText = (0, dingtalkMediaParser_1.stripMediaMarkers)(text, markers);
     ...
     */
    async sendTeamReply(teamId, text, originalMsg, originalSenderId, sessionType) {
        if (!this.v2Client?.V2NIMMessageService || !this.v2Client?.V2NIMMessageCreator) {
            throw new Error('NIM SDK not ready');
        }
     ...
     */
    async sendTeamTextReply(teamId, text, originalMsg, originalSenderId, sessionType) {
        const chunks = splitMessageIntoChunks(text);
        for (let i = 0; i < chunks.length; i++) {
            ...
            const conversationId = buildConversationId(this.v2Client.V2NIMConversationIdUtil, teamId, sessionType);
            ...
            try {
                ...
                if (i === 0) {
                    await this.v2Client.V2NIMMessageService.replyMessage(replyMsg, originalMsg, sendParams, () => { });
                }
                else {
                    // Subsequent chunks are sent as regular messages
                    await this.v2Client.V2NIMMessageService.sendMessage(replyMsg, conversationId, sendParams, () => { });
                }
            }
            catch (error) {
     ...
     */
    async sendMediaToTeam(teamId, filePath, sessionType) {
        if (!this.v2Client?.V2NIMMessageService || !this.v2Client?.V2NIMMessageCreator) {
            throw new Error('NIM SDK not ready');
        }
        const conversationId = buildConversationId(this.v2Client.V2NIMConversationIdUtil, teamId, sessionType);
    ...
    async sendPlainTeamText(teamId, text, sessionType) {
        if (!this.v2Client?.V2NIMMessageService || !this.v2Client?.V2NIMMessageCreator) {
            throw new Error('NIM SDK not ready');
        }
        const chunks = splitMessageIntoChunks(text);
     ...
     */
    setNotificationTarget(senderId) {
        this.lastSenderId = senderId;
    }
    /**
    ...
    async sendConversationNotification(conversationId, text) {
        if (!conversationId) {
            throw new Error('No conversation available for notification');
        }
     ...
     */
    async sendNotificationWithMedia(text) {
        if (!this.lastSenderId || !this.v2Client?.V2NIMMessageService) {
            throw new Error('No conversation available for notification');
        }
    ...
    cleanupMediaFiles() {
        try {
            (0, nimMedia_1.cleanupOldNimMediaFiles)();
        }
        catch (error) {
    ...
    async activateQChat() {
        ...
        this.qchatClient = new nimQChatClient_1.NimQChatClient({
     ...
     */
    async sendQChatReply(serverId, channelId, text) {
        if (!this.qchatClient) {
     ...
     */
    async sendQChatLongText(serverId, channelId, text) {
        const chunks = splitMessageIntoChunks(text);
        for (const chunk of chunks) {
...
exports.NimGateway = NimGateway;
//# sourceMappingURL=nimGateway.js.map