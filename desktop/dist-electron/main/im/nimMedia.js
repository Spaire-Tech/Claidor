...
exports.getNimMediaDir = getNimMediaDir;
exports.inferMediaType = inferMediaType;
exports.inferMimeType = inferMimeType;
exports.inferMediaPlaceholder = inferMediaPlaceholder;
exports.downloadNimMedia = downloadNimMedia;
exports.sendNimMediaMessage = sendNimMediaMessage;
exports.cleanupOldNimMediaFiles = cleanupOldNimMediaFiles;
 ...
 */
function toIMMediaType(mediaType) {
    if (mediaType === 'file')
        return 'document';
    return mediaType;
}
 ...
 */
function downloadFile(url, destPath, maxBytes = MAX_FILE_SIZE) {
 ...
 * @param log 日志函数
 */
async function downloadNimMedia(url, attachment, mediaType, log = console.log) {
    if (!url) {
        return null;
    }
    try {
 ...
 * @param log 日志函数
 */
async function sendNimMediaMessage(messageService, messageCreator, conversationId, filePath, log = console.log) {
    ...
    if (fileSize > MAX_SEND_FILE_SIZE) {
        throw new Error(`文件过大: ${(fileSize / 1024 / 1024).toFixed(1)}MB，超出 100MB 发送限制`);
    }
 ...
 */
function cleanupOldNimMediaFiles(maxAgeDays = 7) {
    const mediaDir = getNimMediaDir();
...
//# sourceMappingURL=nimMedia.js.map