...
exports.IMCoworkHandler = void 0;
...
const imReplyGuard_1 = require("./imReplyGuard");
...
class IMCoworkHandler extends events_1.EventEmitter {
    coworkRuntime;
    coworkStore;
    imStore;
    getSkillsPrompt;
    ...
    messageAccumulators = new Map();
    // Track which sessions are created by IM (to filter events)
    imSessionIds = new Set();
    sessionConversationMap = new Map();
    pendingPermissionByConversation = new Map();
    ...
    onSessionStopped = this.handleSessionStopped.bind(this);
    constructor(options) {
        super();
        ...
        try {
            ...
            if (!this.isSessionNotFoundError(error)) {
                if (this.shouldRetryWithFreshSession(error, message)) {
                    console.warn(`[IMCoworkHandler] Detected recoverable API 400 for ${message.platform}:${message.conversationId}, recreating session and retrying once`);
                    return this.processMessageInternal(message, true);
                }
                throw error;
            }
            console.warn(`[IMCoworkHandler] Cowork session mapping is stale for ${message.platform}:${message.conversationId}, recreating session`);
            return this.processMessageInternal(message, true);
        }
    }
    async processMessageInternal(message, forceNewSession) {
        const coworkSessionId = await this.getOrCreateCoworkSession(message.conversationId, message.platform, forceNewSession, message.senderId, message);
        ...
        if (directScheduledTaskRequest && this.createScheduledTask) {
            return this.handleDirectScheduledTaskRequest(coworkSessionId, message, formattedContent, directScheduledTaskRequest);
        }
        ...
        const onSessionStartError = (error) => {
     ...
     */
    async getOrCreateCoworkSession(imConversationId, platform, forceNewSession = false, senderId, message) {
        if (forceNewSession) {
        ...
        if (existing) {
            ...
            if (!session) {
                console.warn(`[IMCoworkHandler] Found stale mapping for ${platform}:${imConversationId}, session ${existing.coworkSessionId} is missing`);
                this.imStore.deleteSessionMapping(imConversationId, platform);
    ...
    async createCoworkSessionForConversation(imConversationId, platform, senderId, message) {
        // Create new Cowork session
        ...
        const selectedWorkspaceRoot = (this.coworkStore.getAgent(agentId)?.workingDirectory?.trim()
            || config.workingDirectory
            || '').trim();
        if (!selectedWorkspaceRoot) {
            throw new Error('IM 工作目录未配置，请先在应用中选择任务目录。');
        }
        const resolvedWorkspaceRoot = path_1.default.resolve(selectedWorkspaceRoot);
        if (!fs_1.default.existsSync(resolvedWorkspaceRoot) || !fs_1.default.statSync(resolvedWorkspaceRoot).isDirectory()) {
            throw new Error(`IM 工作目录不存在或无效: ${resolvedWorkspaceRoot}`);
        }
        const session = this.coworkStore.createSession(title, resolvedWorkspaceRoot, systemPrompt, config.executionMode || 'local', [], agentId);
        // Save mapping
     ...
     */
    buildSessionTitle(platform, _imConversationId, senderId, message) {
        if (platform === 'nim') {
            const nimLabel = (0, i18n_1.t)('channelPrefixNim');
            if (message?.chatSubType === 'qchat') {
                const channelLabel = message.groupName || _imConversationId;
                return `${nimLabel}-${(0, i18n_1.t)('nimQChat')}-${channelLabel}`;
            }
            if (message?.chatType === 'group') {
                const groupLabel = message.groupName || senderId || _imConversationId;
                return `${nimLabel}-${(0, i18n_1.t)('nimGroup')}-${groupLabel}`;
            }
    ...
    async buildSystemPromptWithSkills() {
        ...
        const mediaInstruction = (0, imMediaInstruction_1.buildIMMediaInstruction)(imSettings);
        const sections = [];
        if (systemPrompt) {
    ...
    async handleDirectScheduledTaskRequest(sessionId, message, formattedContent, request) {
    ...
    isRecoverableApi400Error(error) {
        const message = (error instanceof Error ? error.message : String(error)).toLowerCase();
        if (!message.includes('400')) {
            return false;
        }
        return (message.includes('api error')
    ...
    shouldRetryWithFreshSession(error, message) {
     ...
     */
    handleMessageUpdate(sessionId, messageId, content) {
        // Only process updates from IM sessions
        if (!this.ensureTrackedSession(sessionId))
            return;
    ...
    createConversationKey(conversationId, platform) {
    ...
    ensureBackgroundAccumulator(sessionId) {
    ...
    rejectAccumulator(sessionId, error) {
        const accumulator = this.messageAccumulators.get(sessionId);
        if (!accumulator)
            return;
        this.cleanupAccumulator(sessionId);
    ...
    clearPendingPermissionByKey(key) {
    ...
    clearPendingPermissionsBySessionId(sessionId) {
        const keysToRemove = [];
    ...
    buildIMPermissionPrompt(request) {
    ...
    buildAllowPermissionResult(request) {
    ...
    async handlePendingPermissionReply(message) {
        ...
        this.coworkRuntime.respondToPermission(pending.request.requestId, this.buildAllowPermissionResult(pending.request));
        return responsePromise;
    }
    /**
    ...
    handleComplete(sessionId) {
        ...
        if (accumulator.backgroundDelivery) {
            ...
            if (!(0, imScheduledTaskHandler_1.isReminderSystemTurn)(messages)) {
                console.log('[IMCoworkHandler] not a reminder system turn, skipping async reply');
                return;
            }
            void this.sendAsyncReply(accumulator.backgroundDelivery.platform, accumulator.backgroundDelivery.conversationId, replyText).then((sent) => {
                if (!sent) {
    ...
    handleSessionStopped(sessionId) {
        ...
        const partialReply = this.formatReply(sessionId, accumulator.messages);
        this.cleanupAccumulator(sessionId);
        accumulator.resolve?.(partialReply || (0, i18n_1.t)('imSessionStoppedReply'));
    }
    /**
     * Clean up accumulator
     */
    cleanupAccumulator(sessionId) {
        const accumulator = this.messageAccumulators.get(sessionId);
    ...
    selectCurrentTurnMessages(accumulator, storeMessages) {
    ...
    selectStoreMessagesByAccumulatorIds(accumulator, storeMessages) {
    ...
    selectStoreMessagesByTurnBoundary(accumulator, storeMessages) {
        const boundaryIndex = this.findCurrentTurnBoundary(accumulator, storeMessages);
        if (boundaryIndex < 0) {
            return [];
        }
        const scopedMessages = storeMessages.slice(boundaryIndex).filter((message) => (message.type === 'user'
            || message.type === 'system'
    ...
    findCurrentTurnBoundary(accumulator, storeMessages) {
        ...
        if (accumulatorBoundary) {
            ...
            for (let index = storeMessages.length - 1; index >= 0; index -= 1) {
                ...
                if (!this.isInputMessage(candidate))
                    continue;
                if (candidate.type !== accumulatorBoundary.type)
                    continue;
    ...
    hasTurnOutputMessages(messages) {
        return messages.some((message) => (message.type === 'assistant'
            || message.type === 'tool_use'
            || message.type === 'tool_result'));
    }
    hasVisibleAssistantMessage(messages) {
        return messages.some((message) => (message.type === 'assistant'
            && !message.metadata?.isThinking
            && Boolean((0, imReplyGuard_1.stripThinkingBlocks)(message.content))));
    }
    normalizeBoundaryContent(content) {
     ...
     */
    formatReplyRaw(messages) {
        const parts = [];
        for (const message of messages) {
            if (message.type === 'assistant' && message.content && !message.metadata?.isThinking) {
                const text = (0, imReplyGuard_1.stripThinkingBlocks)(message.content);
                if (text)
                    parts.push(text);
            }
        }
        return parts.join('\n\n') || imReplyGuard_1.DEFAULT_IM_EMPTY_REPLY;
    }
    /**
     * Format accumulated messages into a reply string
     */
    formatReply(sessionId, messages) {
        const analysis = (0, imReplyGuard_1.analyzeIMReply)(messages);
     ...
     */
    formatMessageWithMedia(message) {
        ...
        if (message.attachments && message.attachments.length > 0) {
            const mediaInfo = message.attachments.map((att) => {
...
exports.IMCoworkHandler = IMCoworkHandler;
//# sourceMappingURL=imCoworkHandler.js.map