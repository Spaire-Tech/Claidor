...
const ManualCredentialCaptureChannel = {
  Event: "lobster:browser-credential:capture-event"
};
const ManualCredentialCaptureEventType = {
  Submitted: "submitted",
  PageState: "page-state"
};
const ManualCredentialFormKind = {
  Login: "login",
  Registration: "registration"
};
const USERNAME_HINT_PATTERN = /(?:user(?:name)?|login|email|mail|account|identifier|userid|user-id)/i;
const RECENT_CAPTURE_WINDOW_MS = 1500;
const PAGE_STATE_DEBOUNCE_MS = 150;
const recentForms = /* @__PURE__ */ new WeakMap();
let pageStateTimer;
...
const findUsername = (form, passwordInput) => {
  const candidates = Array.from(form.querySelectorAll("input")).filter((input) => !input.disabled && !input.readOnly && isTextLikeInput(input));
  ...
  const preferred = pool.find((input) => /(?:^|\s)(?:username|email)(?:\s|$)/i.test(input.autocomplete)) ?? pool.find((input) => input.type.toLowerCase() === "email") ?? pool.find((input) => USERNAME_HINT_PATTERN.test([
    input.name,
    input.id,
    input.placeholder,
    input.getAttribute("aria-label") ?? ""
  ].join(" "))) ?? pool.at(-1);
  return (preferred == null ? void 0 : preferred.value.trim()) ?? "";
};
const captureForm = (form) => {
  ...
  const passwordInputs = Array.from(form.querySelectorAll('input[type="password"]')).filter((input) => !input.disabled && !input.readOnly && input.value.length > 0);
  ...
  const event = {
    ...
    username,
    password,
    formKind: passwordInputs.length > 1 ? ManualCredentialFormKind.Registration : ManualCredentialFormKind.Login
  };
  electron.ipcRenderer.send(ManualCredentialCaptureChannel.Event, event);
};
const isVisiblePasswordInput = (input) => {
  ...
  return style.display !== "none" && style.visibility !== "hidden" && style.opacity !== "0" && input.getClientRects().length > 0;
};
const hasPasswordField = () => Array.from(
  document.querySelectorAll('input[type="password"]')
).some(isVisiblePasswordInput);
const emitPageState = () => {
  pageStateTimer = void 0;
...
window.addEventListener("click", (event) => {
  ...
  if (!target) return;
  const type = target instanceof HTMLButtonElement ? (target.getAttribute("type") ?? "submit").toLowerCase() : target.type.toLowerCase();
