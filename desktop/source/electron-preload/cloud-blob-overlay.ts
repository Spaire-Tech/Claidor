...
svg[viewBox="${CLOUD_BLOB_VIEWBOX}"]:not([${CAISRA_CLOUD_BLOB_ATTR}]),
svg[viewBox="${LEGACY_CLOUD_BLOB_VIEWBOX}"]:not([${CAISRA_CLOUD_BLOB_ATTR}]) { visibility: hidden !important; }
[${CAISRA_CLOUD_BLOB_ATTR}] {
  display: block;
  width: 100%;
  height: 100%;
...
[${CAISRA_CLOUD_BLOB_ATTR}] .caisra-cloud-blob__eye {
  transform-box: fill-box;
  transform-origin: center;
  animation: caisra-cloud-blink 5.4s ease-in-out infinite;
}
[${CAISRA_CLOUD_BLOB_ATTR}] .caisra-cloud-blob__eye[data-eye="right"] {
  animation-delay: 0.04s;
}
[${CAISRA_CLOUD_BLOB_ATTR}] .caisra-cloud-blob__pupil {
  animation: caisra-cloud-gaze 6.2s ease-in-out infinite;
}
[${CAISRA_CLOUD_BLOB_ATTR}][data-sleeping="true"] .caisra-cloud-blob__eye,
[${CAISRA_CLOUD_BLOB_ATTR}][data-sleeping="true"] .caisra-cloud-blob__pupil {
  animation: none;
}
@media (prefers-reduced-motion: reduce) {
  [${CAISRA_CLOUD_BLOB_ATTR}] .caisra-cloud-blob__face,
  [${CAISRA_CLOUD_BLOB_ATTR}] .caisra-cloud-blob__eye,
  [${CAISRA_CLOUD_BLOB_ATTR}] .caisra-cloud-blob__pupil { animation: none; }
}
`;
