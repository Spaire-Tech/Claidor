...
export function createSharedInstalledPluginsLoader(deps: SharedInstalledPluginsLoaderDeps): () => Promise<LoadedPlugins> {
  ...
  const resolveCurrentUserId = async (): Promise<number | null> => {
    ...
    catch (error) { const sentence = error instanceof Error ? error.message : String(error); deps.log(`[sand:plugin-skills] could not resolve the signed-in user: ${sentence}`); logHostLine(`${HOST_LOG_PREFIX} plugins GetMe failed: ${clipForHostLog(sentence)}`); }
  ...
  return async () => {
    ...
    for (const failure of result.failures) { deps.log(`[sand:plugin-skills] plugin ${failure.pluginName} failed to load: ${failure.errorMessage}`); logHostLine(`${HOST_LOG_PREFIX} plugins load failed: plugin=${failure.pluginName} ${clipForHostLog(failure.errorMessage)}`); }
    if (result.sourceUnavailable) logHostLine(`${HOST_LOG_PREFIX} plugins listing unavailable: GetEffectiveUserPlugins failed (the daily sync keeps the last index)`);
...
export class SandPluginSkillsService {
  ...
  private async runPass(trigger:string):Promise<PluginSkillRecord[]>{const started=(this.options.now??Date.now)();try{const loaded=await this.options.load();if(this.disposed)return this.current();const previous=this.currentIndex();let records=pluginContentsToSkillRecords(loaded.plugins,loaded.publisherFacts),listed=new Set(loaded.listedPluginIds),loadedIds=new Set(loaded.plugins.flatMap((plugin)=>plugin.loadError!=null||!plugin.installPath?[]:(id=>id==null?[]:[id])(getPluginDbId(plugin.identifier))));records=[...records,...(previous?.skills??[]).filter((record)=>listed.has(record.pluginId)&&!loadedIds.has(record.pluginId))];writePluginSkillsCache(getPluginSkillsDir(this.options.sandRootDir),{currentUserId:loaded.currentUserId??previous?.currentUserId??null,skills:records,authBlocked:loaded.authBlocked});(this.options.prune??pruneUninstalledPluginDirs)(join(getPluginsRootDir(this.options.sandRootDir),"cache"),loaded.listedCacheKeys,records.map((record)=>record.filePath));const changed=previous==null?records.length>0:skillRecordsIdentity(previous.skills)!==skillRecordsIdentity(records);this.options.reportSync?.({trigger,outcome:"ok",changed,skillCount:records.length,durationMs:(this.options.now??Date.now)()-started});
// One line per pass in the box's `/tmp/sand-host.log` (25 September 2026): the sync runs against Simeon Labs' server now.
logHostLine(`${HOST_LOG_PREFIX} plugins sync=${trigger} skills=${records.length} plugins=${loaded.listedPluginIds.length} changed=${changed}`);return records}catch(error){const sentence=error instanceof Error?error.message:String(error);this.options.log?.(`[sand:plugin-skills] sync (${trigger}) failed: ${sentence}`);
// Callers catch (`startPluginSkillsWhenAuthenticated`, `syncBestEffort`); nothing here reaches a turn.
logHostLine(`${HOST_LOG_PREFIX} plugins sync=${trigger} failed: ${clipForHostLog(sentence)}`);this.options.reportSync?.({trigger,outcome:"failed",errorClass:error instanceof Error?error.name:typeof error,durationMs:(this.options.now??Date.now)()-started});throw error}}
}
