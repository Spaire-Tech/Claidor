...
export class SandSessionMaterialization {
  ...
  async materializeSession(agentId: string, profile?: Partial<SandAgentProfile>, origin: "user" | "dev" = "user", purpose?: string): Promise<MaterializedSession> {
    ...
    try {
      ...
      writeSandProfileFile(getSandProfilePath(dirname(dbPath)), { name: profile?.name?.trim() || "Grok", description: profile?.description?.trim() ?? "", title: profile?.title?.trim() ?? "", avatarShape: profile?.avatarShape?.trim() ?? "", avatarColor: profile?.avatarColor?.trim() ?? "", modelRoute: profile?.modelRoute?.trim().toLowerCase() ?? "" });
  ...
  async openSession(agentId: string): Promise<MaterializedSession> {
    ...
    try {
      ...
      try { await stat(profilePath); } catch { writeSandProfileFile(profilePath, { name: db.get("name") || "Grok", description: db.getSandProfile().description, title: "", avatarShape: "", avatarColor: "", modelRoute: "" }); }
      const session = this.compose(agentId, dbPath, db);
