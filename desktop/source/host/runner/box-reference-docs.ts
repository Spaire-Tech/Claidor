...
export const SAND_BOX_DEBUGGING_REFERENCE_DOC = [
  ...
  `- Next steps: retry first, since most failures are just a box still booting. You can't rebuild the box yourself, so if it's wedged or stuck on a stale image, surface a clear status and tell the user to recover it from Settings → Updates tab → "Update Caisra's Computer" (its button says "Update") — it moves the box to a fresh instance while keeping files and logins, and can unstick a wedged box without data loss. That is the recovery action to point users at; the "Reset Caisra's Computer" row below it restores from the last saved snapshot and can lose recent unsynced work, so never direct the user to it. request_box_help is for handing the user a manual step on a working desktop (a login or captcha), not a repair tool.`,
  "",
].join("\n");

export const SAND_APP_UI_REFERENCE_DOC = [
  "# The Caisra app UI (real paths — never invent others)",
  "",
  `A compact map of Caisra's real interface so you can guide the user or self-recover. Use only what's listed here; for anything else, follow "Never fabricate data" and say you're unsure rather than inventing a path.`,
  ...
  `- Updates: box recovery is "Update Caisra's Computer" (its button says "Update"; data-preserving — it moves the box to a fresh instance while keeping files and logins), a two-click confirm ("Click Again to Confirm"). The "Reset Caisra's Computer" row (button "Reset") is the destructive recovery of last resort: it restores from the last saved snapshot and can lose recent unsynced work, so steer users to Update instead. Updates also has "Update Track" (Stable / Nightly) and "Check for Updates", which update the Caisra app itself, distinct from "Update Caisra's Computer" (which recreates the box).`,
