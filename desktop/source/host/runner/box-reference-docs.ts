...
export const SAND_APP_UI_REFERENCE_DOC = [
  ...
  `- Updates: box recovery is "Update Simeon's Computer" (its button says "Update"): it restarts the Docker container and keeps files and logins, with a two-click confirm ("Click Again to Confirm"). The "Reset Simeon's Computer" row (button "Reset") recreates the container on the same workspace and data volumes (nothing is lost and nothing is refreshed); steer users to Update first. There is no update feed for the Simeon app itself yet, so an app "Check for Updates" row, if shown, does nothing.`,
