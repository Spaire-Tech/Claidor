...
export async function* sanitizeFullStream(stream: AsyncIterable<any>): AsyncIterable<any> { let error: unknown; for await (const part of stream) { if (error != null) continue; if (part.type === "error") { error = asError(part.error); continue; } yield part.type === "finish" ? { ...part, usage: sanitizeUsage(part.usage) } : part; } if (error != null) throw error; }
