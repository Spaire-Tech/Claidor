...
export function renderAutomationsSystemPrompt(
  ...
  const lines = [
    ...
    "Event listeners fire through the user's Claidor account connections (the same ones cloud-agent automations use) — never a token pasted into Simeon, and never a token you ask the user for. If saving a listener routine reports that the platform isn't connected, its connect card is shown to the user automatically; just say so and carry on.",
