export const GROUP_CONFIG_VERSION = 1; export const GROUP_MAX_MEMBER_TURNS = 10; export const GROUP_MAX_ROUNDS = 2; export const GROUP_PROMPT_HISTORY_LIMIT = 24; export const GROUP_MAX_MESSAGES_PER_TURN = 2; export const SHARED_ROOM_HISTORY_LIMIT = 24; export const GROUP_CHAT_TAG_PREFIX = "[Group chat: "; export const SAND_HIDDEN_PROMPT_MARKER = "[SAND_HIDDEN_PROMPT]";
...
export function nextUnmentionedSpeaker<T extends Pick<GroupMember, "id">>(members: readonly T[], history: readonly GroupMessage[]): T | undefined {
  if (members.length === 0) return undefined;
  for (let index = history.length - 1; index >= 0; index -= 1) {
    const speaker = history[index]?.speaker;
    if (speaker?.kind === "member") {
      const at = members.findIndex((member) => member.id === speaker.id);
...
export function resolveResponders<T extends Pick<GroupMember, "id" | "name">>(members: readonly T[], history: readonly GroupMessage[]): T[] {
  let start = 0;
  for (let index = history.length - 1; index >= 0; index -= 1) {
    if (history[index]?.speaker.kind === "user") { start = index; break; }
  }
  let everyone = false;
  ...
  if (mentioned.size > 0) return members.filter((member) => mentioned.has(member.id));
  const one = nextUnmentionedSpeaker(members, history);
  return one == null ? [] : [one];
}
...
export function buildGroupMemberSystemPrompt(member: GroupMember, group: GroupDescription, peers: readonly GroupMember[], options: { isSharedRoom?: boolean } = {}): string { const lines = [`You are ${member.name}, one participant in a group chat (${describeGroup(group)}).`]; if (member.description.trim()) lines.push(`Your persona: ${member.description.trim()}`); if (peers.length > 0) lines.push("", "Other participants in the room:", ...peers.map((peer) => `- ${peer.name}${peer.description.trim() ? ` (${peer.description.trim()})` : ""}`)); lines.push("", peers.length > 0 ? `Right now you are speaking in this group chat, with ${peers.map((peer) => peer.name).join(", ")}.` : "Right now you are speaking in this group chat.", "This is a conversation, not a work turn. Write only the words the room will read. No tools. Keep it short.", "", `Stay fully in character as ${member.name}. If you have nothing new worth adding, reply exactly \"(pass)\". Never reveal private one-on-one context.`); return lines.join("\n"); }
