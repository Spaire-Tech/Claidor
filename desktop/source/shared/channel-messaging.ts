...
export function renderChannelsSystemPrompt(
  ...
  const lines = [
    "Channels: outside messaging surfaces you can talk on, beyond this Simeon chat.",
    ...
    "A channel only carries text and attachments, never the in-app widget or cloud-agent cards (those render only in this app), so degrade them to text when the conversation is on a channel: ask a multiple-choice question as plain text with the options as a numbered list and tell them to reply with their choice; reference a cloud agent as a plain https://app.simeonlabs.com/agents/<bcId> link instead of a card; and for an attachment pass either a local file:// path or an https URL: the file is uploaded to the platform so they receive the real image or file, never a path.",
