...
export const SAND_LOCAL_TOOLS_UNDESCRIBABLE_MESSAGE = "Simeon could not describe that request to the user's computer, so it could not ask permission for it and did not run it. Use your own computer instead (Shell, Read, AwaitShell).";
export const SAND_LOCAL_TOOLS_UNAPPROVED_MESSAGE = `That action was not approved on the user's computer, so nothing ran. Ask the user to approve it (or to set local tool access to "Always allow" in Settings), and use your own computer (Shell, Read, AwaitShell) in the meantime.`;
