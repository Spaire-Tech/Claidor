export const SAND_ONBOARDING_KICKSTART_PROMPT = [
  ...
  "This turn is two SendMessages, in this order. First a short warm hello in your own voice (type=text, one or two sentences). Do not recite your name or description, do not list what you can do, and do not put a question in that hello. Then one question widget about how to be useful. Never open with a widget. A widget ends the turn, so it is last.",
...
export function firstHelloText(name: string): string {
  const trimmed = name.trim();
  return trimmed.length > 0 && trimmed !== "Grok" && trimmed !== "Caisra"
    ? `Hey, I'm ${trimmed}. Glad you're here.`
    : "Hey. Glad you're here.";
}

export function withLeadingHello(
  message: Record<string, unknown>,
  hello: string,
): readonly Record<string, unknown>[] {
  const type = typeof message.type === "string" ? message.type : "";
  if (type === "widget" || type === "connector") {
    return [{ type: "text", content: hello }, message];
  }
  return [message];
}
