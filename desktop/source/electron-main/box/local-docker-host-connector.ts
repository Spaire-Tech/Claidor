...
export async function getLocalDockerStatus(settingsPath: string): Promise<LocalDockerStatus> {
  ...
  if (!inspected.owned) return { available: true, running: inspected.running, ready: false, containerName: LOCAL_DOCKER_BOX_CONTAINER, image: inspected.image, detail: `Container ${LOCAL_DOCKER_BOX_CONTAINER} exists but is not owned by Caisra.` };
...
async function ensureLocalDockerBox(settingsPath: string, inferenceCredential?: InferenceCredential): Promise<GatewayConnection> {
  ...
  if (current.exists && !current.running) {
    ...
    const created = await runDocker([
      ...
      // The box must never fall back to DEFAULT_CURSOR_BACKEND_URL: getConfiguredBackendUrl
      // inside the container would then resolve to api2.cursor.sh and the credential
      // renewer would send this account's bearer token there. Pin it unconditionally,
      // not only when the optional 3s credential race is won.
      "--env", `SAND_BACKEND_URL=${getConfiguredBackendUrl()}`,
