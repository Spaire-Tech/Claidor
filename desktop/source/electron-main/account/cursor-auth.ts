...
export class SandAuthSignInRequiredError extends Error { constructor() { super("Sign in to Cursor to run Caisra."); } }
export class SandAuthSignInExpiredError extends Error { constructor() { super("Cursor sign-in expired. Sign in again to run Caisra."); } }
...
const RETAINED_AFTER_FAILED_LOGOUT_STATUS = { kind: "logged-out", errorMessage: "Caisra couldn't remove the saved Cursor sign-in. The account may return after Caisra restarts. Sign in to try again." } as const;
const LOGGED_OUT_STATUS = { kind: "logged-out" } as const;
const SIGN_IN_CONFIRMATION_FAILED_STATUS = { kind: "logged-out", errorMessage: "Caisra couldn't confirm your Cursor sign-in. Restart Caisra or sign in again." } as const;
const SIGN_IN_EXPIRED_STATUS = { kind: "logged-out", errorMessage: "Cursor sign-in expired. Sign in again to run Caisra." } as const;
...
const ACCOUNT_REFUSED_CREDENTIALS_RETAINED_STATUS = { kind: "logged-out", errorMessage: "This computer is linked to another Cursor account. Caisra couldn't remove the saved Cursor sign-in, so the account may return after restart. Sign in with the linked account to continue." } as const;
