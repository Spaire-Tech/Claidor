...
export async function createRoutedMcpBridge(deps: {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      if (message.method === "initialize") { reply({ protocolVersion: "2025-03-26", capabilities: { tools: { listChanged: false } }, serverInfo: { name: "simeon-plugins", version: "1" } }); return; }
      ...
      if (message.method === "tools/call") {
        ...
        if (selected == null) { reply({ isError: true, content: [{ type: "text", text: `Unknown Simeon plugin tool: ${String(name)}` }] }); return; }
