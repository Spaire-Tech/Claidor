export function noRepositoryAccessBullets(): [string, string] {
  return [
    ...
    `If the task requires reading or modifying code in a repository, explain that this conversation runs without repository access and suggest starting the agent from a surface with repository access (for example Simeon's cloud agents page, app.simeonlabs.com/agents) instead.`,
  ];
}
