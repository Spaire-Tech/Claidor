...
export function CreatingPullRequestsSection({
  ...
  return jsxs("section", {
    ...
    children: [
      ...
      enablePrCreationForgeGuidance && jsx("p", { children: "If this conversation already includes preferred pull-request host guidance that names a create command (`gh pr create` or `origin pr create` for Origin — a PR host, not the git remote named `origin`), that guidance OVERRIDES the `gh pr create` steps and example below. If using `gt`, pass `--github` or `--origin`. Keep using `gh` for other GitHub tasks (issues, checks, releases) unless that guidance says otherwise." }),
