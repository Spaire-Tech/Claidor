...
describe('ask-user-question session key gating', () => {
  test('allows desktop sessions across agents', () => {
    expect(isAskUserQuestionCandidateSessionKey('agent:main:swen:session-1')).toBe(true);
    expect(isAskUserQuestionCandidateSessionKey('agent:qa-reviewer:swen:session-2')).toBe(true);
  });
  ...
  test('allows legacy desktop sessions', () => {
    expect(isAskUserQuestionCandidateSessionKey('swen:session-3')).toBe(true);
  });
  ...
  test('rejects channel and malformed session keys', () => {
    ...
    expect(isAskUserQuestionCandidateSessionKey('agent::swen:session-4')).toBe(false);
    expect(isAskUserQuestionCandidateSessionKey('agent:qa-reviewer:swen:')).toBe(false);
