...
import {
  advanceSignedInOnboarding, canSubmitOnboardingDraft, CHARACTER_SHAPES, COMPUTER_DEMO_FRAMES, computerDemoFrame,
...
export function SignedInOnboardingStep({ step, draft, dailyTools, meetBeat = 0, demoBeat = 0, jobsBeat = 0, error = null, computer = EMPTY_COMPUTER, isAvatarLanded = false, renderCharacter, onAvatarSlotAttached, onDraftChange, onDailyToolsChange, onBack, onForward, onCreate }: {
  ...
  if (step === "meet") return <StepLayout step={step} title="Meet Simeon" footer={<Footer onForward={onForward} />}><div aria-hidden="true" className="sand-onboarding__composer"><p>{typedMeetText(meetBeat)}{meetBeat >= ONBOARDING_MEET_DELAY_BEATS && meetBeat < ONBOARDING_MEET_DELAY_BEATS + MEET_TYPED_TEXT.length ? <span style={{ borderLeft: "1px solid currentColor", display: "inline-block", height: "1em" }} /> : null}</p><span><button aria-label="Attach" tabIndex={-1} type="button">+</button><button aria-label="Send" onClick={onForward} tabIndex={-1} type="button">↑</button></span></div></StepLayout>;
  if (step === "computer-demo") return <StepLayout step={step} title="Simeon has its own computer and works just like you" footer={<Footer onBack={onBack} onForward={onForward} />}><ComputerDemo beat={demoBeat} /></StepLayout>;
  if (step === "jobs") return <StepLayout step={step} title="Give each Agent a job" footer={<Footer onBack={onBack} onForward={onForward} />}><div aria-hidden="true">{ONBOARDING_JOBS.map((job) => <span className="sand-onboarding__job-bubble" key={job.id} style={{ opacity: jobsBeat >= 2 ? 1 : 0, transform: `translate(${job.x}px, ${job.y + ONBOARDING_JOB_BUBBLE_Y_OFFSET}px) translate(-50%, 0)` }}>{job.label}</span>)}</div></StepLayout>;
  ...
  if (step === "create") return <section className="sand-onboarding__create"><aside aria-label="Sidebar preview"><div aria-hidden="true" /><div><button aria-label="Create your first Agent" className="sand-agent-item" disabled type="button"><strong>Create your first Agent</strong></button><p>No chats yet</p></div></aside><div><header><h1 id={`sand-onboarding-${step}-heading`}>New Agent</h1></header><div style={{ display: "flex", flexDirection: "column", gap: 12, padding: 24 }}>
    ...
    <div aria-label="Avatar" role="radiogroup" style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>{CHARACTER_SHAPES.map((shape) => <button aria-checked={normalizedDraft.shape === shape} aria-label={`Avatar ${shape.replace("adventurer-", "")}`} key={shape} onClick={() => onDraftChange({ ...normalizedDraft, shape })} role="radio" title={shape} type="button">{(renderCharacter ?? defaultOnboardingCharacterRenderer)({ color: normalizedDraft.color, shape, sizePx: 36, state: "idle", paused: true, sourceId: `sand-onboarding-avatar-${shape}` })}</button>)}</div>
    <label htmlFor="sand-onboarding-create-name">Name</label><input id="sand-onboarding-create-name" onChange={(event) => onDraftChange({ ...normalizedDraft, name: event.currentTarget.value, pickedTemplateId: null })} placeholder="New Agent" spellCheck={false} type="text" value={normalizedDraft.name} /><button disabled={!canSubmitOnboardingDraft(normalizedDraft)} onClick={onCreate} type="button">Get started</button>
  ...
  return <div className="sand-onboarding__hand-off"><span aria-hidden="true"><img alt="" className="sand-1lliihq sand-2lah0s sand-47corl sand-87ps6o sand-10xuot4" draggable={false} height={APP_ICON_SIZE_PX} src={APP_ICON_URL} width={APP_ICON_SIZE_PX} /><span>Simeon</span></span><p aria-live="polite" id={`sand-onboarding-${step}-heading`}>{error == null ? handOffStatus(computer) : "Simeon couldn’t finish setting up"}</p>{error == null ? null : <><p>{error}</p><button autoFocus onClick={onCreate} type="button">Try again</button></>}</div>;
}
