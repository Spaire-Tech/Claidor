...
import {
  advanceSignedInOnboarding, canSubmitOnboardingDraft, CHARACTER_COLORS, COMPUTER_DEMO_FRAMES, computerDemoFrame,
  createOnboardingDraft, createOnboardingHandOffDwell, DAILY_TOOLS, filterDailyTools, handOffStatus, makeOnboardingCreateRequest,
...
export function SignedInOnboardingStep({ step, draft, dailyTools, meetBeat = 0, demoBeat = 0, jobsBeat = 0, error = null, computer = EMPTY_COMPUTER, isAvatarLanded = false, renderCharacter, onAvatarSlotAttached, onDraftChange, onDailyToolsChange, onBack, onForward, onCreate }: {
  ...
  if (step === "meet") return <StepLayout step={step} title="Meet Caisra" footer={<Footer onForward={onForward} />}><div aria-hidden="true" className="sand-onboarding__composer"><p>{typedMeetText(meetBeat)}{meetBeat >= ONBOARDING_MEET_DELAY_BEATS && meetBeat < ONBOARDING_MEET_DELAY_BEATS + MEET_TYPED_TEXT.length ? <span style={{ borderLeft: "1px solid currentColor", display: "inline-block", height: "1em" }} /> : null}</p><span><button aria-label="Attach" tabIndex={-1} type="button">+</button><button aria-label="Send" onClick={onForward} tabIndex={-1} type="button">↑</button></span></div></StepLayout>;
  if (step === "computer-demo") return <StepLayout step={step} title="Caisra has its own computer and works just like you" footer={<Footer onBack={onBack} onForward={onForward} />}><ComputerDemo beat={demoBeat} /></StepLayout>;
  ...
  if (step === "create") return <section className="sand-onboarding__create"><aside aria-label="Sidebar preview"><div aria-hidden="true" /><div><button aria-label="Create your first Bot" className="sand-agent-item" disabled type="button"><strong>Create your first Bot</strong></button><p>No chats yet</p></div></aside><div><header><h1 id={`sand-onboarding-${step}-heading`}>New Bot</h1></header><div style={{ display: "flex", flexDirection: "column", gap: 12, padding: 24 }}>
    ...
    <div aria-label="Character color" role="radiogroup">{CHARACTER_COLORS.map((color) => <button aria-checked={normalizedDraft.color === color.id} aria-label={`${color.label} color`} key={color.id} onClick={() => onDraftChange({ ...normalizedDraft, color: color.id, shape: "cloud" })} role="radio" style={{ backgroundColor: color.value }} title={color.label} type="button" />)}</div>
  ...
  return <div className="sand-onboarding__hand-off"><span aria-hidden="true"><img alt="" className="sand-1lliihq sand-2lah0s sand-47corl sand-87ps6o sand-10xuot4" draggable={false} height={APP_ICON_SIZE_PX} src={APP_ICON_URL} width={APP_ICON_SIZE_PX} /><span>Caisra</span></span><p aria-live="polite" id={`sand-onboarding-${step}-heading`}>{error == null ? handOffStatus(computer) : "Caisra couldn’t finish setting up"}</p>{error == null ? null : <><p>{error}</p><button autoFocus onClick={onCreate} type="button">Try again</button></>}</div>;
}
