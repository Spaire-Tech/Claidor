...
const UPDATE_UNTRACKABLE_COPY = "The computer update started, but Caisra can't track its progress. Restart Caisra after the computer is available again.";
...
function workingDescription(names: readonly string[]): string {
  if (names.length > 1) {
    return `${formatWorkingAgents(names)} are working on Caisra's computer right now. Updating recreates the computer and interrupts their current turns. Files and logins are kept.`;
  }
...
export function projectComputerUpdateConfirmationContent(
  ...
  if (action === "ready") {
    return {
      title: "Update Caisra's Computer?",
      ...
      confirmLabel: "Update Caisra's Computer",
      cancelLabel: "Not now",
      width: "wide"
    };
  }
