...
// reproducible from any terminal state (see SWEN_WEB_* handling below).
...
function webPackageFileName(version) {
  return `swen-${version}-x64.nsis.7z`;
}

function webSetupFileName(version, keyfrom, silentOnDoubleClick) {
  return `Swen-WebSetup-x64-${version}-${keyfrom}${silentOnDoubleClick ? '-silent' : ''}.exe`;
}
...
Modes:
  ...
  --pkg-base-url  one-pass build; the installer downloads <dir>/<keyfrom>/swen-<version>-x64.nsis.7z
