...
// changes unless MATIES_WEB_INSTALLER=1 is set explicitly.
  ...
  if (!raw) {
    throw new Error(
      `[WebInstaller] either ${WEB_PKG_URL_ENV} (exact package URL from object storage) or ` +
        `${WEB_PKG_BASE_URL_ENV} (CDN base directory, e.g. https://cdn.example.com/maties/win) ` +
...
// Sign every Windows binary electron-builder produces (Maties.exe, the
...
config.dmg = {
  ...(config.dmg || {}),
  artifactName: `Maties-darwin-\${arch}-\${version}-${keyfrom}.\${ext}`,
};

config.nsis = {
  ...(config.nsis || {}),
  artifactName: `Maties-Setup-\${arch}-\${version}-${keyfrom}${silentOnDoubleClick ? '-silent' : ''}.\${ext}`,
};
...
if (isWebInstallerEnabled()) {
  ...
  config.nsisWeb = {
    ...
    artifactName: `Maties-WebSetup-\${arch}-\${version}-${keyfrom}${silentOnDoubleClick ? '-silent' : ''}.\${ext}`,
  };
