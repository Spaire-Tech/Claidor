...
// Lightweight setup entry for deferred loading (patched by Maties).
...
function patchFilenameEncoding(larkPluginDir, log) {
  ...
  const replacement = `fileName = decodeURIComponent(match[1].trim());
                // Patched by Maties: fix Latin-1 garbled UTF-8 filenames from Feishu API
  ...
  if (fnIdx !== -1) {
    const helperFn = `// Patched by Maties: detect and fix Latin-1 garbled UTF-8 filenames.
