...
;
; 1. Swen.exe -- the main app AND the OpenClaw gateway (ELECTRON_RUN_AS_NODE)
; 2. node.exe whose binary lives inside the Swen install tree
...
!macro stopSwenProcesses
  DetailPrint "[Installer] Stopping running Swen processes"
  ...
  CreateDirectory "$APPDATA\Swen"
  StrCmp $lobsterTrustedPowerShellPath "" StopSwenProcessesDone
  ...
  ; metacharacters. Cleared at StopSwenProcessesLog, which every path
  ; reaches.
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_STOP_ROOT", t "$INSTDIR")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_STOP_SELF_PID", t "$lobsterCurrentProcessPid")i'
  Push '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "\
    $$root = $$env:SWEN_STOP_ROOT;\
    if ($$root -and -not $$root.EndsWith([char]92)) { $$root = $$root + [char]92 };\
    $$selfPid = $$env:SWEN_STOP_SELF_PID;\
    $$sweep = $$root -and $$root.Length -gt 3;\
    for ($$i = 0; $$i -lt 30; $$i++) {\
      $$procs = @();\
      $$procs += Get-Process -Name Swen -ErrorAction SilentlyContinue;\
      $$procs += Get-Process node -ErrorAction SilentlyContinue | Where-Object { $$_.Path -like \"*Swen*\" };\
  ...
  StrCmp $R2 "3" 0 StopSwenProcessesLog
  ...
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_STOP_LOG_PATH", t "$APPDATA\Swen\install-timing.log")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_STOP_ATTEMPT_ID", t "$lobsterInstallerAttemptId")i'
  Push '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "\
    ...
    foreach ($$p in $$procs) {\
      ...
      Add-Content -LiteralPath $$env:SWEN_STOP_LOG_PATH -Value \"$$ts phase=process-stop-survivor attempt_id=$$env:SWEN_STOP_ATTEMPT_ID name=$$($$p.ProcessName) pid=$$($$p.Id) path=$$fp\" -ErrorAction SilentlyContinue;\
    };\
    exit $$procs.Count"'
  !insertmacro LobsterExecHiddenExitCode
  Pop $1
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_STOP_LOG_PATH", t "")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_STOP_ATTEMPT_ID", t "")i'
  FileOpen $9 "$APPDATA\Swen\install-timing.log" a
  FileSeek $9 0 END
  ...
  !endif
  FileClose $9
  Goto StopSwenProcessesLog

  StopSwenProcessesDone:
  StrCpy $R2 "helper-not-found"

  StopSwenProcessesLog:
  ; Clearing variables that were never set (helper-not-found path) is a no-op.
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_STOP_ROOT", t "")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_STOP_SELF_PID", t "")i'
...
!macro customInit
  ...
  ${EndIf}
  !if "$%SWEN_CHANNEL_BUILD%" == "1"
  !if "$%SWEN_SILENT_ON_DOUBLE_CLICK%" == "1"
    ${If} ${Silent}
    ${Else}
      ${If} ${isUpdated}
      ${Else}
...
!ifndef BUILD_UNINSTALLER
  ...
  Function lobsterSelectPayloadStagingDir
    ...
    CreateDirectory "$INSTDIR\.swen-staging"
    IfFileExists "$INSTDIR\.swen-staging" 0 LobsterStagingRelocateCreateFailed
    StrCpy $appPackageStagingDir "$INSTDIR\.swen-staging"
    ...
    MessageBox MB_OK|MB_ICONEXCLAMATION "${U+78C1}${U+76D8}${U+7A7A}${U+95F4}${U+4E0D}${U+8DB3}${U+FF0C}${U+65E0}${U+6CD5}${U+5B89}${U+88C5} Swen${U+3002}${U+8BF7}${U+6E05}${U+7406}${U+78C1}${U+76D8}${U+7A7A}${U+95F4}${U+540E}${U+91CD}${U+8BD5}${U+3002}$\r$\n$\r$\nThere is not enough free disk space to install Swen: drive $4 has $2 MB free but staging the installation needs about $1 MB, and installing to drive $3 would need about $6 MB free there. Free up disk space and run the installer again. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
    SetErrorLevel 2
    Quit
  ...
  ; Remove a relocated staging directory ($INSTDIR\.swen-staging). Runs
  ...
  Function lobsterRollbackOldInstall
    ...
    LobsterRollbackRestoreSucceeded:
      ...
      System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_DEFENDER_TARGET", t "$lobsterOldInstallOriginalPath")i'
      Push '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "try { Remove-MpPreference -ExclusionPath $$env:SWEN_DEFENDER_TARGET -ErrorAction SilentlyContinue } catch {}"'
      !insertmacro LobsterExecHiddenToStack
      Pop $0
      Pop $1
      System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_DEFENDER_TARGET", t "")i'
      ...
      System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_FAILED_CLEANUP_PATH", t "$lobsterOldInstallFailedPath")i'
      ClearErrors
      StrCmp $lobsterTrustedPowerShellPath "" LobsterRollbackFailedTreeCleanupDone
      Push '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "Remove-Item -LiteralPath $$env:SWEN_FAILED_CLEANUP_PATH -Recurse -Force -ErrorAction SilentlyContinue"'
      !insertmacro LobsterExecHiddenDetached
      LobsterRollbackFailedTreeCleanupDone:
      System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_FAILED_CLEANUP_PATH", t "")i'
...
!macro customCheckAppRunning
  ...
  !ifndef BUILD_UNINSTALLER
    ...
    StrCmp $lobsterTargetProcessesStopStatus "success" TargetProcessesStopped
      ...
      FileClose $9
      MessageBox MB_OK|MB_ICONEXCLAMATION "The Swen update stopped before replacing the previous version because the old application processes could not be confirmed stopped. Please close Swen and retry. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
      SetErrorLevel 2
      Quit
    TargetProcessesStopped:

    ; -- Backup user-created skills to AppData before extraction overwrites them --
    ; Copy non-bundled skills to %APPDATA%\Swen\skills-backup\ so they are
    ...
    ClearErrors
    FileOpen $R0 "$APPDATA\Swen\skill-migrate.log" w
    ...
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_SKILL_SOURCE", t "$INSTDIR\resources\SKILLs")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_SKILL_BACKUP_ROOT", t "$APPDATA\Swen\skills-backup")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_INSTALL_ATTEMPT_ID", t "$lobsterInstallerAttemptId")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_OLD_VERSION", t "$4")i'
    Push '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "\
      $$ErrorActionPreference = \"Stop\";\
      $$src       = $$env:SWEN_SKILL_SOURCE;\
      $$root      = $$env:SWEN_SKILL_BACKUP_ROOT;\
      $$attempt   = $$env:SWEN_INSTALL_ATTEMPT_ID;\
      $$oldVer    = $$env:SWEN_OLD_VERSION;\
    ...
    Pop $1
    StrCpy $R2 $0
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_SKILL_SOURCE", t "")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_SKILL_BACKUP_ROOT", t "")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_INSTALL_ATTEMPT_ID", t "")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_OLD_VERSION", t "")i'
    System::Call 'kernel32::GetTickCount()i .r6'
    IntOp $5 $6 - $7
    ...
    FileSeek $9 0 END
    !insertmacro GetTimestamp $8
    FileWrite $9 "$8 phase=skill-backup-complete attempt_id=$lobsterInstallerAttemptId status=$lobsterLegacySkillsStatus exit=$R2 elapsed_ms=$5 backup=$APPDATA\Swen\skills-backup\$lobsterInstallerAttemptId$\r$\n"
    FileClose $9
    ...
    StrCmp $lobsterLegacySkillsStatus "legacy-backup-succeeded" 0 SkillBackupFailedAbort
      ...
      IfFileExists "$APPDATA\Swen\skills-backup\$lobsterInstallerAttemptId\backup-manifest.json" SkillBackupValidated
      StrCpy $lobsterLegacySkillsStatus "legacy-backup-verify-failed"
      FileOpen $9 "$APPDATA\Swen\install-timing.log" a
      FileSeek $9 0 END
      !insertmacro GetTimestamp $8
      FileWrite $9 "$8 phase=skill-backup-manifest-postcheck-missing attempt_id=$lobsterInstallerAttemptId manifest=$APPDATA\Swen\skills-backup\$lobsterInstallerAttemptId\backup-manifest.json$\r$\n"
      FileClose $9
    SkillBackupFailedAbort:
      ...
      FileClose $9
      Call lobsterTryRelaunchOldApp
      MessageBox MB_OK|MB_ICONEXCLAMATION "The Swen update stopped because legacy user skills could not be safely inspected or backed up (status=$lobsterLegacySkillsStatus). The previous installation was not replaced. Please retry the update. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
      SetErrorLevel 2
      Quit
    ...
    OldInstallRenameVerificationFailed:
      ...
      FileClose $9
      MessageBox MB_OK|MB_ICONEXCLAMATION "The Swen installation stopped because the previous installation move could not be verified and automatic recovery did not complete. No recovery copy was deleted. Restart Windows before retrying. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
      SetErrorLevel 3
      Quit
    ...
    OldInstallRenameVerificationRestored:
      ...
      FileClose $9
      MessageBox MB_OK|MB_ICONEXCLAMATION "The Swen installation stopped because the previous installation move could not be verified. The previous version was restored. Please retry the installation. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
      SetErrorLevel 2
      Quit
...
!ifndef BUILD_UNINSTALLER
  ...
  ; remove Swen exclusions at the end of their --updated flow.
  !macro customAfterUninstallOldVersions
    ...
    CreateDirectory "$INSTDIR"
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_INSTALL_ROOT", t "$INSTDIR")i'
    Push '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "\
      $$target = $$env:SWEN_INSTALL_ROOT;\
    ...
    Pop $1
    StrCpy $R2 $0
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_INSTALL_ROOT", t "")i'
...
!macro customBeforeRegistryAddInstallInfo
  ...
  ; Log file: %APPDATA%\Swen\install-timing.log

  CreateDirectory "$APPDATA\Swen"
  FileOpen $2 "$APPDATA\Swen\install-timing.log" a
  FileSeek $2 0 END
  !insertmacro GetTimestamp $8
  ...
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_WATCHDOG_MARKER_PATH", t "$PLUGINSDIR\lobster-watchdog-$lobsterInstallerAttemptId.marker")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_EXTRACTOR_EXE", t "$INSTDIR\${APP_EXECUTABLE_FILENAME}")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_EXTRACTOR_SCRIPT", t "$INSTDIR\resources\unpack-cfmind.cjs")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_EXTRACTOR_ARCHIVE", t "$INSTDIR\resources\win-resources.tar")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_EXTRACTOR_DESTINATION", t "$INSTDIR\resources")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_EXTRACTOR_LOG", t "$APPDATA\Swen\install-timing.log")i'
  Push '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "\
    $$ErrorActionPreference = \"Stop\";\
    $$marker = $$env:SWEN_WATCHDOG_MARKER_PATH;\
    function Write-LobsterWatchdogMarker {\
      param([string] $$value);\
      try {\
        Set-Content -LiteralPath $$marker -Value $$value -NoNewline -ErrorAction Stop\
      } catch {\
        Write-Output (\"SWEN_WATCHDOG_MARKER_WRITE_FAILED:\" + $$value)\
      }\
    };\
    try {\
      $$extractorArgs = \"`\"\" + $$env:SWEN_EXTRACTOR_SCRIPT + \"`\" `\"\" + $$env:SWEN_EXTRACTOR_ARCHIVE + \"`\" `\"\" + $$env:SWEN_EXTRACTOR_DESTINATION + \"`\" `\"\" + $$env:SWEN_EXTRACTOR_LOG + \"`\"\";\
      $$p = Start-Process -FilePath $$env:SWEN_EXTRACTOR_EXE -ArgumentList $$extractorArgs -NoNewWindow -PassThru\
    } catch {\
      Write-LobsterWatchdogMarker \"process-start-blocked\";\
      Write-Output \"SWEN_WATCHDOG_START_BLOCKED\";\
      exit 125\
    };\
    if ($$p.WaitForExit(600000)) {\
      ...
      if ($$p.ExitCode -eq $$null) {\
        $$sentinelOk = $$false;\
        try {\
          $$sentinel = Join-Path $$env:SWEN_EXTRACTOR_DESTINATION \".unpack-cfmind-ok\";\
    ...
    try {\
      ...
      if (-not $$p.WaitForExit(30000)) {\
        ...
        Write-Output \"SWEN_WATCHDOG_TERMINATION_FAILED\";\
        exit 126\
      }\
    } catch {\
      Write-LobsterWatchdogMarker \"process-termination-failed\";\
    ...
    Write-Output \"SWEN_WATCHDOG_TIMEOUT\";\
    exit 124"'
  !insertmacro LobsterExecHiddenExitCode
  Pop $0
  StrCpy $R2 $0
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_WATCHDOG_MARKER_PATH", t "")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_EXTRACTOR_EXE", t "")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_EXTRACTOR_SCRIPT", t "")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_EXTRACTOR_ARCHIVE", t "")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_EXTRACTOR_DESTINATION", t "")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_EXTRACTOR_LOG", t "")i'
  StrCpy $R4 "none"
  ClearErrors
  ...
  MessageBox MB_OK|MB_ICONEXCLAMATION "The Swen installation stopped because resource extraction completed without all required runtime resources ($R5). The installer will not commit a partial application. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
  ...
  TarExtractProcessFailed:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "The Swen installation stopped because the resource extractor could not be started (exit=$R2). The installer will not commit a partial application. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
    Goto TarExtractFailed
  ...
  TarExtractTimeout:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "The Swen installation stopped because resource extraction timed out after 10 minutes. The blocked extractor was terminated and the installer will not commit a partial application. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
    Goto TarExtractFailed
  ...
  TarExtractTerminationFailed:
    ...
    MessageBox MB_OK|MB_ICONEXCLAMATION "The Swen installation stopped because the extractor process could not be confirmed terminated. No automatic rollback or cleanup was attempted while that process may still be writing files. Restart Windows before retrying. Recovery files (if any): $lobsterOldInstallBackupPath. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
    SetErrorLevel 3
    Quit
  ...
  TarExtractOutputValidationFatal:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "The Swen installation stopped because the resource extractor watchdog returned an invalid result. The installer will not commit a partial application. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
    Goto TarExtractFailed
  ...
  TarExtractNonZero:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "The Swen installation stopped because resource extraction failed (child exit code $R2). The installer will not commit a partial application. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
    Goto TarExtractFailed
  ...
  StrCmp $lobsterOldInstallRollbackStatus "failed" 0 TarExtractAbort
    MessageBox MB_OK|MB_ICONEXCLAMATION "The installation failed and automatic rollback did not complete. No recovery copy was deleted. Previous files: $lobsterOldInstallBackupPath. Partial update: $lobsterOldInstallFailedPath. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
  TarExtractAbort:
  SetErrorLevel 3
  Quit
  TarExtractDone:
  ...
  System::Call 'kernel32::GetTickCount()i .r7'
  IfFileExists "$APPDATA\Swen\skills-backup\$lobsterInstallerAttemptId\backup-manifest.json" SkillRestoreAttemptBackupReady
  ...
  SkillRestoreAttemptBackupReady:
    ...
    FileSeek $2 0 END
    !insertmacro GetTimestamp $8
    FileWrite $2 "$8 phase=skill-restore-start attempt_id=$lobsterInstallerAttemptId backup=$APPDATA\Swen\skills-backup\$lobsterInstallerAttemptId$\r$\n"
    FileClose $2

    StrCmp $lobsterTrustedPowerShellPath "" SkillRestoreHelperMissing
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_SKILL_SOURCE", t "$lobsterOldInstallOriginalPath\resources\SKILLs")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_SKILL_BACKUP_ROOT", t "$APPDATA\Swen\skills-backup")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_SKILL_DESTINATION", t "$INSTDIR\resources\SKILLs")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_INSTALL_ATTEMPT_ID", t "$lobsterInstallerAttemptId")i'
    Push '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "\
      $$ErrorActionPreference = \"Stop\";\
      $$attempt   = $$env:SWEN_INSTALL_ATTEMPT_ID;\
      $$root      = $$env:SWEN_SKILL_BACKUP_ROOT;\
      $$source    = $$env:SWEN_SKILL_SOURCE;\
      $$backup    = Join-Path $$root $$attempt;\
      $$newSkills = $$env:SWEN_SKILL_DESTINATION;\
      try {\
        if ([string]::IsNullOrWhiteSpace($$attempt)) { throw \"attempt id missing\" };\
    ...
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_SKILL_SOURCE", t "")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_SKILL_BACKUP_ROOT", t "")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_SKILL_DESTINATION", t "")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_INSTALL_ATTEMPT_ID", t "")i'
    Goto SkillRestoreCommandDone
    ...
    FileSeek $2 0 END
    !insertmacro GetTimestamp $8
    FileWrite $2 "$8 phase=skill-restore-complete attempt_id=$lobsterInstallerAttemptId status=$lobsterLegacySkillsRestoreStatus exit=$R2 elapsed_ms=$5 backup=$APPDATA\Swen\skills-backup\$lobsterInstallerAttemptId$\r$\n"
    ...
    StrCmp $R2 "20" SkillRestoreConflictPreserved
      ...
      StrCmp $lobsterOldInstallRollbackStatus "success" SkillRestoreRollbackSucceeded
        MessageBox MB_OK|MB_ICONEXCLAMATION "The Swen update could not restore user skills, and automatic rollback did not complete. No recovery copy was deleted. Previous files: $lobsterOldInstallBackupPath. Partial update: $lobsterOldInstallFailedPath. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
        Goto SkillRestoreAbort
      SkillRestoreRollbackSucceeded:
        MessageBox MB_OK|MB_ICONEXCLAMATION "The Swen update could not restore user skills, so the previous version was restored. Please retry the update. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
      SkillRestoreAbort:
      SetErrorLevel 2
      Quit
    ...
    SkillRestoreFailurePreserved:
      ...
      FileSeek $2 0 END
      !insertmacro GetTimestamp $8
      FileWrite $2 "$8 phase=skill-restore-degraded attempt_id=$lobsterInstallerAttemptId status=$lobsterLegacySkillsRestoreStatus action=continue-with-attempt-backup-preserved backup=$APPDATA\Swen\skills-backup\$lobsterInstallerAttemptId$\r$\n"
      FileClose $2
      MessageBox MB_OK|MB_ICONEXCLAMATION "Swen will finish installing, but legacy user skills could not be restored automatically ($lobsterLegacySkillsRestoreStatus). The recovery backup was preserved at $APPDATA\Swen\skills-backup\$lobsterInstallerAttemptId. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
    ...
    SkillRestoreDegradedBackupMissing:
      ...
      FileSeek $2 0 END
      !insertmacro GetTimestamp $8
      FileWrite $2 "$8 phase=skill-restore-degraded attempt_id=$lobsterInstallerAttemptId status=$lobsterLegacySkillsRestoreStatus action=continue-no-backup-found backup=$APPDATA\Swen\skills-backup\$lobsterInstallerAttemptId$\r$\n"
      FileClose $2
      MessageBox MB_OK|MB_ICONEXCLAMATION "Swen will finish installing, but the recovery backup for legacy user skills was not found, so no skills were restored ($lobsterLegacySkillsRestoreStatus). Details: $APPDATA\Swen\install-timing.log" /SD IDOK
      Goto SkillRestoreValidated
    ...
    SkillRestoreConflictPreserved:
      ...
      FileSeek $2 0 END
      !insertmacro GetTimestamp $8
      FileWrite $2 "$8 phase=skill-restore-conflict-preserved attempt_id=$lobsterInstallerAttemptId status=name-conflict action=attempt-backup-preserved backup=$APPDATA\Swen\skills-backup\$lobsterInstallerAttemptId$\r$\n"
      FileClose $2
  ...
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_INSTALL_ROOT", t "$INSTDIR")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_DEFENDER_ADD_PERMANENT", t "$R7")i'
  Push '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "\
    $$root = $$env:SWEN_INSTALL_ROOT;\
    ...
    if ($$env:SWEN_DEFENDER_ADD_PERMANENT -ne \"1\") { $$permanent = \"skipped:opt-out\" } else {\
  ...
  Pop $1
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_INSTALL_ROOT", t "")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_DEFENDER_ADD_PERMANENT", t "")i'
  ...
  NewInstallPrevalidateFailed:
    ...
    StrCmp $lobsterOldInstallRollbackStatus "success" NewInstallPrevalidateRollbackSucceeded
      MessageBox MB_OK|MB_ICONEXCLAMATION "The Swen update could not be validated, and automatic rollback did not complete. No recovery copy was deleted. Previous files: $lobsterOldInstallBackupPath. Partial update: $lobsterOldInstallFailedPath. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
      Goto NewInstallPrevalidateAbortAfterMessage
    NewInstallPrevalidateRollbackSucceeded:
      MessageBox MB_OK|MB_ICONEXCLAMATION "The Swen update could not be validated, so the previous version was restored. Please retry the update. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
      Goto NewInstallPrevalidateAbortAfterMessage
    NewInstallPrevalidateAbort:
      MessageBox MB_OK|MB_ICONEXCLAMATION "The Swen installation stopped because the new application could not be validated ($lobsterNewInstallValidationReason). New registration and shortcuts were not written. Details: $APPDATA\Swen\install-timing.log" /SD IDOK
...
!macro customInstall
  ...
  ${If} $lobsterOldInstallRenameStatus == "committed"
    StrCpy $0 "success"
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_OLD_CLEANUP_PATH", t "$lobsterOldInstallBackupPath")i'
    ClearErrors
    StrCmp $lobsterTrustedPowerShellPath "" OldInstallCleanupHelperMissing
    Push '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "Remove-Item -LiteralPath $$env:SWEN_OLD_CLEANUP_PATH -Recurse -Force -ErrorAction SilentlyContinue"'
    !insertmacro LobsterExecHiddenDetached
    IfErrors 0 +2
    ...
    System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_OLD_CLEANUP_PATH", t "")i'
    FileOpen $2 "$APPDATA\Swen\install-timing.log" a
    FileSeek $2 0 END
    !insertmacro GetTimestamp $8
...
!macro customUnInstall
  ...
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_INSTALL_ROOT", t "$INSTDIR")i'
  Push '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "try { $$root = $$env:SWEN_INSTALL_ROOT; $$targets = @($$root, (Join-Path $$root \"resources\cfmind\"), (Join-Path $$root \"resources\python-win\"), (Join-Path $$root \"resources\SKILLs\"), (Join-Path $$root \"resources\app.asar.unpacked\"), (Join-Path $$root \"resources\win-resources.tar\"), (Join-Path $$root \"resources\app.asar\")); Remove-MpPreference -ExclusionPath $$targets -ErrorAction SilentlyContinue } catch {}"'
  !insertmacro LobsterExecHiddenToStack
  Pop $0
  Pop $1
  System::Call 'Kernel32::SetEnvironmentVariable(t "SWEN_INSTALL_ROOT", t "")i'
