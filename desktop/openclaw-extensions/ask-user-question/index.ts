 ...
 * and waits for the user's response via an HTTP callback to Swen.
 *
 * This enables delete-confirmation modals on the Swen desktop app
...
const plugin = {
  ...
  description: 'Structured user confirmation tool for Swen desktop.',
  configSchema: {
  ...
  register(api: OpenClawPluginApi) {
    ...
    // Use a factory so the tool is only available for Swen local-session candidates.
    // IM channel sessions (qqbot, dingtalk, weixin, feishu, etc.) get null → tool hidden.
    api.registerTool((ctx) => {
      // Enable for Swen desktop sessions across agents and delegated child sessions.
