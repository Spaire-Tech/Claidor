...
export default function EventDetailPage({
  ...
  return (
    ...
    >
      <div className=" flex flex-col gap-y-6 rounded-2xl border border-gray-200 bg-gray-50 p-6">
        <div className="flex flex-col gap-y-4">
          <div className="flex flex-row items-start justify-between gap-x-4">
            ...
            {'_cost' in event.metadata && event.metadata._cost && (
              <span className=" font-mono text-3xl text-gray-600 tabular-nums">
...
const DataRow = ({
  ...
  return (
    <div className="flex flex-row items-center gap-x-4">
      ...
      <dd className=" text-gray-500 tabular-nums">
        {value}
      </dd>
    </div>
  )
...
function DataCard({
  ...
  return (
    <div
      className={twMerge(
        ' flex flex-col gap-4 rounded-2xl border border-gray-200 px-4 pt-3.5 pb-4 text-sm transition-colors duration-150 hover:border-gray-300',
  ...
  return (
    <div className="group relative flex gap-x-4">
      <div className="relative flex w-3 justify-center">
        <div className=" absolute top-[10px] z-10 h-3 w-3 rounded-full bg-gray-400" />
        {!isLast && (
      ...
      <div className="flex flex-1 flex-col gap-y-4 pb-8">
        <div className="-ml-1 flex flex-col gap-y-1 rounded-lg p-1 transition-colors duration-150">
          <div className="flex flex-row items-center justify-between gap-2">
            ...
            {costDisplay && (
              <span className=" font-mono text-base text-gray-500 tabular-nums transition-colors duration-150 group-hover:text-gray-700">
                {costDisplay}
              </span>
            )}
          </div>
