  ...
  return (
    <div className="@container flex flex-col gap-2">
      <div className="flex flex-col gap-4">
        ...
        {llmMetadata && (
          <div className=" flex flex-row gap-4 rounded-2xl bg-gray-50 px-4 pt-3.5 pb-4 text-sm text-xs">
            <dl className="flex flex-row items-center gap-4">
              <div className="flex flex-row items-center gap-1 font-mono">
                <dt>
                  <BotIcon className=" size-3 text-gray-500" />
                </dt>
                <dd>
              ...
              <div className="flex flex-row items-center gap-1 font-mono">
                <dt>
                  <ArrowUpFromLineIcon className=" size-3 text-gray-500" />
                </dt>
              ...
              {llmMetadata.cached_input_tokens && (
                <div className="flex flex-row items-center gap-1 font-mono">
                  <dt>
                    <HardDriveDownloadIcon className=" size-3 rotate-180 transform text-gray-500" />
                  </dt>
              ...
              <div className="flex flex-row items-center gap-1 font-mono">
                <dt>
                  <ArrowDownFromLineIcon className=" size-3 text-gray-500" />
                </dt>
                <dd>{llmMetadata.output_tokens}</dd>
              </div>
              <div className="flex flex-row items-center gap-1 font-mono">
                <dt>
                  <SigmaIcon className=" size-3 text-gray-500" />
                </dt>
  ...
  return (
    <div className="group relative flex gap-x-4">
      <div className="relative flex w-3 justify-center">
        <div className=" absolute top-[10px] z-10 h-3 w-3 rounded-full bg-gray-200" />
        {!isLast && (
          <div className=" absolute top-[16px] bottom-[-16px] w-0.5 bg-gray-200" />
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col gap-y-4 pb-8">
        <div className="-ml-1 flex flex-col gap-y-1 rounded-lg p-1 transition-colors duration-150">
          <div className="flex flex-row items-center justify-between gap-2">
            ...
            {costDisplay && (
              <span className=" font-mono text-sm text-gray-500 tabular-nums transition-colors duration-150 group-hover:text-gray-700">
                {costDisplay}
              </span>
            )}
          </div>
        ...
        >
          ...
          >
            {event.label === eventType.label ? (
              <span className=" truncate font-mono font-normal text-gray-600">
                {event.id}
              </span>
            ) : (
              event.label
