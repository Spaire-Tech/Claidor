...
export default function CostsEventsTable({
  ...
  return events.length > 0 && eventTypes.length > 0 ? (
    <div>
      <div className=" w-full border-collapse overflow-hidden rounded-xl border border-gray-200">
        <table className="w-full table-fixed border-collapse rounded-lg">
          <thead>
            <tr>
              {showEventTypes && (
                <th className="  w-48 border-b border-gray-200 bg-gray-100 p-2 text-left text-sm font-medium whitespace-nowrap text-gray-600">
                  Event Type
                </th>
              )}
              <th className="  border-b border-gray-200 bg-gray-100 p-2 text-left text-sm font-medium whitespace-nowrap text-gray-600">
                Event
              </th>
              <th className="  w-64 border-b border-gray-200 bg-gray-100 p-2 text-left text-sm font-medium whitespace-nowrap text-gray-600">
                Customer
              </th>
              <th className="  w-48 border-b border-gray-200 bg-gray-100 p-2 text-left text-sm font-medium whitespace-nowrap text-gray-600">
                Timestamp
              </th>
              <th className="  w-48 border-b border-gray-200 bg-gray-100 p-2 text-right text-sm font-medium whitespace-nowrap text-gray-600">
                Cost
              </th>
            </tr>
          </thead>
          {dayGroups.map((group, groupIndex) => {
            return (
              <tbody
                ...
                className=" group divide-y divide-gray-200"
              >
                <tr className=" bg-gray-50 not-group-first-of-type:border-t">
                  <th
                    colSpan={showEventTypes ? 5 : 4}
                    className=" p-2 text-left text-sm font-normal text-gray-400"
                  >
                    {group.type === 'empty-range' ? (
                      <FormattedInterval
                ...
                {group.type === 'empty-range' ? (
                  <tr>
                    <td
                      colSpan={showEventTypes ? 5 : 4}
                      className=" p-2 text-center text-sm text-gray-400 italic"
                    >
                      No events
                    </td>
                  </tr>
          ...
          <tfoot>
            <tr>
              ...
              >
                {hasNextPage ? (
                  <button
                    className="group   relative flex h-10 w-full cursor-pointer items-center justify-center gap-x-2 py-3 text-sm text-gray-500 transition-colors hover:bg-gray-50 hover:text-gray-700"
                  ...
                  <span className="group  relative flex h-10 w-full items-center justify-center gap-x-2 bg-gray-50 py-3 text-sm text-gray-400">
