...
const SubscriptionTierCard: React.FC<SubscriptionTierCardProps> = ({
  ...
  return (
    <Card
      ...
      className={twMerge(
        ' relative flex flex-col gap-y-6 overflow-hidden rounded-4xl border-none border-gray-200 bg-gray-100 transition-opacity hover:bg-gray-50 hover:opacity-50',
        className,
      )}
    >
      ...
      >
        ...
        {subscriptionTier.description && (
          <div className="prose  shrink leading-normal text-gray-500">
      ...
      {(subscriptionTier.benefits?.length ?? 0) > 0 && (
        <CardFooter className="flex w-full flex-col items-start p-3 pt-0">
          <div className=" flex w-full flex-col gap-y-3 rounded-3xl bg-white p-5">
