...
import AvatarWrapper from '@claidor/ui/components/atoms/Avatar'
...
export const EventsUpsell = () => {
  ...
  return (
    <div className=" relative flex w-full flex-col gap-y-6 overflow-hidden rounded-4xl bg-gray-50 p-2 xl:flex-row">
      ...
      <div className=" flex w-full flex-1 flex-col gap-y-4 rounded-3xl bg-white p-8">
        ...
        >
          ...
          >
            {mockedEvents.map((event, idx) => (
              <motion.div
                key={idx}
                className=" flex w-full flex-row items-center justify-between gap-x-8 rounded-md border border-gray-100 bg-gray-100 p-2 px-4 font-mono text-xs"
              >
                <div className="flex w-fit flex-row items-center gap-x-8">
                  <h3 className="w-full truncate xl:w-36">{event.name}</h3>
                  <p className=" hidden w-28 text-xs text-gray-500 xl:flex">
