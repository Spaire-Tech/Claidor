...
- **Hosted build (the Claidor product)** — the build that talks to the Claidor backend + Supabase, served at `YOUR-DOMAIN.example.com` via Docker + nginx on Dokploy.
Only Claidor (or someone standing up the full hosted stack) needs this.
...
The community build is a folder of static files: your firm runs the add-in on its own AI key (OpenAI, Anthropic, Google Gemini, Groq, Azure OpenAI, or a local Ollama server), with no Claidor backend, no database, and nothing to keep running.
...
Note the resulting URL, for example `https://claidor.yourfirm.com` or the host's `*.pages.dev` / `*.netlify.app` domain.
...
Everything below is for deploying **Claidor's own hosted product**: the build that uses the Claidor backend + Supabase, served at `YOUR-DOMAIN.example.com`.
...
- **This repo** builds to static files and is served by the rootless nginx in the [Dockerfile](Dockerfile). Host it at `YOUR-DOMAIN.example.com`.
- **The backend** (the Claidor API, already running at `YOUR-API-DOMAIN.example.com`) needs exactly one change: allow the add-in origin through CORS.
- **The manifest** ([manifest.xml](manifest.xml)) is already production-shaped (it points at `YOUR-DOMAIN.example.com` and `YOUR-API-DOMAIN.example.com`). You sideload it to test, then submit it to AppSource later.
...
| --- | --- | --- |
| API base | `VITE_API_BASE` build arg | not deployed yet, so there is no hostname to commit; defaults to `localhost:8000` in dev and is required in a hosted build |
...
5. Add a **Domain**: `YOUR-DOMAIN.example.com`, container port `8080`, HTTPS on, certificate provider **Let's Encrypt**.
   Traefik terminates TLS and proxies to nginx over plain HTTP inside the network.
6. Health check path: `/health`.
7. Deploy.

Point `YOUR-DOMAIN.example.com` DNS at the Dokploy host before the certificate can be issued.

## 3. Backend CORS (one change, separate deploy)

The add-in calls `YOUR-API-DOMAIN.example.com` from the `https://YOUR-DOMAIN.example.com` origin, so that origin must be allowed by CORS on the backend.
...
2. Sideload `manifest.xml` (now pointing at the live `YOUR-DOMAIN.example.com`):
