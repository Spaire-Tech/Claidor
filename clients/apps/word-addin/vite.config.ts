...
export default defineConfig(({ mode }) => {
  // `--mode community` selects the bring-your-own-key build. Upstream carried
  // that in a committed `.env.community`, which this repository's root
  // .gitignore excludes along with every other `.env.*`. A missing file would
  // not fail the build: it would produce a *cloud* bundle labelled community,
  // silently. So the mode sets the variable itself, and the dotfile is gone.
  // Vite exposes VITE_-prefixed variables from process.env like any other.
  if (mode === "community") process.env.VITE_EDITION = "community";
  ...
  return {
    ...
    build: {
      outDir: "dist",
      // No source maps in production: nginx blocks .map (return 404), so
      // shipping a bundle that references one just produces console/log 404
      // noise on every load. This thin client has no server-side error
      // reporting that needs them.
      sourcemap: false,
      rollupOptions: {
        input: {
