...
Claidor for Microsoft Word - a Word task-pane add-in. Vite + React 19 + TypeScript SPA, Office.js (WordApi 1.6 floor). Thin client over the Claidor FastAPI backend in `server/`.

> **This tree is a fork of `Vaquill-AI/ms-word-addin` (Apache 2.0).** Read `FORK.md` before changing anything: it says what is ours and what is upstream's, and most of this tree is upstream's. Upstream's own guidance is kept below because the conventions still hold.

> The add-in performs no contract analysis, retrieval, or generation of its own. The engine lives in `server/polar/redline/`; this app reads the open document through Office.js, calls the backend, and applies results back as native Word tracked changes, comments, and content controls. The community (bring-your-own-key) build is the exception: it runs against the user's own AI provider with no backend at all.
...
pnpm dev                             # serve the task pane on https://localhost:3000
pnpm dev:community                   # the same, with no backend
pnpm test                            # vitest
pnpm sideload                        # load manifest.dev.xml into Word desktop
pnpm sideload:stop
...
Word (desktop / Mac / web)
  task pane (YOUR-DOMAIN.example.com)  --Office.js-->  the open document
        |
        |  Supabase JWT (Bearer) + SSE
        v
  Claidor backend (YOUR-API-DOMAIN.example.com)   [required, unchanged except CORS]
```

Two hosted surfaces: the static task-pane SPA at `YOUR-DOMAIN.example.com`, and the unchanged backend at `YOUR-API-DOMAIN.example.com` reached over the same Supabase-JWT bearer auth and SSE contract the web app uses.

### The only required backend change

Add `https://YOUR-DOMAIN.example.com` (and `https://localhost:3000` for dev) to `CORS_ORIGINS` in the backend `app/core/config.py`. Because `CORS_ALLOW_CREDENTIALS` is true, the exact origin must match, so the pane must be served from `YOUR-DOMAIN.example.com`. `Authorization`, `Content-Type`, `X-Organization-ID`, and `X-Timezone` are already whitelisted. Everything else reuses existing endpoints.
...
Office.js cannot set the author of a tracked change. Every programmatic edit made while tracking is on is attributed to the signed-in Word user. When edits must be attributed to "Claidor Contract Review" regardless of the user's identity, use the **server-side export path** (`POST /legal-tools/export-corrected`, inserted via `body.insertFileFromBase64`), not in-pane edits.
...
- `src/config.ts`: `apiBase`/`appBase` come from `VITE_API_BASE` / `VITE_APP_BASE`, defaulting to the local dev servers; Supabase URL + public anon key injected at build (Docker build args). The hosted build asserts `apiBase` at boot. The client bundle holds **no secrets**. `service_role` key must never appear here.
...
- Hosting: static HTTPS, real cert. CSP `script-src` allows `https://appsforoffice.microsoft.com`; `connect-src` includes `YOUR-API-DOMAIN.example.com` + Supabase origins; `frame-ancestors` permits the Office web hosts; never send `X-Frame-Options: DENY`. Ribbon icons must stay cacheable (no `no-store`).
