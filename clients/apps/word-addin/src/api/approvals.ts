 ...
 *
 * Only usable when the reviewed contract has been saved to Claidor (which
...
export class InsufficientAuthorityError extends Error {
  ...
  constructor(approvalLevel: ApprovalLevel, message?: string) {
    super(
      message ||
        `Your account does not have ${approvalLevel} authority to sign this off. Ask someone with the required authority to approve it in Claidor.`,
    );
