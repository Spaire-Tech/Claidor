...
export const GOVERNANCE_NS = "urn:claidor:governance:1";
...
export interface GovernanceLedger {
  ...
  matterId?: string;
  /**
   * The saved Claidor draft id, present only when the reviewed contract was
   * saved to Claidor (which yields a draft id). Its presence is what lets the
...
export interface ReviewMeta {
  ...
  playbookId?: string;
  matterId?: string;
  /** Saved Claidor draft id, when the contract was already saved. */
  draftId?: string;
}
