...
type Props = (
  ...
  /**
   * Fired after the contract is saved to Claidor and a draft id is known
...
/**
 * Save the reviewed contract or generated draft back into Claidor: as an
 ...
 */
export function SaveToClaidor(props: Props) {
  // Saving back into the hosted Claidor product is a hosted-plan feature. In the
  // community edition, show a locked upsell instead of hiding it.
  if (isCommunity()) return <UpgradeLink label="Save to Claidor (hosted)" />;
  ...
  async function saveDraft() {
    ...
    try {
      if (props.mode === "draft") {
        ...
        props.onSaved?.(id);
        setSaved({
          label: matterId ? "Filed under the matter." : "Open your draft in Claidor.",
      ...
      setSaved({
        label: "Saved to Claidor with redlines.",
  ...
  const draftLabel =
    props.mode === "review"
      ? "Open in Claidor (with redlines)"
      : matterId
        ? "File under matter"
        : "Open in Claidor drafting";

  return (
    <div className="card doc-tools">
      <h2 className="small muted" style={{ margin: 0 }}>Save to Claidor</h2>
