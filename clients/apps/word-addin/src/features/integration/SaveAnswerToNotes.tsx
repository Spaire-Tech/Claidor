...
/**
 * Save an assistant answer as a note on a Claidor client. Lazy: it loads
...
export function SaveAnswerToNotes({
  ...
  async function save() {
    ...
    setError(null);
    try {
      await createClientNote(clientId, { content, title: "Note from Claidor for Word" });
      setDone(true);
      setOpen(false);
    } catch (e) {
  ...
  // Saving to a client note needs a hosted Claidor account (clients live in
