...
function commentText(r: AuthorityResult): string {
  ...
  if (r.kind === "statute") {
    if (r.verdict === "verified") {
      ...
      return `${label}: resolved in Claidor's US statutes corpus. Confirm it is current (not amended or repealed) before relying on it.${source}`;
    }
    return `Could not resolve ${r.raw} in Claidor's US statutes corpus. Verify this citation manually before relying on it.`;
  }
  if (r.verdict === "verified") {
    ...
    return `${name}${yr}: found in Claidor's US case-law corpus. Confirm current treatment (not overruled or superseded) before relying on it.${treatment}${source}`;
  }
            ...
            if (unresolved) {
              items.push({
                ...
                onSelect: () =>
                  navigate("assistant", {
                    kind: "assistantAsk",
                    prompt: `The citation "${result.raw}" did not resolve in Claidor's US corpus. Help me find the correct authority, or tell me whether it looks misstated or nonexistent.`,
                    autoSend: true,
                  }),
              });
            }
