 ...
 * matches the one placed at review time (`Claidor_clause_${index + 1}`). `text`
...
async function jumpToItem(item: MemoItem): Promise<boolean> {
  ...
  try {
    if (
      ...
      (await goToBookmark(`Claidor_clause_${loc.bookmarkIndex + 1}`))
    ) {
      return true;
    }
...
export function ReviewMemo({
  ...
  // (native tracked changes, authored "Claidor Contract Review"). Insertions
