...
import { lockClaidorControls } from "@/office/contentControls";
import { stampClaidorReview } from "@/office/properties";
      ...
      <div className="banner banner--gov-signed">
        ...
        <p className="small muted" style={{ margin: "4px 0 0" }}>
          {ledger.signoffEnforced
            ? `Authority checked by Claidor${roleLabel ? ` (recorded as ${roleLabel})` : ""}.`
    ...
    <div className="gov-action stack">
      ...
      <p className="small muted" style={{ margin: 0 }}>
        {enforced
          ...
          : "Stamps your name and level as an attestation in the file (not authority-checked). Save to Claidor first for an enforced approval."}
      </p>
    </div>
  );
}
...
function LockControl() {
  ...
  async function toggle() {
    ...
    setNote(null);
    try {
      const n = await lockClaidorControls(!locked);
      if (n === 0 && !locked) {
  ...
  return (
    <div className="gov-action stack">
      ...
      <p className="small muted" style={{ margin: 0 }}>
        Make the fields Claidor tagged (amounts, dates, defined terms) read-only, so nobody edits the
...
export function GovernanceView() {
  ...
  return (
    <div className="stack governance-view">
      ...
      {integrity === "modified" && (
        <p className="small muted" style={{ margin: 0 }}>
          This record was changed outside Claidor. Re-run the review to re-establish it.
        </p>
      )}
