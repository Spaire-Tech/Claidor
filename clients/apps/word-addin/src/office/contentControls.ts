 ...
 *
 * Every control we create carries the same tag (CLAIDOR_TAG) so we can find,
...
export const CLAIDOR_TAG = "claidor";
...
/**
 * Lock (or unlock) every Claidor-tagged content control. This is how sign-off
 * physically prevents further edits to the reviewed clauses. Returns the count.
 */
export async function lockClaidorControls(lock: boolean): Promise<number> {
  return runWord(async (context) => {
    const controls = context.document.contentControls;
    controls.load("tag");
    await context.sync();

    const ours = controls.items.filter((cc) => cc.tag === CLAIDOR_TAG);
