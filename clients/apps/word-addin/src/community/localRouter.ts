...
/**
 * Community request shim. Replaces `fetch(YOUR-API-DOMAIN.example.com)` for the JSON API when
 ...
 * needs Claidor's hosted data or account says so instead of failing oddly.
 */
...
function requiresAccount(): never {
  throw new ApiError(
    "unknown",
    0,
    "This feature needs a Claidor account and is not available in the community edition.",
    "REQUIRES_ACCOUNT",
  );
}
