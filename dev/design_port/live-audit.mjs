...
const sweep = async (where) => {
  ...
  for (const demo of DEMO_ONLY) {
    if (text.includes(demo)) offenders.push(`${where}: ${demo}`)
  }
}
  ...
  .evaluate(() => {
    ...
    while (queue.length) {
      ...
      if (node.stateNode && node.stateNode.setState && node.stateNode.state) {
        node.stateNode.setState({ searchOpen: true, searchQ: '', menu: 'client' })
        return
      }
      if (node.child) queue.push(node.child)
      if (node.sibling) queue.push(node.sibling)
...
console.log(`LIVE AUDIT: OK (${VIEWS.length} views + palette, no demo data)`)
process.exit(0)
