...
if __name__ == "__main__":
    ...
    seeded_debt = None
    for pair in structure.pairs:
        if pair.sheet not in {
            b.sheet for b in structure.located if b.kind == "debt-schedule"
        }:
            continue
        ...
        for c, _ in axis.columns:
            ...
            cell = source.cells.get(
                f"{pair.sheet}!{get_column_letter(c)}{pair.closing_row}"
            )
    ...
    if sub is None:
        ...
        opening_values = {
            ...
            if c.sheet == sub.sheet and c.row == sub.opening_row and c.value is not None
        }
        ...
        print(
            f"interest-departure: enabled 5% on row {interest_row}, "
        ...
        if not after:
            ...
            print(
                f"interest-after-repayment: enabled 5% on row "
