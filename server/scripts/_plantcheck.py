...
for label, path in (("host", "scripts/corpus_sft/inverness_college_model.xlsm"),
                    ("planted", str(planted))):
    b = read_workbook(path)
    rows = rows_from_cells(b.cells, sheet)
    labels = classify_sheet(rows)
    lab = labels.get((sheet, row))
    fmts = sorted({c.number_format for c in b.cells.values()
                   if c.sheet == sheet and c.row == row})
    print(f"{label:<8} row {row}: currency={lab.currency if lab else 'NO ROW'} fmts={fmts[:2]}")
    cell = b.cells.get(ref)
    if cell:
        terms = [(p, labels.get((sheet, b.cells[p].row)).currency if p in b.cells and (sheet, b.cells[p].row) in labels else '?')
                 for p in (cell.precedents or ())]
        print(f"         {ref} = {cell.formula[:50]}  precedent currencies: {terms[:6]}")
