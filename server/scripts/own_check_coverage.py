...
def one(path: Path) -> dict:
    ...
    for key, valued in sorted(rows.items()):
        ...
        kept = valued if axis is None else [
            o for o in valued if o[0].column in {c for c, _ in axis.columns}
        ]
        now = admitted(kept)
        before += was
        after += now
        if was and not now:
            dropped.append({
                ...
                "axis_from": (L(axis.columns[0][0]) if axis and axis.columns else None),
...
def main() -> None:
    ...
    print(f"corpus: {tb} -> {ta} check rows examined, "
          f"{sum(len(r['dropped']) for r in results)} dropped", flush=True)
