...
class TestBalance:
    ...
    def test_an_imbalance_is_a_finding_with_its_period(self) -> None:
        result = _run(
            self._sheet([100, 100, 100, 100, 100, 1100], [100, 100, 100, 100, 100, 100])
        )
    ...
    def test_the_models_own_passing_check_wins(self) -> None:
        ...
        assert any(
            "disagree" in a.why for a in result.abstentions if a.rule == "balance-sheet"
        )
    ...
    def test_rounding_residue_stays_silent(self) -> None:
        result = _run(
            self._sheet(
                [100, 100, 100, 100, 100, 100.0003], [100, 100, 100, 100, 100, 100]
            )
        )
        assert [f for f in result.findings if f.rule == "balance-sheet"] == []
...
class TestCashContinuity:
    ...
    def test_a_clean_carry_says_nothing(self) -> None:
        result = _run(self._pair([0, 10, 20, 30, 40, 50], [10, 20, 30, 40, 50, 60]))
    ...
    def test_a_break_in_a_living_account_is_a_finding(self) -> None:
        ...
        result = _run(self._pair([0, 10, 999, 30, 40, 50], [10, 20, 30, 40, 50, 60]))
    ...
    def test_a_wind_down_is_not_a_break(self) -> None:
        ...
        result = _run(self._pair([0, 10, 20, 30, 0, 0], [10, 20, 30, -5, 40, 0]))
        assert [f for f in result.findings if f.rule == "cash-continuity"] == []
    ...
    def test_wholesale_disagreement_claims_nothing(self) -> None:
        ...
        result = _run(self._pair([0, 1, 2, 3, 4, 5], [66, 77, 88, 99, 111, 122]))
        assert [f for f in result.findings if f.rule == "cash-continuity"] == []


class TestDebtTerminal:
    def _tranche(
        self, closing: list[float], label: str = "Senior loan closing balance"
    ) -> list:
        count = len(closing)
        cells = [
        ...
        cells += [
            _cell("Debt", f"C{i}14", 14, i + 3, value=Decimal(str(v)), row_label=label)
            for i, v in enumerate(closing)
        ]
        return cells
...
class TestInterestConsistency:
    ...
    def _fired(self, cells: list) -> list:
        return [f for f in _run(cells).findings if f.rule == "interest-consistency"]
    ...
    def test_a_steady_rate_says_nothing(self) -> None:
        ...
        assert [f for f in result.findings if f.rule == "interest-consistency"] == []
    ...
    def test_two_interest_rows_abstain_by_name(self) -> None:
        ...
        result = _run(self._tranche(opening, interest, second_interest=interest))
        assert [f for f in result.findings if f.rule == "interest-consistency"] == []
        assert any(
    ...
    def test_a_dead_second_row_does_not_block_association(self) -> None:
        ...
        interest = [5, 4.5, 4, 22.5, 3, 2.5, 2]
        fired = self._fired(
            self._tranche(opening, interest, second_interest=[0] * len(opening))
        )
        assert len(fired) == 1
        assert fired[0].period == "FY2021"
...
class TestGrading:
    ...
    def _axis(self) -> list[Cell]:
        return [
            _cell("S", f"H{i}1", 1, i + 3, value=None, column_label=f"FY{2020 + i}")
            for i in range(6)
        ]
