...
class TestBalancePairs:
    ...
    def test_vocabulary_pairs_match_by_account(self) -> None:
        pairs = balance_pairs(_book(self._sheet()), period_axes(_book(self._sheet())))
        assert [(p.sheet, p.opening_row, p.closing_row, p.label) for p in pairs] == [
            ("Debt", 10, 14, "dsra")
        ]
...
class TestLocation:
    ...
    def test_values_pasted_is_a_fact_not_a_failure(self) -> None:
        cells = [
            _cell("S", f"C{row}", row, 3, value=Decimal(row)) for row in range(1, 1500)
        ]
