...
from polar.desktop.endpoints import (
...
#: A picture, as OpenAI hands one back: a 1x1 PNG, base64. Small enough
#: to read, real enough that the app's decoder would accept it.
ONE_PIXEL_PNG = (
...
def _images_answer(count: int = 1) -> dict[str, Any]:
    return {"created": 1, "data": [{"b64_json": ONE_PIXEL_PNG} for _ in range(count)]}


@pytest.mark.asyncio
class TestImageModels:
    """The menu the agent may read before it draws."""

    async def test_it_offers_the_one_model_with_what_the_app_reads(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
        ...
        access, _ = await _signed_in(client, session, user)
        response = await client.get(
            "/desktop/api/media/images/models",
            headers={"Authorization": f"Bearer {access}"},
        )
        assert response.status_code == 200
        body = response.json()
        ...
        rows = body["data"]
        assert len(rows) == 1
        # `main.ts` reads exactly these three off each row when it builds
        # the text the agent sees.
        assert rows[0]["modelId"] == IMAGE_MODEL_ID
        assert rows[0]["displayName"]
        assert rows[0]["capabilities"]

    async def test_no_key_reads_as_nothing_available_and_not_as_an_error(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
        ...
        # The rule `offered_models()` follows: a missing key must read as
        # « not available here » when the menu is drawn, never as a
        # failure at the moment somebody asks for a picture.
        mocker.patch.object(settings, "OPENAI_API_KEY", "")
        access, _ = await _signed_in(client, session, user)
        response = await client.get(
            "/desktop/api/media/images/models",
    ...
    async def test_video_says_none_in_the_app_s_words_rather_than_404(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
    ) -> None:
        # A 404 reaches the agent as an unparsed failure it may retry; an
        # empty list reaches it as "No video models available."
        access, _ = await _signed_in(client, session, user)
        response = await client.get(
            "/desktop/api/media/videos/models",
            headers={"Authorization": f"Bearer {access}"},
        )
        assert response.status_code == 200
        assert response.json() == {"code": 0, "data": []}
    ...
    async def test_it_needs_a_session(self, client: httpx.AsyncClient) -> None:
        response = await client.get("/desktop/api/media/images/models")
        assert response.status_code == 401


@pytest.mark.asyncio
class TestImageGenerate:
    async def test_a_picture_is_drawn_forwarded_and_metered(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
        ...
        with respx.mock(assert_all_called=True) as mock:
            route = mock.post(
                f"{settings.DESKTOP_OPENAI_BASE_URL}/images/generations"
            ).mock(return_value=httpx.Response(200, json=_images_answer()))
            response = await client.post(
                "/desktop/api/media/images/generate",
                headers={"Authorization": f"Bearer {access}"},
                json={
                    "model": "",
                    "type": "image",
        ...
        # The shape `main.ts` reads. `succeeded` is the whole point: it is
        # what sends the app down the terminal branch and stops it ever
        # registering a poll.
        assert data["status"] == "succeeded"
        assert data["model"] == IMAGE_MODEL_ID
        assert data["taskId"]
        assert len(data["resultUrls"]) == 1
        # A data URL, because `mediaAssetPersistence.ts` decodes one inline
        # and never goes back to the network for it.
        assert data["resultUrls"][0].startswith("data:image/png;base64,")
        assert ONE_PIXEL_PNG in data["resultUrls"][0]

        sent = json.loads(route.calls[0].request.content)
        assert sent["model"] == IMAGE_MODEL_ID
        assert sent["prompt"] == "a red door"
        assert sent["n"] == 1
        assert sent["size"] == IMAGE_SIZE_DEFAULT
        assert route.calls[0].request.headers["authorization"] == "Bearer sk-test"

        rows = (await session.execute(DesktopUsage.__table__.select())).all()
        assert len(rows) == 1
        assert rows[0].model == IMAGE_MODEL_ID
        assert rows[0].credits == credits_for_image(IMAGE_SIZE_DEFAULT, 1)
        assert rows[0].credits > 0

    async def test_the_price_follows_how_many_came_back_not_how_many_were_asked_for(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
        ...
        # A call told to draw three that returns two is charged for two.
        mocker.patch.object(settings, "OPENAI_API_KEY", "sk-test")
        access, _ = await _signed_in(client, session, user)
        with respx.mock(assert_all_called=True) as mock:
            mock.post(f"{settings.DESKTOP_OPENAI_BASE_URL}/images/generations").mock(
                return_value=httpx.Response(200, json=_images_answer(2))
            )
            response = await client.post(
                "/desktop/api/media/images/generate",
                headers={"Authorization": f"Bearer {access}"},
                json={"prompt": "three doors", "params": {"n": 3}},
            )
        assert response.status_code == 200
        assert len(response.json()["data"]["resultUrls"]) == 2
        rows = (await session.execute(DesktopUsage.__table__.select())).all()
        assert rows[0].credits == credits_for_image(IMAGE_SIZE_DEFAULT, 2)

    async def test_a_size_asked_for_is_the_size_drawn_and_the_size_priced(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
        ...
        with respx.mock(assert_all_called=True) as mock:
            ...
            response = await client.post(
                "/desktop/api/media/images/generate",
                headers={"Authorization": f"Bearer {access}"},
                json={"prompt": "a wide view", "params": {"size": "1536x1024"}},
            )
        assert response.status_code == 200
        assert json.loads(route.calls[0].request.content)["size"] == "1536x1024"
        rows = (await session.execute(DesktopUsage.__table__.select())).all()
        assert rows[0].credits == credits_for_image("1536x1024", 1)

    async def test_an_aspect_ratio_is_read_as_a_size(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
        ...
        # `main.ts` fills `aspectRatio` on some paths and `size` on
        # others; both have to land somewhere real.
        mocker.patch.object(settings, "OPENAI_API_KEY", "sk-test")
        access, _ = await _signed_in(client, session, user)
        with respx.mock(assert_all_called=True) as mock:
            route = mock.post(
            ...
            await client.post(
                "/desktop/api/media/images/generate",
                headers={"Authorization": f"Bearer {access}"},
                json={"prompt": "a tall tree", "params": {"aspectRatio": "2:3"}},
            )
        assert json.loads(route.calls[0].request.content)["size"] == "1024x1536"

    async def test_a_size_we_do_not_draw_is_refused_before_openai_and_costs_nothing(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
        ...
        access, _ = await _signed_in(client, session, user)
        with respx.mock(assert_all_called=False) as mock:
            route = mock.post(f"{settings.DESKTOP_OPENAI_BASE_URL}/images/generations")
            response = await client.post(
                "/desktop/api/media/images/generate",
                headers={"Authorization": f"Bearer {access}"},
                json={"prompt": "a door", "params": {"size": "64x64"}},
            )
            assert not route.called
        body = response.json()
        assert body["code"] != 0
        # The refusal has to say which sizes exist, or the agent's only
        # move is to guess again.
        assert IMAGE_SIZE_DEFAULT in body["message"]
        rows = (await session.execute(DesktopUsage.__table__.select())).all()
        assert rows == []

    async def test_drawing_from_a_picture_is_refused_by_name(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
        ...
        # `/v1/images/edits` is a different endpoint with its own price
        # and is not served. Refused by name rather than quietly ignored:
        # an ignored reference image makes a picture that is wrong in a
        # way the agent cannot see.
        mocker.patch.object(settings, "OPENAI_API_KEY", "sk-test")
        access, _ = await _signed_in(client, session, user)
        with respx.mock(assert_all_called=False) as mock:
            route = mock.post(f"{settings.DESKTOP_OPENAI_BASE_URL}/images/generations")
            response = await client.post(
                ...
                headers={"Authorization": f"Bearer {access}"},
                json={
                    "prompt": "like this one but blue",
                    "params": {"images": ["data:image/png;base64,xx"]},
                },
            )
            assert not route.called
        body = response.json()
    ...
    async def test_more_pictures_than_the_cap_are_refused_rather_than_clamped(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
        ...
        # Drawing three when four were asked for, and charging for three,
        # is a picture missing from a document with nothing saying why.
        mocker.patch.object(settings, "OPENAI_API_KEY", "sk-test")
        access, _ = await _signed_in(client, session, user)
        with respx.mock(assert_all_called=False) as mock:
            route = mock.post(f"{settings.DESKTOP_OPENAI_BASE_URL}/images/generations")
            response = await client.post(
                "/desktop/api/media/images/generate",
                headers={"Authorization": f"Bearer {access}"},
                json={"prompt": "a crowd", "params": {"n": IMAGE_MAX_IMAGES + 1}},
            )
            assert not route.called
        assert response.json()["code"] != 0
        assert str(IMAGE_MAX_IMAGES) in response.json()["message"]
    ...
    async def test_the_model_asked_for_is_ignored_and_the_reason_is_said(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
        ...
        # The founder, 16 September: *"my users should never put a key.
        # everything happens under the hood. not a setting."* A caller
        # naming another model gets a picture and an explanation, not a
        # refusal it has to recover from. `modelSelectionReason` is a
        # field the app already prints.
        mocker.patch.object(settings, "OPENAI_API_KEY", "sk-test")
        access, _ = await _signed_in(client, session, user)
        with respx.mock(assert_all_called=True) as mock:
            route = mock.post(
            ...
            response = await client.post(
                "/desktop/api/media/images/generate",
                headers={"Authorization": f"Bearer {access}"},
                json={"model": "seedream-3", "prompt": "a door", "params": {}},
            )
        assert json.loads(route.calls[0].request.content)["model"] == IMAGE_MODEL_ID
        data = response.json()["data"]
        assert data["model"] == IMAGE_MODEL_ID
        assert data["modelSelectionReason"]

    async def test_openai_s_own_sentence_comes_back_and_nothing_is_charged(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
        ...
        # The lesson of 13 September: a status without the provider's
        # sentence costs hours. A refused prompt is the common case here
        ...
        with respx.mock(assert_all_called=True) as mock:
            ...
            response = await client.post(
                "/desktop/api/media/images/generate",
                headers={"Authorization": f"Bearer {access}"},
                json={"prompt": "something refused", "params": {}},
            )
        body = response.json()
        assert body["code"] != 0
        assert "safety system" in body["message"]
        # Nothing was drawn, so nothing is charged. Unlike speech, where
        # the provider has done the work by the time it answers.
        rows = (await session.execute(DesktopUsage.__table__.select())).all()
        assert rows == []

    async def test_an_answer_with_no_pictures_in_it_is_not_charged_for(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
        ...
        with respx.mock(assert_all_called=True) as mock:
            ...
            response = await client.post(
                "/desktop/api/media/images/generate",
                headers={"Authorization": f"Bearer {access}"},
                json={"prompt": "a door", "params": {}},
            )
        assert response.json()["code"] != 0
        rows = (await session.execute(DesktopUsage.__table__.select())).all()
        assert rows == []

    async def test_no_openai_key_is_refused_before_anything_is_attempted(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
        ...
        assert response.json()["code"] == IMAGE_NOT_CONFIGURED

    async def test_an_exhausted_allowance_uses_the_code_the_app_understands(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
        ...
        # **Not** `QUOTA_EXHAUSTED_CODE`. The app's media handler knows
        # 40203 and 40204 on this path and nothing else (`main.ts:6569`,
        # `main.ts:6577`); 40200 would fall through to its generic branch
        # and the person would read "Media generation request failed"
        # instead of being told their month ran out.
        mocker.patch.object(settings, "OPENAI_API_KEY", "sk-test")
        access, _ = await _signed_in(client, session, user)
        await save_fixture(
            DesktopUsage(
        ...
        with respx.mock(assert_all_called=False) as mock:
            ...
            assert not route.called
        assert response.json()["code"] == IMAGE_QUOTA_EXHAUSTED
        assert response.json()["code"] != QUOTA_EXHAUSTED_CODE

    async def test_a_lost_usage_row_does_not_cost_the_person_the_picture(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
        ...
        # OpenAI has drawn it and the money has already gone, so handing
        # back a 500 would lose the person both the picture and the
        # payment. The rollback is what makes that true: `get_db_session`
        # commits this session when the handler returns, and a session
        # left in a failed transaction raises *there* — so without it the
        # swallow would simply happen one frame further out.
        mocker.patch.object(settings, "OPENAI_API_KEY", "sk-test")
        access, _ = await _signed_in(client, session, user)
        mocker.patch.object(
            desktop, "record_usage", side_effect=RuntimeError("no room at the inn")
        )
        with respx.mock(assert_all_called=True) as mock:
            mock.post(f"{settings.DESKTOP_OPENAI_BASE_URL}/images/generations").mock(
                return_value=httpx.Response(200, json=_images_answer())
            )
            response = await client.post(
                "/desktop/api/media/images/generate",
                headers={"Authorization": f"Bearer {access}"},
                json={"prompt": "a red door", "params": {}},
            )
        assert response.status_code == 200
        body = response.json()
        assert body["code"] == 0
        assert len(body["data"]["resultUrls"]) == 1
        # The figure is left out rather than guessed at; the app only
        # prints it when it is there (`main.ts:6644`).
        assert "quotaRemaining" not in body["data"]

    async def test_it_needs_a_session(self, client: httpx.AsyncClient) -> None:
        response = await client.post(
            "/desktop/api/media/images/generate", json={"prompt": "a door"}
        )
        assert response.status_code == 401
...
@pytest.mark.asyncio
class TestImageTask:
    async def test_polling_is_answered_with_a_sentence_that_ends_the_loop(
        self,
        client: httpx.AsyncClient,
        session: AsyncSession,
        user: User,
    ) -> None:
        # Drawing is synchronous, so the app never registers a poll
        # (`main.ts:6679`) and this is only reached by an agent following
        # the polling half of its brief out of habit. A 404 would read to
        # it as a transient fault worth retrying.
        access, _ = await _signed_in(client, session, user)
        response = await client.get(
            "/desktop/api/media/images/tasks/whatever",
            headers={"Authorization": f"Bearer {access}"},
        )
        assert response.status_code == 200
        body = response.json()
        assert body["code"] != 0
        assert "no task" in body["message"].lower()
