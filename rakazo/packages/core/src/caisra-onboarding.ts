/**
 * Yodo's first step, and how it streams.
 *
 * **Every sentence here is the founder's, from the canvas of 16 September
 * 2026, verbatim.** This file is a port of
 * `desktop/src/shared/onboarding/script.ts` and nothing in the copy is new: the
 * greeting, the ten roles and their ten replies, the three task cards, the
 * three endings. One of those endings exists because the canvas had only "Done"
 * and "No bother", and saying "Done" over a note that was never written is the
 * lie the app must not tell — so a task that was allowed and then failed gets
 * its own words.
 *
 * How it streams is the canvas's too: thirteen words a second, a pause of three
 * word-beats between lines, each word fading in as it lands. `shownWords` is
 * that arithmetic, pure, so the renderer's only job is to keep a clock and
 * draw.
 *
 * The three tasks are the Mac's. On a hosted backend there is no Mac to reach,
 * and the design already has an answer for that: a task the computer cannot do
 * is drawn dim and says "Not yet". Nothing here pretends otherwise.
 */
...
export const OnboardingTask = {
  ...
  /** Text a pep talk for tomorrow morning. */
  Messages: "messages",
} as const;
...
export interface OnboardingStatus {
  /** `darwin`, `win32`, `linux`, or nothing when there is no computer to ask. */
  platform: string;
  /** What the Mac is showing now, so the card offers the other one. */
  appearance: Appearance;
  /** Which of the three can actually be done here today. */
...
/**
 * The one riddle. The founder: *"if its a riddle, have one ready. no generate
 * new one, lets just have one that we save."*
 */
export const RIDDLE = {
...
/** The mark on each task card, by the slug our own logos are filed under. */
export const ONBOARDING_LOGOS: Record<OnboardingTask, string> = {
  [OnboardingTask.Notes]: "apple-notes",
  [OnboardingTask.Appearance]: "macos-settings",
  [OnboardingTask.Messages]: "imessage",
};
...
export const ROLES: readonly Role[] = [
  ...
  { label: "Engineering / Tech", reply: "Right. I'll stay literal and leave the detail in." },
  {
    ...
    reply: "Thought so. I'll keep things short and decision-shaped.",
  },
  {
    label: "Sales / Marketing",
    reply: "Pipeline work. I'll keep you in front of people instead of admin.",
  },
  {
    label: "Office Work",
    reply: "The stuff nobody thanks you for. That's the easiest to take off you.",
  },
  {
    label: "Content Creator",
    reply: "Output work. I'll keep the drafts moving so you're never starting cold.",
  },
  { label: "Design / Creative", reply: "Fine. I'll keep the output visual where that helps." },
  {
    label: "Consultant / Freelance",
    reply: "Many masters. I'll keep the clients from blurring into each other.",
  },
  { label: "Student", reply: "Deadlines and reading. I can carry most of that." },
  {
    label: "Slacker (my kind)",
    reply: "Ha. My kind of client. We'll find you agents who work while you don't.",
  },
];
  ...
  return [
    {
      ...
      permLine:
        "I can do that, but I'll need access to Notes to write it in there. Mind allowing that?",
    ...
    },
    {
      ...
      permLine:
        "Easy enough. I'll need permission to change your system appearance. Mind allowing that?",
    ...
    },
    {
      ...
      permSub: "App access requested",
      permLine:
        "Can do. I'll need access to Messages to send it to you in the morning. Mind allowing that?",
...
export const PHASES: readonly OnboardingPhase[] = [
  OnboardingPhase.Open,
  OnboardingPhase.Role,
  OnboardingPhase.Perm,
  OnboardingPhase.Task,
];
