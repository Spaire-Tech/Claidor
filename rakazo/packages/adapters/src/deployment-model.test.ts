...
describe("Claidor, when it is configured", () => {
  it("is the model service, and carries its address with its key", () => {
    ...
    expect(resolveDeploymentModel({ CLAIDOR_ACCESS_TOKEN: "claidor_pat_x" })).toEqual({
  ...
  it("wins over every other provider, whatever else is set", () => {
    ...
    const resolved = resolveDeploymentModel({
      CLAIDOR_ACCESS_TOKEN: "claidor_pat_x",
  ...
  it("takes an operator's own address and model when given them", () => {
    expect(
      resolveDeploymentModel({
        CLAIDOR_ACCESS_TOKEN: "claidor_pat_x",
        CLAIDOR_MODEL_BASE_URL: "http://127.0.0.1:8000/desktop/api/proxy/v1",
  ...
  it("leaves the bring-your-own-key providers alone when it is absent", () => {
    ...
    // deleted.
    const blank = resolveDeploymentModel({
      CLAIDOR_ACCESS_TOKEN: "   ",
      OPENROUTER_API_KEY: "or-key",
    });
...
describe("the menu Claidor serves", () => {
  ...
  it("names the models in plain words, with no key to enter", () => {
    const menu = claidorCatalog({ CLAIDOR_ACCESS_TOKEN: "claidor_pat_x" });
  ...
  it("never lists Claude, because that wire cannot reach it", () => {
    ...
    const ids = claidorCatalog({ CLAIDOR_ACCESS_TOKEN: "claidor_pat_x" }).map((one) => one.id);
...
describe("the cheap model, for work nobody reads", () => {
  it("comes with Claidor, and is not the everyday one", () => {
    ...
    const resolved = resolveDeploymentModel({ CLAIDOR_ACCESS_TOKEN: "claidor_pat_x" });
    expect(resolved.model).toBe(CLAIDOR_DEFAULT_MODEL_ID);
