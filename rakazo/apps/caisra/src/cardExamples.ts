/**
 * The founder's OpenUI pictures as programs in OpenUI's chat library.
 *
 * Moved across from `desktop/harness/cardExamples.ts` unchanged: the four of
 * 17 September — Seattle restaurants, Paris hotels, a Tokyo itinerary, the
 * 2026 World Cup — and the composed answer of the 18th, the fourteen-day meal
 * plan they looked at and said *"i've never seen anything quite like this.
 * its perfect."*
 *
 * They are here rather than invented because a screenshot is only worth
...
const book = (context: string): string =>
  `Button("Book", {type: "continue_conversation", context: "${context}"}, "primary", "normal", "small")`;
...
export function cardExamples(photo: (name: string) => string): Record<string, CardExample> {
  return {
    ...
    "cards-hotels": {
      ...
      before:
        "Five on the Right Bank, all within a walk of each other. Prices are for next Tuesday, one night, from the hotels’ own sites.",
      program: [
      ...
      after:
        "The Meurice is the one I’d take for a week: it is the cheapest of the five and the only one facing the park.",
    },
    "cards-trip": {
    ...
    "cards-fifa": {
      ...
      before:
        "The first with 48 teams and the first across three countries. It opens on 11 June in Mexico City and ends on 19 July in New Jersey.",
      program: [
